...
1. **Backup exfiltration** — a Time Machine snapshot, a Dropbox sync, or any backup tool that copies `.codex/` reads with the *backup user's* permissions, not the original mode.
...
|---|---|---|---|
| Session JSON | `.codex/sessions/*.json` | **High** | Bundles memory snapshots + agent prompts on save |
| Memory DB (sql.js SQLite) | `.swarm/memory.db` | **High** | Conversation entries + 384-dim embeddings; embeddings leak topics |
| Terminal history | `.codex/terminals/store.json` | **High** | Pasted shell commands often include API keys, DB passwords |
| Agent registry | `.codex/agents/store.json` | Medium | Agent configs incl. model + domain; no prompts |
| Task store | `.codex/tasks/store.json` | Medium | Task descriptions + status |
| Claims / config / workflow / neural / DAA / GitHub stores | various under `.codex/` | Low–Medium | Mostly metadata; case-by-case |
| Update history | `~/.codex/update-history.json` | Low | Package names + versions; relevant only for the integrity gate added in `c1b57e4f` |
...
**Posture**: opt-in via `RUFLO_ENCRYPT_AT_REST=1`. Default off so the 1865 existing tests and current users keep working unchanged.
...
1. `RUFLO_ENCRYPTION_KEY` — base64-encoded 32 bytes. Highest precedence, useful for CI / containers / users who already have a secret manager.
2. OS keychain — `keytar`-style lookup under service `codex`, account `default`. macOS Keychain, Windows DPAPI, libsecret on Linux. **Optional dependency**: `keytar` is a native module; if unavailable, fall back to (3).
3. Passphrase prompt + scrypt KDF — interactive only. Stored derived key in process memory for the session, never on disk. Salt persisted at `~/.codex/.kdf-salt` (16 bytes random, mode 0600).

If `RUFLO_ENCRYPT_AT_REST=1` and *no* key source resolves, the CLI **errors immediately** rather than silently writing plaintext. Fail-closed posture.
...
7. **Documentation** — `docs/security/encryption.md` covers user-facing setup, recovery if a key is lost (the data is gone — by design), and CI guidance (set `RUFLO_ENCRYPTION_KEY` in repo secrets).
...
- **MCP server mode**: when started by Codex via `codex mcp add`, the MCP server inherits Codex's environment. The user has to set `RUFLO_ENCRYPTION_KEY` in the env Codex launches with — which is doable but non-obvious. A `~/.codex/encryption.json` config (mode 0600, keychain reference) might be cleaner than env-var-everywhere. Decide in implementation.
...
- [x] `RUFLO_ENCRYPT_AT_REST=1` round-trips a session save → restore unchanged — pinned by `__tests__/session-encryption.test.ts:run_save → run_restore` (commit `98aa256`).
- [x] A plaintext `.codex/sessions/foo.json` from before the upgrade is still readable after the upgrade (magic-sniff backward compat) — pinned by `__tests__/session-encryption.test.ts > migration` and the analogous case in `terminal-encryption.test.ts` + `memory-db-encryption.test.ts` (commits `98aa256`, `49c8019`, `841365f`).
...
- [x] The 1865-test baseline stays green with `RUFLO_ENCRYPT_AT_REST` unset — full vitest run is now **1933/1933 passing, 46 skipped, 0 failures** with the env var unset (started this loop at 1865 + 25 pre-existing failures; +68 new tests across the encryption track).
...
**High-tier targets shipped end-to-end opt-in encrypted under `RUFLO_ENCRYPT_AT_REST=1`:**
- `.codex/sessions/*.json` (memory snapshots + agent prompts)
- `.codex/terminals/store.json` (pasted shell command history → frequent credentials)
