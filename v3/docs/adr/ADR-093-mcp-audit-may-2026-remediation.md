...
| Path | Used by |
|---|---|
| `v3/mcp/tools/hooks-tools.ts` | Standalone `mcp` package (not loaded by the `codex` MCP server) |
| `v3/@ruflo/cli/src/mcp-tools/hooks-tools.ts` | **Actual CLI MCP server** (`npx @ruflo/cli mcp start`) |
...
1. Re-spawn the same six-agent verification swarm (memory-tester, hooks-tester, swarm-tester, neural-tester, system-tester, workflow-tester) against `@ruflo/cli@3.6.14`.
...
- The parallel `v3/mcp/tools/` and `v3/@ruflo/cli/src/mcp-tools/` implementations should be unified in a follow-up ADR. Maintaining both creates the kind of fix-in-the-wrong-file confusion that turned the 3.6.13 #1686 fix into a no-op on the runtime path. Recommend deprecating `v3/mcp/tools/` and re-pointing its consumers at the CLI mcp-tools.
