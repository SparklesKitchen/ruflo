...
Comprehensive tool verification (~190 MCP tools across 4 servers) revealed 6 bugs in input validation and WASM serialization within `@ruflo/cli`. All stem from overly strict or mismatched validators, null safety gaps, and raw WASM objects leaking through the MCP JSON layer.
...
**Symptom**: `transfer_plugin-info(name: "@ruflo/embeddings")` → error "name contains invalid characters".
