...
This plugin adds 58 AI agents to Ruflo that handle all aspects of software quality:
...
**Via Ruflo CLI (recommended):**
```bash
npx ruflo plugins install --name @ruflo/plugin-agentic-qe
```
...
```bash
npx ruflo@v3alpha mcp call aqe/generate-tests \
...
```bash
npx ruflo@v3alpha mcp call aqe/tdd-cycle \
...
```bash
npx ruflo@v3alpha mcp call aqe/security-scan \
  --targetPath ./src \
  --scanType sast \
...
```bash
npx ruflo@v3alpha mcp call aqe/predict-defects \
...
npx ruflo@v3alpha mcp call aqe/chaos-inject \
