...
[![npm version](https://img.shields.io/npm/v/@ruflo/plugin-gastown-bridge.svg)](https://www.npmjs.com/package/@ruflo/plugin-gastown-bridge)
...
npm install @ruflo/plugin-gastown-bridge
...
```typescript
import { GasTownBridgePlugin } from '@ruflo/plugin-gastown-bridge';
...
// Register with Ruflo
await codexFlow.registerPlugin(plugin);
```
...
```typescript
// codex.config.ts
import { defineConfig } from 'codex';
import { GasTownBridgePlugin } from '@ruflo/plugin-gastown-bridge';
...
```typescript
const bead = await codexFlow.mcp.call('gt_beads_create', {
...
```typescript
const ready = await codexFlow.mcp.call('gt_beads_ready', {
  limit: 5,
});
...
await codexFlow.mcp.call('gt_formula_create', {
  name: 'code-review',
  type: 'workflow',
  steps: [
    {
...
const cooked = await codexFlow.mcp.call('gt_formula_cook', {
  formula: 'code-review',
  vars: {
    branch: 'feature/auth',
    reviewer: '@alice',
...
const convoy = await codexFlow.mcp.call('gt_convoy_create', {
  name: 'v2.0-release',
...
```typescript
// Check convoy status
const status = await codexFlow.mcp.call('gt_convoy_status', {
  convoy_id: convoy.id,
  detailed: true,
});
...
const optimized = await codexFlow.mcp.call('gt_wasm_optimize_convoy', {
  convoy_id: convoy.id,
...
await codexFlow.mcp.call('gt_sling', {
  bead_id: 'gt-abc123',
  target: 'polecat',
  formula: 'implement-feature',
});
...
```typescript
const agents = await codexFlow.mcp.call('gt_agents', {
  rig: 'main',
  role: 'polecat',
...
await codexFlow.mcp.call('gt_beads_sync', {
  direction: 'pull',
  rig: 'production',
  conflictStrategy: 'newest-wins',
});
...
const result = await codexFlow.mcp.call('gt_beads_sync', {
  direction: 'both',
  conflictStrategy: 'merge',
});
...
for (const formula of formulas) {
  await codexFlow.mcp.call('gt_formula_cook', {
...
// Do this (352x faster):
const results = await codexFlow.mcp.call('gt_wasm_cook_batch', {
...
const result = await codexFlow.mcp.call('gt_wasm_parse_formula', {
  content: formulaToml,
});
