...
This document defines the **GitHub issue tracking strategy** for the 15-agent swarm implementation of Codex-Flow v3. Agent #1 (Queen Coordinator) manages all issue lifecycle with frequent reply updates from all agents.
...
```
GitHub Project: Codex-Flow v3 Implementation
...
---
_Reply generated by Agent #X (codex v3 swarm)_
```
  ...
  status-update:
    ...
    steps:
      ...
      - name: Collect agent status
        ...
        run: |
          # Query swarm status
          STATUS=$(npx ruflo swarm status --json)
  ...
  daily-summary:
    ...
    steps:
      ...
      - name: Generate daily summary
        run: npx ruflo swarm summary --daily > summary.md
