...
spawn_command: |
  npx ruflo agent spawn queen-coordinator \
...
spawn_command: |
  npx ruflo agent spawn security-architect \
...
fixes_required:
  CVE-1:
    ...
    action: npm update @openai-ai/codex-code@^2.0.31
    files: package.json
...
spawn_command: |
  npx ruflo agent spawn security-implementer \
...
spawn_command: |
  npx ruflo agent spawn security-tester \
...
spawn_command: |
  npx ruflo agent spawn core-architect \
...
spawn_command: |
  npx ruflo agent spawn core-implementer \
...
unification_plan:
  ...
  target_architecture:
    primary: AgentDB (from agentic@alpha)
    features:
...
spawn_command: |
  npx ruflo agent spawn memory-specialist \
...
spawn_command: |
  npx ruflo agent spawn swarm-specialist \
...
spawn_command: |
  npx ruflo agent spawn mcp-specialist \
    --focus "mcp,transport" \
...
type: architect
role: agentic@alpha deep integration
...
spawn_command: |
  npx ruflo agent spawn integration-architect \
    --focus "integration,agentic" \
...
spawn_command: |
  npx ruflo agent spawn cli-hooks-developer \
    --focus "cli,hooks" \
    --interactive enabled
```
...
spawn_command: |
  npx ruflo agent spawn neural-learning-developer \
...
spawn_command: |
  npx ruflo agent spawn tdd-test-engineer \
    --focus "testing,tdd" \
...
spawn_command: |
  npx ruflo agent spawn performance-engineer \
...
spawn_command: |
  npx ruflo agent spawn release-engineer \
...
npx ruflo swarm init v3-implementation \
  --topology hierarchical \
  --max-agents 15 \
  --github-sync enabled

# Spawn Queen Coordinator first
npx ruflo agent spawn queen-coordinator --id 1

# Spawn Security Domain (parallel)
npx ruflo agent spawn security-architect --id 2 &
npx ruflo agent spawn security-implementer --id 3 &
npx ruflo agent spawn security-tester --id 4 &

# Spawn Core Domain (parallel)
npx ruflo agent spawn core-architect --id 5 &
npx ruflo agent spawn core-implementer --id 6 &

wait

# Phase 2: Specialist agents
npx ruflo agent spawn memory-specialist --id 7 &
npx ruflo agent spawn swarm-specialist --id 8 &
npx ruflo agent spawn mcp-specialist --id 9 &

wait

# Phase 3: Integration agents
npx ruflo agent spawn integration-architect --id 10 &
npx ruflo agent spawn cli-hooks-developer --id 11 &
npx ruflo agent spawn neural-learning-developer --id 12 &

wait

# Phase 4: Quality & Deployment
npx ruflo agent spawn tdd-test-engineer --id 13 &
npx ruflo agent spawn performance-engineer --id 14 &
npx ruflo agent spawn release-engineer --id 15 &
