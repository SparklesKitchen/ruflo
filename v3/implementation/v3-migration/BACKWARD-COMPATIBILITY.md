...
|--------|----|----|-----------|
| Core Foundation | Custom implementation | agentic@alpha | Update imports |
...
```typescript
// v3/@ruflo/shared/src/compat/v2-aliases.ts
// Provides V2-compatible imports

// V2 imports
import { HiveMind } from 'codex/hive-mind';
import { SwarmCoordinator } from 'codex/swarm';
import { MemoryManager } from 'codex/memory';

// V3 compatibility layer
export { UnifiedSwarmCoordinator as HiveMind } from '@ruflo/swarm';
export { UnifiedSwarmCoordinator as SwarmCoordinator } from '@ruflo/swarm';
export { UnifiedMemoryService as MemoryManager } from '@ruflo/memory';
```
...
```typescript
// v3/@ruflo/cli/src/compat/v2-commands.ts
...
```typescript
// v3/@ruflo/cli/src/commands/migrate.ts
export async function migrateConfig(v2ConfigPath: string): Promise<void> {
  ...
  const v3Config = {
    ...
    memory: {
      backend: 'hybrid',
      sqlite: {
        path: v2Config.memory?.path || './.codex/memory.db'
      },
      agentdb: {
        enableHNSW: true,
        dimensions: 384
  ...
  await saveYaml('.codex/config.yaml', v3Config);
}
...
```typescript
// v3/@ruflo/memory/src/migration.ts
