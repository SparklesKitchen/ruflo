# Codex-Flow v2 to v3 Migration Guide

## Overview

This guide walks you through migrating from Codex-Flow v2.x to v3.0. The migration is designed to be **zero-breaking-changes** - all v2 code continues to work while you gradually adopt v3 features.

## Prerequisites

- Node.js >= 18.0.0
- Codex-Flow v2.7.x installed
...
This command:
- Backs up your current config to `.codex/config.json.v2.backup`
...
cp .codex/settings-enhanced.json .codex/settings.prod.json
...
```bash
rm .codex/settings-complete.json
rm .codex/settings-checkpoint-*.json
rm .codex/settings.reasoningbank-*.json
rm .codex/settings-npx-hooks.json
```
...
// v3 enhanced (opt-in)
import { HybridMemorySystem } from 'codex/v3';
...
Codex-Flow v3 automatically detects Windows and uses sql.js.
...
```bash
# Restore v2 config
cp .codex/config.json.v2.backup .codex/config.json

# Downgrade package
npm install codex@2.7.47

# Restore v2 settings (if needed)
git checkout HEAD~1 -- .codex/settings-*.json
```

## Troubleshooting

### "Module not found: agentic"
...
1. Open `.codex/config.json.v2.backup`
2. Copy hook definitions
3. Paste into `.codex/config.json` under `hooks` key

## Support

- Documentation: https://github.com/SparklesKitchen/ruflo/docs/v3
- Issues: https://github.com/SparklesKitchen/ruflo/issues
- Discussions: https://github.com/SparklesKitchen/ruflo/discussions
