...
| **Workflow persistence** | Saves to `.codex/workflows/store.json` | `workflow-tools.ts:52-78` — `loadWorkflowStore()`/`saveWorkflowStore()` |
...
| **P2** | Intelligence stats → neural bridge | Stats counter disconnected | Read from ReasoningBank `.codex/neural/patterns.json` |
...
- Read pattern count from `.codex/neural/patterns.json`
- Read trajectory count from `.codex/neural/stats.json`
...
| `@ruflo/memory` | 3.0.0-alpha.12 | AgentDB + ControllerRegistry |
