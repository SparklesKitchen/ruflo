...
The memory system has three powerful modules — AutoMemoryBridge (storage), MemoryGraph (PageRank + community detection), LearningBridge (confidence tracking) — all shipped and tested (219 tests, published as `@ruflo/memory@3.0.0-alpha.8`). But they are not wired into the hook system that runs during Codex sessions.
...
Hooks are short-lived Node.js processes invoked by Codex. `hook-handler.cjs` uses `require()`. ESM dynamic `import()` is async and adds ~50ms overhead per invocation. The memory package (`@ruflo/memory`) is ESM-only. The intelligence layer must be CJS for synchronous, fast loading.
...
2. **ESM import of @ruflo/memory** — Use the full memory package in hooks. Rejected: CJS hooks can't synchronously import ESM.
...
| Action | File | Lines |
|--------|------|-------|
| CREATE | `cli/.codex/helpers/intelligence.cjs` | ~560 |
| CREATE | `v3/implementation/adrs/ADR-050-intelligence-loop.md` | ~150 |
| MODIFY | `cli/.codex/helpers/hook-handler.cjs` | +30 |
| MODIFY | `cli/.codex/helpers/session.js` | +6 |
