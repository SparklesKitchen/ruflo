...
Build `@ruflo/plugin-agent-federation` as a first-class Ruflo plugin that enables cross-installation agent collaboration with security-first design.
...
- Discovery uses three mechanisms:
  1. **Static peers** -- configured in `codex.config.json` under `federation.peers[]`
  2. **DNS-SD** -- `_codex-federation._tcp` SRV records for LAN discovery
  3. **Registry** -- optional IPFS-pinned registry (extends existing Pinata infrastructure in `v3/@ruflo/cli/src/plugins/store/discovery.ts`)

**Phase B: Handshake (mTLS + Challenge-Response)**
- Nodes establish mTLS connections using ed25519 keypairs generated by `@ruflo/security/TokenGenerator`.
...
| Decision | Rationale |
|----------|-----------|
| **Plugin, not core** | Federation is opt-in; many installations are single-node. Ships as `@ruflo/plugin-agent-federation`. |
...
| **Claims-based authorization** | Every cross-boundary operation requires a claim. Extends `@ruflo/claims` with federation-specific claim types. |
...
All primitives reuse existing `@ruflo/security`:
...
The existing PII patterns in `@ruflo/aidefence` detect email, SSN, credit card, API key, GitHub token, and hardcoded passwords. The federation plugin extends this with:
...
```
v3/@ruflo/plugin-agent-federation/
  package.json
  tsconfig.json
  vitest.config.ts
  src/
    index.ts                           # Plugin entry point
    plugin.ts                          # FederationPlugin class (CodexFlowPlugin)
...
export class AgentFederationPlugin implements CodexFlowPlugin {
  ...
  readonly dependencies = ['@ruflo/security', '@ruflo/aidefence', '@ruflo/claims'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    // 1. Register federation claim types with @ruflo/claims
...
**Integration Points:**
- `@ruflo/aidefence` -- `hasPII()`, `detect()`, PII_PATTERNS
- `@ruflo/security` -- `TokenGenerator`, `PasswordHasher`
- `@ruflo/memory` -- AgentDB for audit storage
...
**Integration Points:**
- `@ruflo/swarm/federation-hub` -- heartbeat, registration
- `@ruflo/security` -- mTLS certificate validation
- `@ruflo/guidance/authority` -- `AuthorityGate` for peer acceptance
...
**Integration Points:**
- `@ruflo/aidefence` -- `detect()`, `hasPII()`, `learnFromDetection()`
- `@ruflo/claims` -- `ClaimService` for federation claims
- `@ruflo/hooks` -- hook registration for send/receive lifecycle
...
- npm publish as `@ruflo/plugin-agent-federation`
