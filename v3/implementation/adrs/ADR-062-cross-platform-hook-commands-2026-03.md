...
```
node -e "var c=require('child_process'),p=require('path'),r;try{r=c.execSync('git rev-parse --show-toplevel',{encoding:'utf8'}).trim()}catch(e){r=process.cwd()}var s=p.join(r,'.codex/helpers/hook-handler.cjs');process.argv.splice(1,0,s);require(s)" pre-bash
```
...
```
node .codex/helpers/hook-handler.cjs pre-bash
```

### Why This Works

The `node -e` one-liner existed to resolve the git root at runtime, ensuring hooks worked regardless of CWD. However, **Codex always runs hooks from the project root directory**, making git-root resolution redundant. Direct invocation with relative paths works identically on Windows, macOS, and Linux.

### Platform Detection

Added `detectPlatform()` integration to `generateSettings()`. The detected platform (OS, architecture, shell type) is now stored in `codexFlow.platform` within settings.json:
...
| Before | After |
|--------|-------|
| `node -e "var c=require('child_process')..." pre-bash` | `node .codex/helpers/hook-handler.cjs pre-bash` |
| `node -e "...import(u.pathToFileURL(f)...)" import` | `node .codex/helpers/auto-memory-hook.mjs import` |
| `node -e "..." (statusline)` | `node .codex/helpers/statusline.cjs` |
