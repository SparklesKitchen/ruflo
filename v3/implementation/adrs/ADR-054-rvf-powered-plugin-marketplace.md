...
1. **Search is keyword-only** — `searchPlugins()` in `search.ts` does case-insensitive substring matching across name, description, tags, keywords. A query like "I need permission management" won't find `@ruflo/claims` unless the user knows the exact term "claims."
...
5. **No behavioral context** — The system cannot observe that a developer working on security tasks should be shown security plugins, or that users who install `@ruflo/security` almost always also install `@ruflo/claims`.
...
**Impact:** "permission management" finds `@ruflo/claims`. "machine learning" finds `@ruflo/neural`. "code quality" finds `@ruflo/plugin-agentic-qe`. Zero change to plugin commands — the search function is transparently upgraded.
...
npx ruflo plugins deps @ruflo/security --transitive
...
# Check for conflicts
npx ruflo plugins conflicts @ruflo/plugin-a @ruflo/plugin-b
```
...
npx ruflo plugins why @ruflo/X # "Why is this recommended?"
```
...
# → @ruflo/security (InputValidator — Zod-based validation)
# → @ruflo/claims (claims-based authorization)

npx ruflo plugins find-for "train neural patterns from code"
# → @ruflo/neural (SONA, MoE, EWC++)
# → @ruflo/plugin-neural-coordinator
```
...
- **MCP Server**: Expose as MCP tool for Codex integration
...
If `@ruflo/memory` or AgentDB is unavailable, every feature falls back to the existing implementation. Zero breaking changes.
...
|---|---|---|
| Embedding model download (100MB+) blocks first search | High latency on first use | Pre-warm in `daemon start`; cache model in `.codex/models/` |
...
- Plugin Store Types: `v3/@ruflo/cli/src/plugins/store/types.ts`
- Plugin Search: `v3/@ruflo/cli/src/plugins/store/search.ts`
- Plugin Discovery: `v3/@ruflo/cli/src/plugins/store/discovery.ts`
- ControllerRegistry: `v3/@ruflo/memory/src/controller-registry.ts`
