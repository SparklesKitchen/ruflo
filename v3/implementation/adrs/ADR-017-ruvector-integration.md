...
The `@ruflo/cli` package requires integration with `ruvector` for advanced code intelligence features:
...
These features are unique to ruvector and complement codex's existing capabilities without duplicating functionality already present in `@ruflo/embeddings` or `@ruflo/memory`.

## Decision

Implement ruvector as an **OPTIONAL dependency** with graceful fallback, following the existing patterns in `@ruflo/cli`.
...
```typescript
// v3/@ruflo/cli/src/ruvector/availability.ts
...
```typescript
// v3/@ruflo/cli/src/ruvector/adapters/router-adapter.ts
...
```typescript
// v3/@ruflo/cli/src/ruvector/adapters/ast-adapter.ts
...
```typescript
// v3/@ruflo/cli/src/ruvector/adapters/diff-adapter.ts
...
```typescript
// v3/@ruflo/cli/src/ruvector/adapters/graph-adapter.ts
...
```typescript
// v3/@ruflo/cli/src/commands/route.ts
...
export const routeCommand: Command = {
  ...
  examples: [
    {
      command: 'codex route -t "Implement user authentication" --q-learning',
      description: 'Route with Q-Learning model'
    },
    {
      command: 'codex route -t "Fix login bug" --coverage-aware',
...
```typescript
// v3/@ruflo/cli/src/commands/analyze.ts
...
const astSubcommand: Command = {
  ...
  examples: [
    { command: 'codex analyze ast -p src/', description: 'Analyze src directory' },
    { command: 'codex analyze ast -p src/api.ts --complexity', description: 'Get complexity for file' }
  ],
  action: astAction
};
...
const diffSubcommand: Command = {
  ...
  examples: [
    { command: 'codex analyze diff --risk', description: 'Analyze current diff with risk' },
    { command: 'codex analyze diff --base main --target feature', description: 'Compare branches' },
    { command: 'git diff | codex analyze diff --stdin --risk', description: 'Pipe diff from git' }
  ],
  action: diffAction
};
...
const boundariesSubcommand: Command = {
  ...
  examples: [
    { command: 'codex analyze boundaries -p src/', description: 'Detect boundaries in src' },
    { command: 'codex analyze boundaries -a louvain', description: 'Use Louvain algorithm' }
  ],
...
```typescript
// v3/@ruflo/cli/src/ruvector/errors.ts
...
```typescript
// v3/@ruflo/cli/src/mcp-tools/ruvector-tools.ts
...
```typescript
// v3/@ruflo/cli/src/ruvector/index.ts
...
```typescript
// v3/@ruflo/cli/src/commands/index.ts
...
```typescript
// v3/@ruflo/cli/tests/ruvector/availability.test.ts
...
```typescript
// v3/@ruflo/cli/tests/commands/route.test.ts
...
All cache utilities exported from `@ruflo/cli/ruvector`:

```typescript
// Diff caches
import { clearDiffCache, clearAllDiffCaches } from '@ruflo/cli/ruvector';

// Graph caches
import { clearGraphCaches, getGraphCacheStats } from '@ruflo/cli/ruvector';

// Coverage caches
import { clearCoverageCache, getCoverageCacheStats } from '@ruflo/cli/ruvector';
```
...
**Changes Made:**
- `@ruflo/cli/src/commands/index.ts` - Refactored to use dynamic imports
...
**Changes Made:**
- `@ruflo/memory/src/agentdb-adapter.ts` - Optimized bulk methods
...
|---------|--------|-------|
| `codex` | 3.0.0-alpha.17 | 3.0.0-alpha.18 |
| `@ruflo/cli` | 3.0.0-alpha.24 | 3.0.0-alpha.25 |
| `@ruflo/mcp` | 3.0.0-alpha.7 | 3.0.0-alpha.8 |

### Usage Examples

**Lazy Command Loading:**
```typescript
import { getCommandAsync, loadAllCommands } from '@ruflo/cli';
...
```typescript
import { UnifiedMemoryService } from '@ruflo/memory';
...
- `codex@3.0.0-alpha.18`
- `@ruflo/cli@3.0.0-alpha.25`
- `@ruflo/mcp@3.0.0-alpha.8`
- `@ruflo/memory@3.0.0-alpha.2`

### Performance Validation

All builds pass successfully:
```
✓ @ruflo/cli build passed
✓ @ruflo/mcp build passed
✓ @ruflo/memory build passed
```
