...
### Package Version
- `@ruflo/memory@3.0.0-alpha.2` (published 2026-01-07)
...
Added `memory init` command to CLI (`@ruflo/cli@3.0.0-alpha.56`) using **sql.js** (WASM SQLite) for cross-platform compatibility without native compilation.

```bash
# Initialize memory database
npx @ruflo/cli@latest memory init

# Options
npx @ruflo/cli@latest memory init --backend sqlite  # Default
npx @ruflo/cli@latest memory init --path ./data/custom.db
npx @ruflo/cli@latest memory init --force  # Overwrite existing
```
...
- `.codex/memory.db` - Sync location for Codex hooks
