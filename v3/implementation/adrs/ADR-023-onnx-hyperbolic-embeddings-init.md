...
├── neural-integration.ts    # agentic substrate wrapper
...
function getMigrationSteps(target: string) {
  ...
  if (target === 'all' || target === 'embeddings') {
    steps.push({
      ...
      dest: '.codex/models/',
      execute: async () => {
        const { downloadEmbeddingModel } = await import('@ruflo/embeddings');
        await downloadEmbeddingModel('all-MiniLM-L6-v2', '.codex/models/');
      }
    });
  }
}
...
npx ruflo@v3alpha embeddings init [options]
...
npx ruflo@v3alpha embeddings status

Output:
╭────────────────────────────────────────────────────╮
│ Embedding System Status                            │
├────────────────────────────────────────────────────┤
│ Provider:    agentic (ONNX)                   │
...
npx ruflo@v3alpha embeddings download <model-id>

# Example
npx ruflo@v3alpha embeddings download all-mpnet-base-v2
...
async function initializeEmbeddings(options: InitOptions): Promise<void> {
  ...
  const configDir = path.join(options.targetDir, '.codex');
...
async function migrateEmbeddings(ctx: CommandContext): Promise<void> {
  ...
  const v2CachePath = path.join(ctx.cwd, '.codex', 'cache', 'embeddings.db');
  const v2Exists = fs.existsSync(v2CachePath);

  // 2. Download V3 model
  const { downloadEmbeddingModel, listEmbeddingModels } = await import('@ruflo/embeddings');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (ctx.flags.embeddings) {
      ...
      const embedder = createEmbeddingService({
        provider: 'agentic',
        modelPath: '.codex/models/all-MiniLM-L6-v2',
      });
...
const initSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      await downloadEmbeddingModel(model, '.codex/models/', (p) => {
...
const downloadSubcommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    await downloadEmbeddingModel(modelId, '.codex/models/', (p) => {
...
- Requires `@ruflo/embeddings` package
