...
The `@openai-ai/codex-code` package (v2.1.1) provides the official CLI for Codex AI. Deep integration with Codex enables enhanced developer experience for codex users. This ADR documents **undocumented integration points** discovered through source code analysis that are not covered in official documentation.

### Analysis Methodology

1. Downloaded and extracted `@openai-ai/codex-code@2.1.1` to `/tmp/package/`
...
Implement Codex integration as an **OPTIONAL peer dependency** with graceful fallback, leveraging undocumented APIs where beneficial while maintaining compatibility.
...
**Location:** `node_modules/@openai-ai/codex-code/sdk-tools.d.ts`

Codex exports complete TypeScript definitions for all tool inputs. These can be used for:
...
Codex supports a comprehensive hook system with these event types:
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": ["npx ruflo@v3alpha hooks modify-bash"]
      },
      {
        "matcher": "Write|Edit",
        "hooks": ["npx ruflo@v3alpha hooks modify-file"]
      }
    ],
    "PostToolUse": [
      {
        "matcher": ".*",
        "hooks": ["npx ruflo@v3alpha hooks post-command"]
      }
    ],
    "UserPromptSubmit": [
      {
        "hooks": ["npx ruflo@v3alpha hooks route --task \"$PROMPT\""]
      }
    ]
  }
}
...
|----------|---------|---------|
| `CODEX_CONFIG` | Config file path | `~/.codex/settings.json` |
| `CODEX_DEBUG` | Enable debug output | `false` |
| `CODEX_DISABLE_TELEMETRY` | Disable telemetry | `false` |
| `CODEX_HEADLESS` | Non-interactive mode | `false` |
| `CODEX_MAX_CONTEXT` | Max context tokens | Model default |
| `CODEX_SANDBOX_MODE` | Sandbox type | `auto` |
| `CODEX_SKIP_HOOKS` | Skip hook execution | `false` |
| `CODEX_TIMEOUT` | Default command timeout | `120000` |
| `OpenAI_MODEL` | Override model selection | - |
| `OpenAI_BASE_URL` | API endpoint override | `https://api.openai.com` |
...
Codex has an undocumented plugin marketplace:
...
### 7. AGENTS.md Project Configuration
...
```typescript
// src/codex-code/integration.ts

interface CodexCodeStatus {
  installed: boolean;
  version?: string;
  configPath?: string;
  features: {
...
/**
 * Detect Codex installation and capabilities
 */
export async function detectCodexCode(): Promise<CodexCodeStatus> {
  try {
...
export async function configureIntegration(options: {
  ...
  const status = await detectCodexCode();
  if (!status.installed) {
    throw new Error('Codex not installed. Run: npm install -g @openai-ai/codex-code');
  }

  // Add MCP server if requested
  if (options.enableMcp && status.configPath) {
    await exec(`codex mcp add ${options.mcpServerName || 'codex'} npx ruflo@v3alpha mcp start`);
  }
}
```
...
```typescript
// src/codex-code/hooks.ts

/**
 * Install codex hooks into Codex settings
 */
export async function installHooks(): Promise<void> {
  const status = await detectCodexCode();
  if (!status.features.hooks) {
    throw new Error('Codex version does not support hooks');
  }
...
### `codex setup codex-code`

```bash
# Auto-detect and configure integration
npx ruflo@v3alpha setup codex-code

# Options:
#   --hooks         Install hooks into Codex settings
#   --mcp           Register codex MCP server
#   --agents        Install custom agent definitions
#   --verify        Verify integration status
```

### `codex doctor --codex-code`

```bash
# Check Codex integration health
npx ruflo@v3alpha doctor --codex-code

# Output:
# ✓ Codex installed (v2.1.1)
...
1. Create `src/codex-code/integration.ts`
2. Implement `detectCodexCode()`
3. Add to `doctor` command
4. Update package.json with peer dependency

### Phase 2: Hook Installation (Week 2)

1. Create `src/codex-code/hooks.ts`
2. Implement `installHooks()`
3. Add `setup codex-code` command
...
1. **Seamless Integration** - Works automatically when Codex installed
...
1. **Version Coupling** - Must track Codex API changes
2. **Undocumented APIs** - May break with updates
3. **Complexity** - More configuration options for users

### Neutral

1. **Optional Dependency** - Users without Codex unaffected
...
npx @ruflo/cli@latest init --start-all

# Equivalent to running:
# 1. npx @ruflo/cli@latest init
# 2. npx @ruflo/cli@latest memory init
# 3. npx @ruflo/cli@latest daemon start
# 4. npx @ruflo/cli@latest swarm init --topology hierarchical
```
...
This simplifies the Codex integration setup from multiple commands to a single invocation.

**CLI Version:** `@ruflo/cli@3.0.0-alpha.56`
