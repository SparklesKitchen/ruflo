...
- `@ruflo/memory` upgraded from `agentdb@2.0.0-alpha.3.7` to `agentdb@^3.0.0-alpha.7`
...
| `LLMRouter` | API keys (OpenRouter/Gemini/OpenAI) | Local ONNX models or template-based |
...
| `LearningBridge` | Yes (via `@ruflo/memory`) | No | #1213 |
| `MemoryGraph` | Yes (via `@ruflo/memory`) | No | #1214 |
...
| `TieredCacheManager` | Yes (via `@ruflo/memory`) | No | #1220 |
...
Additionally, the `HybridBackend` facade (#1212) does not proxy new v3 methods (`recordFeedback`, `verifyWitnessChain`, `getWitnessChain`), and the hook handler (#1211) ignores stdin on Codex 2.x, making all hooks non-functional.
...
|-----------|--------|-------------|
| **Eliminate dual memory system** | — | Refactor CLI to use `@ruflo/memory` → `HybridBackend` → AgentDB v3 instead of raw `sql.js` in `memory-initializer.js`. This is the single largest blocker. |
...
| **Init hook config fix** | #1230 | Remove invalid `TaskCompleted`/`TeammateIdle` keys from generated hook config that cause Codex settings warnings. |
...
A central registry (replacing the current `memory-initializer.js`) that wraps the `AgentDB` class and adds CLI-specific controllers from `@ruflo/memory`:
...
interface ControllerRegistry {
  ...
  // CLI-layer controllers (from @ruflo/memory, not in AgentDB)
...
// CLI-layer controllers (from @ruflo/memory or new)
...
2. **Auto-start dependencies**: `ruflo` / `codex` should auto-start MCP when needed.
...
The most critical integration gap is that the CLI (`memory-initializer.js`, 1929 lines) runs a **self-contained SQLite memory system** that duplicates what `@ruflo/memory` + AgentDB already provides. Before wiring controllers, the CLI must be refactored to use `@ruflo/memory`'s `HybridBackend` as its storage layer instead of raw `sql.js` calls.

| Current (Broken) | Target |
|-------------------|--------|
| CLI → `memory-initializer.js` → raw `sql.js` | CLI → `@ruflo/memory` → `HybridBackend` → AgentDB v3 |
...
The bridge pattern was implemented as `memory-bridge.ts` (858 lines) in `@ruflo/cli`, routing CLI operations through `ControllerRegistry` → `HybridBackend` → AgentDB v3. This eliminates the dual memory system described in the "Prerequisite" section.
...
- Contributor: @sparkling (codex-patch repository)
...
- `@ruflo/memory`: upgraded to `agentdb@^3.0.0-alpha.7`
