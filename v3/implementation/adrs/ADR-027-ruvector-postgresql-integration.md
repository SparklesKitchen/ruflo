# ADR-027: RuVector PostgreSQL Integration for Codex-Flow v3
...
Codex-Flow v3 currently uses a hybrid memory backend (ADR-009) combining SQLite for structured queries and AgentDB for vector search. While this approach works well for many use cases, production deployments increasingly require:
...
Integrate `@ruvector/postgres-cli` as an **optional plugin bridge** in Codex-Flow v3, following the plugin architecture established in ADR-015-v2. This provides a production-grade vector database option while maintaining backward compatibility with existing AgentDB deployments.
...
```typescript
// v3/@ruflo/plugins/src/bridges/ruvector-postgres/plugin.ts
...
```typescript
// v3/@ruflo/plugins/src/bridges/ruvector-postgres/connection-manager.ts
...
```typescript
// v3/@ruflo/plugins/src/mcp-tools/ruvector-postgres-tools.ts
...
```typescript
// v3/@ruflo/plugins/src/bridges/ruvector-postgres/embedding-adapter.ts
...
```typescript
// v3/@ruflo/plugins/src/bridges/ruvector-postgres/migration-helper.ts
...
- [ ] CLI integration (`codex memory --backend ruvector-postgres`)
- [ ] User documentation
