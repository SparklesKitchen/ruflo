...
The `@ruflo/guidance` package (published as `3.0.0-alpha.1`) provides a governance control plane for Codex sessions:

- **Compile**: AGENTS.md → Constitution + Rule Shards + Rule Manifest
...
- **Templates**: 6 AGENTS.md templates (minimal, standard, full, security, performance, solo)

Currently, `@ruflo/guidance` is:
1. A standalone package at `v3/@ruflo/guidance/`
2. Used by `@ruflo/cli` via dynamic `import()` — but NOT declared as a dependency
3. Not included in the umbrella `codex` package's `files` array
4. Not available to end users running `npx ruflo@alpha`

This means the `guidance` CLI commands silently fail at runtime when installed from npm.

## Decision

Integrate `@ruflo/guidance` as a **first-class dependency** in both `@ruflo/cli` and the `codex` umbrella package, making it a core component of Ruflo v3.1.

### 1. Dependency Graph

```
codex (umbrella v3.1.0-alpha.1)
  └── @ruflo/cli (v3.1.0-alpha.1)
        ├── @ruflo/guidance (v3.0.0-alpha.1)  ← NEW
        ├── @ruflo/shared
        ├── @ruflo/mcp
        └── @ruflo/aidefence
```
...
{
  "dependencies": {
    "@ruflo/guidance": "^3.0.0-alpha.1"
  }
}
```

**`codex/package.json` (umbrella)**:
```json
{
  "files": [
    "v3/@ruflo/guidance/dist/**/*.js",
    "v3/@ruflo/guidance/dist/**/*.d.ts",
    "v3/@ruflo/guidance/package.json"
  ]
}
```
...
|------------|-------------|
| `guidance compile` | Compile AGENTS.md into constitution + shards + manifest |
...
| `guidance optimize` | Analyze and optimize AGENTS.md for higher governance score |
| `guidance ab-test` | Run A/B comparative benchmark between configurations |

### 4. Init System Integration

The AGENTS.md generator (`codexmd-generator.ts`) now:
- Produces analyzer-validated A-grade templates (91-95/100)
- Supports 6 template variants via `runtime.codexMdTemplate` option
...
|---------|---------|------|
| `codex` (umbrella) | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@ruflo/cli` | 3.0.0-alpha.185 | 3.1.0-alpha.1 |
| `@ruflo/guidance` | 3.0.0-alpha.1 | 3.0.0-alpha.1 (unchanged) |
...
- AGENTS.md quality is measurable and enforceable
...
1. Add `@ruflo/guidance` to CLI's `dependencies`
