...
The V3 Hooks System provides a comprehensive event-driven architecture for intercepting, modifying, and recording operations throughout the codex lifecycle. It integrates with the **ReasoningBank** neural learning system to enable self-improving agent behaviors.
...
### 1. Hook Registry (`@ruflo/shared/src/hooks/registry.ts`)

Manages hook registration, priority ordering, and lifecycle.

```typescript
import { createHookRegistry, HookEvent, HookPriority } from '@ruflo/shared';
...
### 2. Hook Executor (`@ruflo/shared/src/hooks/executor.ts`)

Executes registered hooks in priority order with error handling.

```typescript
import { createHookExecutor, HookContext } from '@ruflo/shared';
...
### 4. CLI Commands (`@ruflo/cli/src/commands/hooks.ts`)

User-accessible CLI for hooks operations.

```bash
# Pre/Post Edit Hooks
npx ruflo hooks pre-edit <filePath> [--operation modify]
npx ruflo hooks post-edit <filePath> --success true

# Pre/Post Command Hooks
npx ruflo hooks pre-command "npm test"
npx ruflo hooks post-command "npm test" --success true --exit-code 0

# Task Routing
npx ruflo hooks route "Implement user authentication"
npx ruflo hooks explain "Implement user authentication" --verbose

# Intelligence Bootstrap
npx ruflo hooks pretrain [--include-git --include-deps]
npx ruflo hooks build-agents [--focus security]

# Metrics & Management
npx ruflo hooks metrics [--category routing]
npx ruflo hooks list [--category pre-edit]
npx ruflo hooks transfer <sourceProject>
```
...
```typescript
import { swarmComm } from '@ruflo/hooks';
...
```bash
# Send message
npx @ruflo/hooks swarm-send security-auditor "Found vulnerability" context high

# Broadcast to all
npx @ruflo/hooks swarm-broadcast "Switching to security focus"

# Get messages
npx @ruflo/hooks swarm-messages 10
```
...
npx @ruflo/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

# List recent broadcasts
npx @ruflo/hooks swarm-patterns memory 0.8

# Import a broadcast pattern
npx @ruflo/hooks swarm-import-pattern bc_1234567890_abc123
```
...
```bash
# Start consensus
npx @ruflo/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session" 30000

# Vote
npx @ruflo/hooks swarm-vote cons_1234567890_abc "JWT"

# Check status
npx @ruflo/hooks swarm-consensus-status cons_1234567890_abc
```
...
```bash
# Initiate handoff
npx @ruflo/hooks swarm-handoff test-architect "Write auth tests" \
  '{"filesModified":["src/auth/login.ts"],"nextSteps":["Write unit tests"]}'

# Accept handoff
npx @ruflo/hooks swarm-accept-handoff ho_1234567890_abc

# Complete handoff
npx @ruflo/hooks swarm-complete-handoff ho_1234567890_abc '{"testsWritten":15}'

# List pending handoffs
npx @ruflo/hooks swarm-handoffs
```
...
npx ruflo hooks post-edit --file "file.ts" --memory-key "swarm/agent/step"
npx ruflo hooks notify --message "completed"
...
│   │   └── guidance-provider.ts    # Codex-visible output generator
│   ├── swarm/
