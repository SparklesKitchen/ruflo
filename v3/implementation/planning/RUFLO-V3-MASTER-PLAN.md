# Codex-Flow v3: Complete Reimagining with agentic@alpha Foundation

## Executive Summary

Codex-Flow v3 represents a complete architectural overhaul that builds on **agentic@alpha** as its core foundation while maintaining full backward compatibility with v2.x. This plan consolidates findings from concurrent swarm analysis covering architecture, security, dead code, Windows compatibility, repository cleanup, and .codex/ optimization.
...
├── Dependencies: agentic (^1.9.4), ruv-swarm, flow-nexus
...
| **agentic as add-on** | Not leveraged properly | Missing performance | Native foundation |
...
**Key Files to Remove**:
- `src/api/codex-client-v2.5.ts` (deprecated)
...
## 3. agentic@alpha Integration

### 3.1 Package Capabilities

**Version**: `agentic@2.0.1-alpha.0`
...
```typescript
// src/v3/integrations/agentic-adapter.ts
...
- Add agentic as optional enhancement
...
| `codex-wiki/` | 0 | Remove empty directory |
...
## 6. .Codex/ Optimization

### 6.1 Current State (14.2MB)

| Directory | Size | Issues |
|-----------|------|--------|
| `.codex/` | 11MB | 9 settings variants, 3,720 checkpoints |
| `.codex/` | 2.5MB | Stale training data |
| `.codex-plugin/` | 81KB | Hook duplication |
...
.swarm/                            # → .codex/coordination/
.hive-mind/                        # → .codex/coordination/
...
```typescript
// .codex/agents/core/coder.md (v3 enhanced)
---
name: coder
version: 3.0.0
category: core
...
- `.codex/settings-enhanced.json`
- `.codex/settings-complete.json`
- `.codex-plugin/hooks/hooks.json`

**v3 Solution**: Single source in `config.json`

```typescript
// .codex/config.json (v3)
{
  "version": "3.0.0",
  "hooks": {
    "PreToolUse": [
...
### 7.5 Hook Integration with agentic
...
export class SwarmCoordinator {
  ...
  async spawnAgent(profile: AgentProfile): Promise<string> {
    // Internally uses v3 with agentic
...
- [ ] Integration tests with agentic
...
| 3 | 3 | Foundation | v3 structure, agentic installed |
...
docs/v3/
├── RUFLO-V3-MASTER-PLAN.md (this file)
├── MIGRATION-GUIDE.md
├── API-REFERENCE.md
└── ARCHITECTURE.md
```
...
# Duplicate settings
.codex/settings-complete.json
.codex/settings-enhanced.json (merge into settings.prod.json)
.codex/settings-checkpoint-*.json
.codex/settings.reasoningbank-*.json
.codex/settings-npx-hooks.json
```

### Files to Modify

```
package.json                    # Add sql.js, update agentic
.gitignore                      # Add dist-cjs, cleanup duplicates
.codex/config.json             # Add v3 structure, unified hooks
...
npm install agentic@2.0.1-alpha.0 sql.js
...
import { SwarmCoordinator } from 'codex';  // v2 API still works

// v3 native
import { AgenticFlowAdapter } from 'codex/v3';
