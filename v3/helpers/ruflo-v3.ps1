# Ruflo V3 Master Helper (Windows PowerShell)
# Cross-platform development automation for codex v3
...
$HelpersDir = Join-Path $CodexDir "helpers"
$MetricsDir = Join-Path $ProjectRoot ".codex\metrics"
$SecurityDir = Join-Path $ProjectRoot ".codex\security"
...
function Setup-Directories {
    @($CodexDir, $HelpersDir, $MetricsDir, $SecurityDir) | ForEach-Object {
        if (!(Test-Path $_)) {
...
function Initialize-V3Project {
    Log-Header "🚀 Initializing Ruflo V3 Project"
    ...
    try {
        ...
        if (Test-Path $validatorPath) {
            ...
            Log-Info "Run 'codex-v3.ps1 status' to see current progress"
        } else {
            Log-Warning "Validator not found, but basic setup complete"
        }
    }
    catch {
        Log-Error "Initialization completed with warnings. Run 'codex-v3.ps1 validate' for details"
    }
}
...
function Show-Status {
    ...
    if (Test-Path $statusScript) {
        ...
        Log-Info "Status display not available. Run 'codex-v3.ps1 init' to set up helpers."
    }
}
...
function Update-Progress {
    ...
    if (Test-Path $progressScript) {
        ...
        Log-Error "Progress manager not available. Run 'codex-v3.ps1 init' first."
        exit 1
    }
}
...
function Validate-Config {
    ...
    if (Test-Path $validatorScript) {
        ...
        Log-Error "Config validator not available. Run 'codex-v3.ps1 init' first."
        exit 1
    }
}
...
Usage: .\codex-v3.ps1 <command> [options]
...
Examples:
  .\codex-v3.ps1 init                  # Set up V3 project
  .\codex-v3.ps1 status                # Show current progress
  .\codex-v3.ps1 update domain 3       # Mark 3 domains complete
  .\codex-v3.ps1 update agent 8        # Set 8 agents active
  .\codex-v3.ps1 checkpoint "Feature complete"
  .\codex-v3.ps1 github status         # GitHub integration status

Platform: Windows (PowerShell $($PSVersionTable.PSVersion))
Codex Directory: $CodexDir
...
"@
    ...
    default {
        ...
        Log-Info "Run '.\codex-v3.ps1 help' for usage information"
        exit 1
    }
}