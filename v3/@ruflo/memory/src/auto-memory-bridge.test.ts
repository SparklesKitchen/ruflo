 ...
 * between Codex auto memory and AgentDB.
 */
...
describe('resolveAutoMemoryDir', () => {
  it('should derive path from working directory', () => {
    ...
    expect(result).toContain('.codex/projects/');
...
`;
  ...
  describe('curateIndex', () => {
    it('should generate MEMORY.md from topic files', async () => {
      ...
      expect(indexContent).toContain('# Ruflo V3 Project Memory');
    ...
    // hardcoded DEFAULT_TOPIC_MAPPING filenames (as happens when Codex's
    ...
    it('should not overwrite hand-curated MEMORY.md when no topic files match (#1556)', async () => {
      ...
      // Write files using Codex's native <type>_<topic>.md convention —
