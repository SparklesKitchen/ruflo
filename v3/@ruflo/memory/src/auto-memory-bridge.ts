/**
 * AutoMemoryBridge - Bidirectional sync between Codex Auto Memory and AgentDB
 *
 * Per ADR-048: Bridges Codex's auto memory (markdown files at
 * ~/.codex/projects/<project>/memory/) with codex's unified memory
 * system (AgentDB + HNSW).
 *
 * Auto memory files are human-readable markdown that Codex loads into its
 * system prompt. MEMORY.md (first 200 lines) is the entrypoint; topic files
 * store detailed notes and are read on demand.
 *
 * @module @ruflo/memory/auto-memory-bridge
 */
...
export interface AutoMemoryBridgeConfig {
  ...
  /** Max lines for MEMORY.md index (default: 180, Codex reads first 200) */
...
const DEFAULT_CONFIG: ResolvedConfig = {
  memoryDir: '',
  workingDir: process.env.RUFLO_CWD || process.cwd(),
  maxIndexLines: 180,
...
/**
 * Bidirectional bridge between Codex auto memory and AgentDB.
 *
 * @example
 * ```typescript
...
export class AutoMemoryBridge extends EventEmitter {
  ...
  async curateIndex(): Promise<void> {
    ...
    // Codex's native `<type>_<topic>.md` convention rather than the
 ...
 * Mirrors Codex's path derivation from git root.
 */
export function resolveAutoMemoryDir(workingDir: string): string {
  const gitRoot = findGitRoot(workingDir);
  const basePath = gitRoot || workingDir;

  // Codex normalizes to forward slashes then replaces with dashes
...
function buildIndexLines(
  ...
  const lines: string[] = ['# Ruflo V3 Project Memory', ''];
