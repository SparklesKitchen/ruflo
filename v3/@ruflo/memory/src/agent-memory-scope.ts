/**
 * Agent-Scoped Memory - Support for Codex's 3-scope agent memory directories
 *
 * Codex organizes agent memory into three scopes:
 ...
 *
 * @module @ruflo/memory/agent-memory-scope
 */
...
/** Codex's 3-scope agent memory system */
 ...
 *
 * Path resolution matches Codex binary behavior:
 * ```
...
export function resolveAgentMemoryDir(
  ...
  if (scope === 'user') {
    ...
    return path.join(home, '.codex', 'agent-memory', safeName);
  }
  ...
  if (scope === 'local') {
    return path.join(baseDir, '.codex', 'agent-memory-local', safeName);
  }

  // scope === 'project'
  return path.join(baseDir, '.codex', 'agent-memory', safeName);
}
...
export function listAgentScopes(
  ...
  const projectDir = path.join(baseDir, '.codex', 'agent-memory');
  const localDir = path.join(baseDir, '.codex', 'agent-memory-local');
  const userDir = path.join(home, '.codex', 'agent-memory');
