...
The `@ruflo/memory` module provides **universal cross-platform support** through intelligent database provider selection. This ensures your application works seamlessly on Windows, macOS, and Linux without code changes.
...
```typescript
import { createDatabase, getPlatformInfo } from '@ruflo/memory';
...
```typescript
import { createDatabase } from '@ruflo/memory';
...
```typescript
import { createDatabase, getAvailableProviders } from '@ruflo/memory';
...
npm install @ruflo/memory --no-optional
...
RUFLO_DB_PROVIDER=sql.js

# Enable verbose logging
RUFLO_DB_VERBOSE=true

# Set auto-persist interval (ms)
RUFLO_DB_PERSIST_INTERVAL=10000
```
