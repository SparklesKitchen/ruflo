...
This implementation adds **complete Windows cross-platform support** to the `@ruflo/memory` module using sql.js as a WASM-based SQLite fallback when native compilation fails.
...
```typescript
import { getPlatformInfo, getAvailableProviders } from '@ruflo/memory';
