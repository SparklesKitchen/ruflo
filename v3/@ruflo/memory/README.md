...
[![npm version](https://img.shields.io/npm/v/@ruflo/memory.svg)](https://www.npmjs.com/package/@ruflo/memory)
[![npm downloads](https://img.shields.io/npm/dm/@ruflo/memory.svg)](https://www.npmjs.com/package/@ruflo/memory)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0+-blue.svg)](https://www.typescriptlang.org/)
[![Performance](https://img.shields.io/badge/Performance-150x--12500x%20Faster-brightgreen.svg)](https://github.com/SparklesKitchen/ruflo)
...
- **Auto Memory Bridge** - Bidirectional sync between Codex auto memory and AgentDB (ADR-048)
...
```bash
npm install @ruflo/memory
```

## Quick Start

```typescript
import { HNSWIndex, AgentDBAdapter, CacheManager } from '@ruflo/memory';
...
```typescript
import { HNSWIndex } from '@ruflo/memory';
...
```typescript
import { AgentDBAdapter } from '@ruflo/memory';
...
```typescript
import { CacheManager } from '@ruflo/memory';
...
```typescript
import { QueryBuilder } from '@ruflo/memory';
...
```typescript
import { MemoryMigration } from '@ruflo/memory';
...
Bidirectional sync between Codex's [auto memory](https://code.codex.com/docs/en/memory) files and AgentDB. Auto memory is a persistent directory (`~/.codex/projects/<project>/memory/`) where Codex writes learnings as markdown. `MEMORY.md` (first 200 lines) is loaded into the system prompt; topic files are read on demand.

### Quick Start

```typescript
import { AutoMemoryBridge } from '@ruflo/memory';
...
Connects insights to the `@ruflo/neural` learning pipeline. When neural is unavailable, all operations degrade to no-ops.

### Quick Start

```typescript
import { AutoMemoryBridge, LearningBridge } from '@ruflo/memory';
...
```typescript
import { LearningBridge } from '@ruflo/memory';

const lb = new LearningBridge(backend, {
  // Optional: inject neural loader for custom setups
  neuralLoader: async () => {
    const { NeuralLearningSystem } = await import('@ruflo/neural');
...
```typescript
import { AutoMemoryBridge, MemoryGraph } from '@ruflo/memory';
...
```typescript
import { MemoryGraph } from '@ruflo/memory';
...
Maps Codex's 3-scope agent memory directories for per-agent knowledge isolation and cross-agent transfer.

### Quick Start

```typescript
import { createAgentBridge, transferKnowledge } from '@ruflo/memory';
...
|-------|-----------|----------|
| `project` | `<gitRoot>/.codex/agent-memory/<agent>/` | Project-specific learnings |
| `local` | `<gitRoot>/.codex/agent-memory-local/<agent>/` | Machine-local data |
| `user` | `~/.codex/agent-memory/<agent>/` | Cross-project user knowledge |
...
// → /workspaces/my-project/.codex/agent-memory/my-agent/
...
- `@ruflo/neural` - **Optional peer dependency** for self-learning (graceful fallback when unavailable)

## Related Packages

- [@ruflo/neural](../neural) - Neural learning integration (SONA, ReasoningBank, EWC++)
- [@ruflo/shared](../shared) - Shared types and utilities
- [@ruflo/hooks](../hooks) - Session lifecycle hooks for auto memory sync
