...
<p align="center">
  <a href="https://www.npmjs.com/package/@ruflo/codex"><img src="https://img.shields.io/npm/v/@ruflo/codex?label=npm&color=blue" alt="npm version"></a>
  <a href="https://github.com/SparklesKitchen/ruflo"><img src="https://img.shields.io/badge/license-MIT-green" alt="license"></a>
...
Transform OpenAI Codex CLI into a **self-improving AI development system**. While Codex executes code, codex orchestrates, coordinates, and **learns from every interaction**.

| Traditional Codex | With Codex-Flow |
|-------------------|------------------|
...
| One platform | Dual-mode (Codex + Codex) |
...
# Dual mode (both Codex and Codex)
...
| **Dual Platform** | Supports both Codex and Codex |
...
# codex  npx      codex mcp start  enabled
```
...
```bash
codex mcp add codex -- npx ruflo mcp start
```
...
| `.codex/data/memory.db` | Vector memory database |
...
npx ruflo@alpha init --codex --minimal
...
<details>
<summary><b>Platform Comparison (Codex vs Codex)</b></summary>
...
| Overrides | `.codex.local.md` | `.codex/config.toml` |
...
| V3 Integration Deep | `$v3-integration-deep` | Deep agentic@alpha integration (ADR-001) |
...
| Skill | Syntax | Description |
|-------|--------|-------------|
| Swarm Orchestration | `$swarm-orchestration` | Multi-agent swarms with agentic |
...
| Skill Builder | `$skill-builder` | Create new Codex Skills |
...
| Skill | Syntax | Description |
|-------|--------|-------------|
| Dual Spawn | `$dual-spawn` | Spawn parallel Codex workers from Codex |
| Dual Coordinate | `$dual-coordinate` | Coordinate Codex + Codex execution |
...
<details>
<summary><b>Dual-Mode Integration (Codex + Codex)</b></summary>
...
From Codex, spawn headless Codex instances:
...
| `$dual-collect` | Codex | Collect results from Codex workers |
...
| Medium (3-5 files) | Codex | Needs context |
| Complex (architecture) | Codex | Reasoning required |
| Bulk operations | Codex Workers | Parallelize |
| Final review | Codex | Integration |

### Example Workflow

```
1. Codex receives complex feature request
...
5. Codex collects and integrates results
6. Final review and refinement in Codex
```

### Memory Sharing

Both platforms share the same `.codex/` runtime:
...
The `@ruflo/codex` package now includes built-in dual-mode orchestration:
...
npx ruflo-codex dual run --template feature --task "Add user authentication"
...
|----------|----------|-----------|
| **feature** | architect → coder → tester → reviewer | Codex (architect, reviewer) + Codex (coder, tester) |
| **security** | scanner → analyzer → fixer | Codex (scanner, fixer) + Codex (analyzer) |
| **refactor** | analyzer → planner → refactorer → validator | Codex (analyzer, planner) + Codex (refactorer, validator) |

### Programmatic API

```typescript
import { DualModeOrchestrator, CollaborationTemplates } from '@ruflo/codex';
...
```typescript
import { initializeCodexProject } from '@ruflo/codex';
...
```typescript
import { CodexInitializer, initializeCodexProject } from '@ruflo/codex';
...
<details>
<summary><b>Migration from Codex</b></summary>

### Convert AGENTS.md to AGENTS.md

```typescript
import { migrate } from '@ruflo/codex';
...
1. **Rename config file**: `AGENTS.md` → `AGENTS.md`
2. **Move skills**: `.codex/skills/` → `.agents/skills/`
3. **Update syntax**: `/skill-name` → `$skill-name`
4. **Convert settings**: `settings.json` → `config.toml`
5. **Register MCP**: `codex mcp add codex -- npx ruflo mcp start`
...
This keeps both `AGENTS.md` and `AGENTS.md` in sync.
...
# Rebuild skills
npx ruflo@alpha init --codex --force
```
...
| Package | Description |
|---------|-------------|
| [@ruflo/cli](https://www.npmjs.com/package/@ruflo/cli) | Main CLI (26 commands, 140+ subcommands) |
| [codex](https://www.npmjs.com/package/codex) | Umbrella package |
| [@ruflo/memory](https://www.npmjs.com/package/@ruflo/memory) | AgentDB with HNSW vector search |
| [@ruflo/security](https://www.npmjs.com/package/@ruflo/security) | Security module |
