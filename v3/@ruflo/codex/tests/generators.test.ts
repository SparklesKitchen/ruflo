...
describe('generateAgentsMd', () => {
  ...
  describe('default template', () => {
    ...
    it('should include commit message format', async () => {
      ...
      expect(result).toContain('Co-Authored-By: codex');
...
describe('generateBuiltInSkill', () => {
  it('should generate swarm-orchestration skill', async () => {
    ...
    expect(result.skillMd).toContain('npx @ruflo/cli swarm init');
  });
...
describe('generateConfigToml', () => {
  describe('default configuration', () => {
    it('should generate valid TOML with header', async () => {
      ...
      expect(result).toContain('# Ruflo V3 - Codex Configuration');
      expect(result).toContain('# Generated by: @ruflo/codex');
      expect(result).toContain('# Documentation: https://github.com/SparklesKitchen/ruflo');
    });
    ...
    it('should include default codex MCP server', async () => {
      const result = await generateConfigToml();

      expect(result).toContain('[mcp_servers.codex]');
      expect(result).toContain('command = "npx"');
      expect(result).toContain('args = ["-y", "@ruflo/cli@latest"]');
  ...
  describe('MCP servers configuration', () => {
    ...
    it('should not duplicate codex server if already specified', async () => {
      const options: ConfigTomlOptions = {
        mcpServers: [
          {
            name: 'codex',
      ...
      const matches = result.match(/\[mcp_servers\.codex\]/g);
      expect(matches).toHaveLength(1);
      expect(result).toContain('command = "custom-codex"');
    });
  });
...
describe('generateMinimalConfigToml', () => {
  it('should generate minimal config', async () => {
    ...
    expect(result).toContain('# Ruflo V3 - Minimal Codex Configuration');
    expect(result).toContain('model = "gpt-5.3-codex"');
    expect(result).toContain('approval_policy = "on-request"');
    expect(result).toContain('sandbox_mode = "workspace-write"');
    expect(result).toContain('[mcp_servers.codex]');
...
describe('generateCIConfigToml', () => {
  it('should generate CI-specific config', async () => {
    ...
    expect(result).toContain('# Ruflo V3 - CI/CD Pipeline Configuration');
    expect(result).toContain('approval_policy = "never"');
