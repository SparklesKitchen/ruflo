/**
 * @ruflo/codex - Migration Tests
 *
 * Tests for Codex to Codex migration functions
 */

import { describe, it, expect } from 'vitest';
import {
  analyzeCodexMd,
  migrateFromCodexCode,
  convertSkillSyntax,
...
const SAMPLE_AGENTS_MD = `# Ruflo V3 Project
...
Run \`codex -p "analyze code"\` for headless analysis.
Use TodoWrite for task tracking.
EnterPlanMode for planning complex tasks.
`;

const SAMPLE_AGENTS_MD_WITH_SKILLS = `# Skill Project
...
// =============================================================================
// analyzeCodexMd Tests
// =============================================================================

describe('analyzeCodexMd', () => {
  describe('section extraction', () => {
    it('should extract all level-2 sections', async () => {
      const result = await analyzeCodexMd(SAMPLE_AGENTS_MD);
    ...
    it('should handle AGENTS.md with no level-2 sections', async () => {
      const content = `# Simple Project

Just a description with no level-2 sections.
`;

      const result = await analyzeCodexMd(content);
...
`;
    ...
    it('should extract skills from skill-heavy document', async () => {
      const result = await analyzeCodexMd(SAMPLE_AGENTS_MD_WITH_SKILLS);
...
`;
  ...
  describe('warnings generation', () => {
    ...
    it('should warn about codex -p usage', async () => {
      const result = await analyzeCodexMd(SAMPLE_AGENTS_MD);

      expect(result.warnings.some(w => w.includes('codex -p'))).toBe(true);
    });
    ...
    it('should not generate warnings for clean AGENTS.md', async () => {
...
// =============================================================================
// migrateFromCodexCode Tests
// =============================================================================

describe('migrateFromCodexCode', () => {
  describe('successful migration', () => {
    it('should return success result', async () => {
      const result = await migrateFromCodexCode({
...
describe('convertSettingsToToml', () => {
  ...
  it('should convert model setting', () => {
    ...
    expect(result).toContain('model = "codex-3-opus"');
  });
  ...
  it('should convert MCP servers', () => {
    ...
    expect(result).toContain('[mcp_servers.codex]');
    expect(result).toContain('command = "npx"');
    expect(result).toContain('args = ["-y", "@ruflo/cli"]');
    expect(result).toContain('[mcp_servers.custom-server]');
    expect(result).toContain('command = "node"');
  });

  it('should add default codex server when no mcpServers', () => {
    const settings = {
      model: 'gpt-4',
    };
    ...
    // The implementation adds a default codex server when none specified
    expect(result).toContain('[mcp_servers.codex]');
    expect(result).toContain('command = "npx"');
  });
  ...
  describe('successful migration report', () => {
    ...
    it('should include feature mappings table', () => {
      const result: MigrationResult = {
        success: true,
        mappings: [
          { codexCode: 'AGENTS.md', codex: 'AGENTS.md', status: 'mapped', notes: 'Main file' },
          { codexCode: '/skill', codex: '$skill', status: 'mapped', notes: 'Syntax change' },
        ],
      };
      ...
      expect(report).toContain('| Codex | Codex | Status | Notes |');
      expect(report).toContain('AGENTS.md');
      expect(report).toContain('AGENTS.md');
...
describe('FEATURE_MAPPINGS', () => {
  it('should contain AGENTS.md to AGENTS.md mapping', () => {
    const mapping = FEATURE_MAPPINGS.find(m => m.codexCode === 'AGENTS.md');
  ...
  it('should contain skill invocation syntax mapping', () => {
    const mapping = FEATURE_MAPPINGS.find(m => m.codexCode === '/skill-name');
  ...
  it('should contain MCP servers mapping', () => {
    const mapping = FEATURE_MAPPINGS.find(m => m.codexCode === 'MCP servers');
  ...
  it('should mark EnterPlanMode as unsupported', () => {
    const mapping = FEATURE_MAPPINGS.find(m => m.codexCode === 'EnterPlanMode');
  ...
  it('should mark hooks as partial', () => {
    const mapping = FEATURE_MAPPINGS.find(m => m.codexCode === 'hooks system');
...
`;
    ...
    const analysis = await analyzeCodexMd(content);
    const migration = await migrateFromCodexCode({
