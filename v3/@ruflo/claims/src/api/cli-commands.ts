// @ts-nocheck - CLI integration requires the full @ruflo/cli package
/**
...
const claimCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      ...
      output.printInfo('Usage: codex issues claim <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const releaseCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues release <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const handoffCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues handoff <issueId> --to <target>');
      return { success: false, exitCode: 1 };
    }
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues status <issueId> [options]');
      return { success: false, exitCode: 1 };
    }
...
const stealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln();
      output.printInfo('Use "codex issues steal <issueId>" to take over an issue');
...
const stealCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues steal <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const markStealableCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues mark-stealable <issueId>');
      return { success: false, exitCode: 1 };
    }
...
const contestCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!issueId) {
      output.printError('Issue ID is required');
      output.printInfo('Usage: codex issues contest <issueId> --reason "..."');
      return { success: false, exitCode: 1 };
    }
...
export const issuesCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'codex issues list --available', description: 'List unclaimed issues' },
    { command: 'codex issues list --mine', description: 'List my claims' },
    { command: 'codex issues claim GH-123', description: 'Claim an issue' },
    { command: 'codex issues release GH-123', description: 'Release a claim' },
    { command: 'codex issues handoff GH-123 --to agent:coder-1', description: 'Request handoff to agent' },
    { command: 'codex issues handoff GH-123 --to human:alice', description: 'Request handoff to human' },
    { command: 'codex issues status GH-123 --blocked "Waiting for API"', description: 'Mark as blocked' },
    { command: 'codex issues status GH-123 --review-requested', description: 'Request review' },
    { command: 'codex issues board', description: 'View who is working on what' },
    { command: 'codex issues stealable', description: 'List stealable issues' },
    { command: 'codex issues steal GH-123', description: 'Steal an issue' },
    { command: 'codex issues mark-stealable GH-123', description: 'Mark my claim as stealable' },
    { command: 'codex issues contest GH-123 -r "I was actively working on it"', description: 'Contest a steal' },
    { command: 'codex issues load', description: 'View agent load distribution' },
    { command: 'codex issues load --agent coder-1', description: 'View specific agent load' },
    { command: 'codex issues rebalance --dry-run', description: 'Preview rebalancing' },
    { command: 'codex issues rebalance', description: 'Trigger swarm rebalancing' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex issues <subcommand> [options]');
    output.writeln();
    ...
    output.writeln();
    output.writeln('Run "codex issues <subcommand> --help" for subcommand help');
