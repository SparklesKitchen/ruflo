...
CREATE OR REPLACE FUNCTION codex_flow.softmax(
    scores REAL[]
) RETURNS REAL[] AS $$
DECLARE
    max_score REAL;
...
CREATE OR REPLACE FUNCTION codex_flow.scaled_dot_product_attention(
...
BEGIN
    ...
    -- Apply softmax
    attention_weights := codex_flow.softmax(scores);
...
CREATE OR REPLACE FUNCTION codex_flow.multi_head_attention(
...
BEGIN
    ...
    FOR h IN 1..num_heads LOOP
        ...
        head_output := codex_flow.scaled_dot_product_attention(head_query, head_keys, head_values);
...
CREATE OR REPLACE FUNCTION codex_flow.flash_attention(
...
CREATE OR REPLACE FUNCTION codex_flow.sparse_attention(
    query REAL[],        -- Query vector
    keys REAL[][],       -- Key matrix
    values REAL[][],     -- Value matrix
...
CREATE OR REPLACE FUNCTION codex_flow.linear_attention(
    query REAL[],        -- Query vector
    keys REAL[][],       -- Key matrix
    values REAL[][],     -- Value matrix
...
CREATE OR REPLACE FUNCTION codex_flow.cross_attention(
...
DECLARE
    result RECORD;
BEGIN
    SELECT * INTO result FROM codex_flow.multi_head_attention(
        decoder_query,
        encoder_keys,
        encoder_values,
        num_heads
...
CREATE OR REPLACE FUNCTION codex_flow.cache_attention_result(
    p_query_hash TEXT,
    p_keys_hash TEXT,
    p_values_hash TEXT,
    p_num_heads INTEGER,
...
BEGIN
    ...
    INSERT INTO codex_flow.attention_cache (
        cache_key,
        query_hash,
        keys_hash,
        values_hash,
    ...
    SET attention_weights = EXCLUDED.attention_weights,
        ...
        hit_count = codex_flow.attention_cache.hit_count + 1,
...
CREATE OR REPLACE FUNCTION codex_flow.get_cached_attention(
    p_query_hash TEXT,
    p_keys_hash TEXT,
    p_values_hash TEXT,
    p_num_heads INTEGER,
...
BEGIN
    ...
    INTO v_record
    FROM codex_flow.attention_cache ac
    WHERE ac.cache_key = v_cache_key
      AND (ac.expires_at IS NULL OR ac.expires_at > NOW());

    IF FOUND THEN
        -- Update access stats
        UPDATE codex_flow.attention_cache
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS codex_flow.get_cached_attention(TEXT, TEXT, TEXT, INTEGER, TEXT);
-- DROP FUNCTION IF EXISTS codex_flow.cache_attention_result(TEXT, TEXT, TEXT, INTEGER, TEXT, REAL[], REAL[], INTEGER[], REAL, INTEGER);
-- DROP FUNCTION IF EXISTS codex_flow.cross_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS codex_flow.linear_attention(REAL[], REAL[][], REAL[][], TEXT);
-- DROP FUNCTION IF EXISTS codex_flow.sparse_attention(REAL[], REAL[][], REAL[][], TEXT, INTEGER, INTEGER);
-- DROP FUNCTION IF EXISTS codex_flow.flash_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS codex_flow.multi_head_attention(REAL[], REAL[][], REAL[][], INTEGER);
-- DROP FUNCTION IF EXISTS codex_flow.scaled_dot_product_attention(REAL[], REAL[][], REAL[][], REAL);
-- DROP FUNCTION IF EXISTS codex_flow.softmax(REAL[]);
-- DELETE FROM codex_flow.migrations WHERE name = '005_create_attention_functions';
-- COMMIT;
