...
--
-- Enables required PostgreSQL extensions and creates the codex_flow schema.
...
-- ----------------------------------------------------------------------------
-- Schema: codex_flow
-- Namespace for all Ruflo tables and functions
-- ----------------------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS codex_flow;

-- Set search path to include codex_flow schema
COMMENT ON SCHEMA codex_flow IS 'Ruflo V3 - RuVector PostgreSQL Bridge schema';

-- Grant usage on schema
DO $$
BEGIN
    -- Grant to PUBLIC for general access (modify as needed for security)
    GRANT USAGE ON SCHEMA codex_flow TO PUBLIC;
    GRANT CREATE ON SCHEMA codex_flow TO PUBLIC;
END $$;

-- ----------------------------------------------------------------------------
-- Configuration table for RuVector settings
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS codex_flow.config (
...
INSERT INTO codex_flow.config (key, value) VALUES
    ('version', '"1.0.0"'),
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS codex_flow.migrations (
...
-- ============================================================================
-- BEGIN;
-- DROP TABLE IF EXISTS codex_flow.migrations;
-- DROP TABLE IF EXISTS codex_flow.config;
-- DROP SCHEMA IF EXISTS codex_flow CASCADE;
