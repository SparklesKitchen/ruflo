 ...
 * Outputs plain text or JSON that Codex hooks can consume.
 *
 * Usage:
 *   npx @ruflo/hooks session-context
 *   npx @ruflo/hooks user-prompt "Fix authentication bug"
 *   npx @ruflo/hooks pre-edit "/path/to/file.ts"
 *   npx @ruflo/hooks route "Implement caching layer"
 *
 * @module @ruflo/hooks/cli/guidance-cli
 */
...
function printHelp(): void {
  console.log(`
@ruflo/hooks - V3 Guidance System CLI

Usage: npx @ruflo/hooks <command> [args]

Guidance Commands (for Codex hooks):
...
Exit Codes:
  0 - Success (stdout added as context for Codex)
  2 - Block (stderr shown to Codex as reason)
  1 - Error
...
Examples:
  ...
  npx @ruflo/hooks user-prompt "Fix authentication security vulnerability"

  # Pre-edit security check
  npx @ruflo/hooks pre-edit "src/auth/login.ts"

  # Agent routing
  npx @ruflo/hooks route "Implement HNSW vector search"

  # Store a learned pattern
  npx @ruflo/hooks store "Use dependency injection for testability" architecture

  # Broadcast pattern to swarm
  npx @ruflo/hooks swarm-pattern-broadcast "Use HNSW for 150x faster search" memory

  # Initiate consensus
  npx @ruflo/hooks swarm-consensus "Which auth method?" "JWT,OAuth2,Session"

  # Hand off task
  npx @ruflo/hooks swarm-handoff security-auditor "Review auth implementation"
`);
}
