 ...
 *
 * Generates Codex-visible guidance output from ReasoningBank patterns.
 * Outputs plain text (exit 0) or JSON with additionalContext.
 *
 * @module @ruflo/hooks/reasoningbank/guidance-provider
 */
...
/**
 * Official Codex hook output format
 */
 ...
 *
 * Converts ReasoningBank patterns into Codex-visible guidance.
 */
export class GuidanceProvider {
   ...
   * Returns plain text that Codex will see
   */
  async generateSessionContext(): Promise<string> {
    ...
    const lines = [
      ...
      '',
      '**Architecture**: Domain-Driven Design with 15 @codex modules',
      ...
      '- agentic@alpha as core foundation',
   ...
   * Returns JSON for Codex hook system
   */
  async generatePreEditGuidance(filePath: string): Promise<CodexHookOutput> {
   ...
   */
  async generatePreCommandGuidance(command: string): Promise<CodexHookOutput> {
