 ...
 * - ONNX embeddings via @ruflo/embeddings (MiniLM-L6 384-dim)
 * - AgentDB backend for persistence
 * - Pattern promotion from short-term to long-term memory
 *
 * @module @ruflo/hooks/reasoningbank
 */
...
export class ReasoningBank extends EventEmitter {
  ...
  private async loadDependencies(): Promise<void> {
    ...
    const memoryModule = await dynamicImport('@ruflo/memory');
    if (memoryModule) {
      AgentDBAdapter = memoryModule.AgentDBAdapter;
      HNSWIndex = memoryModule.HNSWIndex;
    }

    const embeddingsModule = await dynamicImport('@ruflo/embeddings');
    if (embeddingsModule) {
...
/**
 * Real embedding service using @ruflo/embeddings
 */
...
class FallbackEmbeddingService implements IEmbeddingService {
  ...
  async embed(text: string): Promise<Float32Array> {
    ...
    // Try agentic ONNX embeddings first
    try {
      ...
      const result = execFileSync(
        'npx',
        ['agentic@alpha', 'embeddings', 'generate', safeText, '--format', 'json'],
