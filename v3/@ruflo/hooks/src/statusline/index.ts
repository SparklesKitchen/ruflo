 ...
 *
 * Generates statusline data for Codex integration.
 * Provides real-time progress, metrics, and status information.
 *
 * Format matches the working .codex/statusline.sh output:
...
export class StatuslineGenerator {
  ...
  /**
   * Generate formatted statusline string matching .codex/statusline.sh format
   */
  ...
  /**
   * Generate single-line output for Codex compatibility
   * This avoids the multi-line collision bug where Codex's internal status
   * (written at absolute terminal coordinates ~cols 15-25) bleeds into conversation
   *
   * @see https://github.com/SparklesKitchen/ruflo/issues/985
   */
  ...
  generateSafeStatusline(): string {
    ...
    // Line 5: Architecture status (LAST LINE - Codex writes BELOW this)
    const dddColor = data.v3Progress.dddProgress >= 50 ? c.brightGreen :
                     data.v3Progress.dddProgress > 0 ? c.yellow : c.red;
    const dddDisplay = String(data.v3Progress.dddProgress).padStart(3);
    const integrationColor = data.swarm.coordinationActive ? c.brightCyan : c.dim;
  ...
  private getV3Progress(): StatuslineData['v3Progress'] {
    ...
    const metricsPath = join(this.projectRoot, '.codex', 'metrics', 'v3-progress.json');
    try {
  ...
  private getSecurityStatus(): StatuslineData['security'] {
    ...
    const auditPath = join(this.projectRoot, '.codex', 'security', 'audit-status.json');
    try {
  ...
  private getSwarmActivity(): StatuslineData['swarm'] {
    ...
    try {
      ...
      const agenticCount = (ps.match(/agentic/g) || []).length;
    ...
    const activityPath = join(this.projectRoot, '.codex', 'metrics', 'swarm-activity.json');
    try {
  ...
  private getSystemMetrics(): ExtendedStatuslineData['system'] {
    ...
    try {
      ...
      const ps = execSync('ps aux 2>/dev/null | grep -E "(node|agentic|codex)" | grep -v grep | awk \'{sum += $6} END {print int(sum/1024)}\'', { encoding: 'utf-8' });
    ...
    const patternsPath = join(this.projectRoot, '.codex', 'learning', 'patterns.db');
    try {
    ...
    return {
      memoryMB,
      contextPct: 0, // Requires Codex input
      intelligencePct,
      subAgents,
    };
  }
  ...
  private getUserInfo(): ExtendedStatuslineData['user'] {
    ...
    // Model name would come from Codex input
