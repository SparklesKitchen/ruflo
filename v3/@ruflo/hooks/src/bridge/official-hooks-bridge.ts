/**
 * Official Codex Hooks Bridge
 *
 * Maps V3 internal hook events to official Codex hook events.
 * This bridge enables seamless integration between codex's
 * internal hook system and the official Codex plugin API.
 *
...
/**
 * Official Codex hook event types
 * Based on https://code.codex.com/docs/en/hooks
 */
...
export interface OfficialHookOutput {
  ...
  /** Whether to continue processing (false stops Codex) */
  continue?: boolean;
...
export class OfficialHooksBridge {
  ...
  static createCLICommand(event: HookEvent, handler: string): string {
    const baseCommand = 'npx ruflo@alpha hooks';
...
/**
 * Process stdin from official Codex hook system
 */
...
/**
 * Output result to official Codex hook system
 */
