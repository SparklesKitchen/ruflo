 ...
 *
 * Optimizes Ruflo with non-blocking, scheduled workers.
...
export class WorkerManager extends EventEmitter {
  ...
  constructor(projectRoot?: string) {
    ...
    this.metricsDir = path.join(this.projectRoot, '.codex', 'metrics');
...
export function createSwarmWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const activityPath = path.join(projectRoot, '.codex', 'metrics', 'swarm-activity.json');
    ...
    const queuePath = path.join(projectRoot, '.codex', 'swarm', 'queue');
    let queueCount = 0;
    try {
...
export function createLearningWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const patternsDbPath = path.join(projectRoot, '.codex', 'learning', 'patterns.db');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.codex', 'metrics', 'learning.json');
      try {
...
export function createADRWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const [
      ...
      fs.readFile(path.join(v3Path, 'package.json'), 'utf-8')
        .then(content => {
          ...
          return {
            compliant: pkg.dependencies?.['agentic'] !== undefined ||
                       pkg.devDependencies?.['agentic'] !== undefined,
            reason: 'agentic dependency',
          };
        })
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.codex', 'metrics', 'adr-compliance.json');
...
export function createDDDWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save metrics
    try {
      const outputPath = path.join(projectRoot, '.codex', 'metrics', 'ddd-progress.json');
      await fs.writeFile(outputPath, JSON.stringify({
        timestamp: new Date().toISOString(),
...
export function createSecurityWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Save results
    try {
      const outputPath = path.join(projectRoot, '.codex', 'security', 'scan-results.json');
...
export function createPatternsWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    const learningDir = path.join(projectRoot, '.codex', 'learning');
    ...
    try {
      ...
      const metricsPath = path.join(projectRoot, '.codex', 'metrics', 'patterns.json');
...
export function createCacheWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    ...
    // Only clean directories within .codex (safe)
    const safeCleanDirs = [
      '.codex/cache',
      '.codex/temp',
    ];
...
export function createV3ProgressWorker(projectRoot: string): WorkerHandler {
  return async (): Promise<WorkerResult> => {
    const startTime = Date.now();
    const v3Path = path.join(projectRoot, 'v3');
    const cliPath = path.join(v3Path, '@codex', 'cli', 'src');
    ...
    // Count @codex packages (excluding hidden directories)
    let packages = 0;
    const packageDirs: string[] = [];
    try {
      const packagesPath = path.join(v3Path, '@codex');
    ...
    for (const pkg of packageDirs) {
      ...
      try {
        const srcPath = path.join(v3Path, '@codex', pkg, 'src');
    ...
    let totalLines = 0;
    try {
      const v3CodexFlow = path.join(v3Path, '@codex');
      totalFiles = await countFilesRecursive(v3CodexFlow, '.ts');
      totalLines = await countLines(v3CodexFlow, '.ts');
    } catch {
      totalFiles = 419;
      totalLines = 290913;
    }
    ...
    try {
      const metricsDir = path.join(projectRoot, '.codex', 'metrics');
