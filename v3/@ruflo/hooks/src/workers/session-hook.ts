 ...
 *
 * Auto-starts workers when Codex session begins.
 */
...
/**
 * Generate session start output for Codex hooks
 *
 * Returns formatted output suitable for Codex SessionStart hook.
 */
...
/**
 * Generate a shell hook script for integration with .codex/settings.json
 */
export function generateShellHook(projectRoot: string): string {
  const hookPath = path.join(projectRoot, 'v3', '@codex', 'hooks');

  return `#!/bin/bash
# Ruflo V3 Workers - Session Start Hook
