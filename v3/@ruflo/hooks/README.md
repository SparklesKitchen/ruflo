...
[![npm version](https://img.shields.io/npm/v/@ruflo/hooks.svg)](https://www.npmjs.com/package/@ruflo/hooks)
[![npm downloads](https://img.shields.io/npm/dm/@ruflo/hooks.svg)](https://www.npmjs.com/package/@ruflo/hooks)
...
The `@ruflo/hooks` package provides a comprehensive hooks system for intercepting and extending Ruflo operations. It enables intelligent task routing, pattern learning, background metrics collection, and real-time statusline integration.
...
- 📊 **Statusline Integration** - Real-time status display for Codex
...
# Using pnpm
pnpm add @ruflo/hooks

# Using yarn
yarn add @ruflo/hooks
```
...
```typescript
import { initializeHooks } from '@ruflo/hooks';
...
```typescript
import { hooksMCPTools, getHooksTool } from '@ruflo/hooks';
...
Generate statusline output for Codex integration.
...
codex hooks worker detect --prompt "deep dive into auth" --auto-dispatch --min-confidence 0.6
...
Workers are automatically triggered via the `UserPromptSubmit` hook when prompt patterns match worker triggers with confidence ≥0.6. Add this to your Codex settings:
...
|----------|-------------|---------|
| `RUFLO_HOOK_TIMEOUT` | Hook execution timeout (ms) | `5000` |
| `RUFLO_REASONINGBANK_ENABLED` | Enable ReasoningBank | `true` |
| `RUFLO_HOOKS_NAMESPACE` | Learning namespace | `hooks-learning` |
| `RUFLO_HOOKS_LOG_LEVEL` | Logging level | `info` |
| `RUFLO_SHOW_HOOKS_METRICS` | Show hooks in statusline | `true` |
| `RUFLO_SHOW_SWARM_ACTIVITY` | Show swarm in statusline | `true` |
| `RUFLO_SHOW_PERFORMANCE` | Show performance targets | `true` |

## Integration with Codex

Add to your Codex settings (`~/.codex/settings.json`):
...
- [@ruflo/shared](../shared) - Shared utilities and types
- [@ruflo/neural](../neural) - Neural network and SONA learning
- [@ruflo/swarm](../swarm) - Multi-agent coordination
- [@ruflo/memory](../memory) - AgentDB memory system

## License

MIT © [Ruflo Team](https://github.com/SparklesKitchen/ruflo)
