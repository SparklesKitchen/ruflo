...
[![npm version](https://img.shields.io/npm/v/@ruflo/guidance.svg?style=flat-square&label=npm)](https://www.npmjs.com/package/@ruflo/guidance)
[![npm downloads](https://img.shields.io/npm/dm/@ruflo/guidance.svg?style=flat-square&label=downloads)](https://www.npmjs.com/package/@ruflo/guidance)
[![license](https://img.shields.io/npm/l/@ruflo/guidance.svg?style=flat-square)](https://github.com/SparklesKitchen/ruflo/blob/main/LICENSE)
[![tests](https://img.shields.io/badge/tests-1%2C328%20passing-brightgreen?style=flat-square)](https://github.com/SparklesKitchen/ruflo)
[![node](https://img.shields.io/badge/node-%3E%3D20-blue?style=flat-square)](https://nodejs.org)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.3+-3178C6?style=flat-square&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![GitHub stars](https://img.shields.io/github/stars/ruvnet/codex?style=flat-square&logo=github)](https://github.com/SparklesKitchen/ruflo)
[![codex](https://img.shields.io/npm/v/codex.svg?style=flat-square&label=codex&color=blueviolet)](https://www.npmjs.com/package/codex)
...
**Long-horizon governance for Codex agents.**
...
`@ruflo/guidance` fixes this. It takes the memory files Codex already uses — `AGENTS.md` and `.codex/AGENTS.override.md` — and turns them into a structured control plane that compiles rules, enforces them through gates the agent cannot bypass, proves every decision cryptographically, and evolves the rule set over time based on what actually works.

The result: agents that can operate for days instead of minutes.

## The Problem

Codex agents load `AGENTS.md` into their context at session start. That's the entire governance mechanism — a text file that the model reads once and then gradually forgets. There is no enforcement, no audit trail, no memory protection, and no way to measure whether the rules are working.
...
This is not a prompt engineering library. It is not a wrapper around `AGENTS.md`. It is a runtime governance system with enforcement gates, cryptographic proofs, and feedback loops.

| Capability | Plain AGENTS.md | Prompt libraries | @ruflo/guidance |
|-----------|:-:|:-:|:-:|
...
1. **Compiles** `AGENTS.md` + `.codex/AGENTS.override.md` into a typed policy bundle — a constitution (always-loaded invariants) plus task-scoped rule shards
...
Codex uses two plain-text files as agent memory. Understanding them is essential because they are the input to the control plane.

| File | Scope | Purpose |
|------|-------|---------|
| **AGENTS.md** | Team / repo | Shared guidance: architecture, workflows, build commands, coding standards, domain rules. Lives at `./AGENTS.md` or `./.codex/AGENTS.md`. Committed to git. |
| **.codex/AGENTS.override.md** | Individual / machine | Private notes: local sandbox URLs, test data, machine quirks, personal preferences. Auto-added to `.gitignore` by Codex. Stays local. |

**How they get loaded:** Codex searches upward from the current working directory and loads every `AGENTS.md` and `.codex/AGENTS.override.md` it finds on the path. In monorepos and nested projects, child directories can have their own files that layer on top of parent ones. It also discovers additional `AGENTS.md` files in subtrees as it reads files there.

**The @import pattern:** For "local" instructions that work cleanly across multiple git worktrees, you can use `@` imports inside `AGENTS.md` that point to a file in each developer's home directory:

```markdown
# Individual Preferences
@~/.codex/my_project_instructions.md
```

**Verification:** Run `/memory` in Codex to see which files were loaded. You can test by placing a unique rule in each file and asking Codex to restate both.

## Architecture

The control plane is organized as a 7-phase pipeline. Each module is independently testable with a clean API boundary. The WASM kernel accelerates security-critical paths, and the generate/analyze layers provide tooling for creating and measuring AGENTS.md quality.

```mermaid
graph TB
  subgraph Compile["Phase 1: Compile"]
    CODEX["AGENTS.md"] --> GC["GuidanceCompiler"]
    GC --> PB["PolicyBundle"]
...
## How AGENTS.md Becomes Enforceable Policy
...
graph LR
    subgraph "Your repo"
        A["AGENTS.md<br/>(team rules)"]
        B[".codex/AGENTS.override.md<br/>(your overrides)"]
    end
...
The compiler splits `AGENTS.md` into two parts:
...
`.codex/AGENTS.override.md` overlays the root. The optimizer watches which local experiments reduce violations and promotes winners to root `AGENTS.md`, generating an ADR for each change.
...
|-------|-----------|---------|
| **Compile** | `GuidanceCompiler` | AGENTS.md → constitution + task-scoped shards |
...
| **Generate** | `generateCodexMd` | Scaffold AGENTS.md from a project profile |
| | `generateCodexLocalMd` | Scaffold .codex/AGENTS.override.md from a local profile |
| | `generateSkillMd` / `generateAgentMd` | Scaffold skill definitions and agent manifests |
| | `scaffold` | Full project scaffolding with AGENTS.md, agents, and skills |
...
| | `headlessBenchmark` | Headless `codex -p` benchmarking with proof chain |
...
```typescript
import { getKernel } from '@ruflo/guidance/wasm-kernel';
...
## AGENTS.md vs. .codex/AGENTS.override.md — What Goes Where

Two files, two audiences. `AGENTS.md` carries team-wide rules that every agent follows. `.codex/AGENTS.override.md` carries individual experiments and machine-specific config. The optimizer watches local experiments and promotes winning ones to the shared file.

### AGENTS.md (team shared, committed to git)
...
### .codex/AGENTS.override.md (personal, stays local)
...
If you use multiple git worktrees, `.codex/AGENTS.override.md` gets awkward because each worktree needs its own copy. Use `@` imports instead:

```markdown
# In your committed AGENTS.md:
@~/.codex/my_project_instructions.md
```
...
```typescript
// Compile AGENTS.md into structured policy
import { createCompiler } from '@ruflo/guidance/compiler';
const compiler = createCompiler();
const bundle = compiler.compile(codexMdContent);

// Retrieve task-relevant shards by intent
import { createRetriever } from '@ruflo/guidance/retriever';
...
import { createGates } from '@ruflo/guidance/gates';
...
```typescript
import { createContinueGate } from '@ruflo/guidance/continue-gate';
const gate = createContinueGate({
...
```typescript
import { createProofChain } from '@ruflo/guidance/proof';
...
import { createToolGateway } from '@ruflo/guidance/gateway';
...
// Memory write gating
import { createMemoryWriteGate } from '@ruflo/guidance/memory-gate';
...
import { TrustSystem } from '@ruflo/guidance/trust';
...
import { createTruthAnchorStore, createTruthResolver } from '@ruflo/guidance/truth-anchors';
...
import { UncertaintyLedger } from '@ruflo/guidance/uncertainty';
...
import { TemporalStore, TemporalReasoner } from '@ruflo/guidance/temporal';
...
```typescript
import { AuthorityGate, IrreversibilityClassifier } from '@ruflo/guidance/authority';
...
import { createThreatDetector, createCollusionDetector, createMemoryQuorum }
  from '@ruflo/guidance/adversarial';
...
```typescript
import { createMetaGovernor } from '@ruflo/guidance/meta-governance';
...
<details>
<summary><strong>Tutorial: Wiring into Codex hooks</strong></summary>

```typescript
import { createGuidanceHooks } from '@ruflo/guidance';
...
```typescript
import { createConformanceRunner, createMemoryClerkCell } from '@ruflo/guidance/conformance-kit';
...
```typescript
import { createEvolutionPipeline } from '@ruflo/guidance/evolution';
...
### Generators (AGENTS.md Scaffolding)

Instead of writing AGENTS.md from scratch, use the generators to scaffold high-scoring files from a project profile. The generated files follow best practices for structure, coverage, and enforceability.
...
import {
  ...
  generateAgentIndex,
  scaffold,
} from '@ruflo/guidance/generators';

// Generate a AGENTS.md from a project profile
const codexMd = generateCodexMd({
  name: 'my-api',
...
// Generate a .codex/AGENTS.override.md for local dev
const localMd = generateCodexLocalMd({
  name: 'Alice',
...
The analyzer answers a question most teams cannot: "Is our AGENTS.md actually working?" It scores files across 6 dimensions, auto-optimizes them for higher scores, and empirically validates that higher scores produce better agent behavior using statistical correlation.
...
} from '@ruflo/guidance/analyzer';

// 1. Score a AGENTS.md file
const result = analyze(codexMdContent);
...
// 5. Headless Codex benchmarking (codex -p integration)
const headless = await headlessBenchmark(originalMd, optimizedMd, {
  executor: myExecutor,  // or uses real `codex -p` by default
  proofKey: 'bench-key',
});
...
const validation = await validateEffect(originalMd, optimizedMd, {
  executor: myContentAwareExecutor,  // varies behavior per AGENTS.md
...
**Content-aware executors** implement `IContentAwareExecutor` — they receive the AGENTS.md content via `setContext()` before each validation phase, allowing their responses to vary based on the quality of guidance loaded. This is what makes the empirical proof meaningful.

```typescript
import type { IContentAwareExecutor } from '@ruflo/guidance/analyzer';

class MyExecutor implements IContentAwareExecutor {
  private rules: string[] = [];

  setContext(codexMdContent: string): void {
    // Parse loaded AGENTS.md to determine how to behave
    this.rules = codexMdContent.match(/\b(NEVER|ALWAYS|MUST)\b.+/g) || [];
  }
...
```typescript
import { abBenchmark, getDefaultABTasks } from '@ruflo/guidance/analyzer';

// Run A/B benchmark with content-aware executor
const report = await abBenchmark(codexMdContent, {
...
|-----------|------:|-------------------|
| compiler | 11 | AGENTS.md parsing, constitution extraction, shard splitting |
...
| generators | 68 | AGENTS.md scaffolding, profiles, skills, agents, full scaffold |
...
| Resource | URL |
|----------|-----|
| **GitHub** | [github.com/SparklesKitchen/ruflo](https://github.com/SparklesKitchen/ruflo) |
| **npm: @ruflo/guidance** | [npmjs.com/package/@ruflo/guidance](https://www.npmjs.com/package/@ruflo/guidance) |
| **npm: codex** | [npmjs.com/package/codex](https://www.npmjs.com/package/codex) |
| **npm: ruvbot** | [npmjs.com/package/ruvbot](https://www.npmjs.com/package/ruvbot) |
| **ruv.io** | [ruv.io](https://ruv.io) |
| **Issues** | [github.com/SparklesKitchen/ruflo/issues](https://github.com/SparklesKitchen/ruflo/issues) |
| **API Reference** | [docs/reference/api-quick-reference.md](docs/reference/api-quick-reference.md) |
| **ADR Index** | [docs/adrs/](docs/adrs/) |

## License

MIT — see [LICENSE](https://github.com/SparklesKitchen/ruflo/blob/main/LICENSE) for details.
