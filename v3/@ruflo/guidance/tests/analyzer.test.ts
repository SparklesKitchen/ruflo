...
const MINIMAL_CODEX_MD = `# My Project

Some basic info about the project.
`;

const WELL_STRUCTURED_CODEX_MD = `# My Project
...
const POOR_CODEX_MD = `some rules
dont do bad stuff
be good
`;
...
describe('analyze', () => {
  describe('compositeScore', () => {
    it('returns a score between 0 and 100', () => {
      const result = analyze(WELL_STRUCTURED_CODEX_MD);
      expect(result.compositeScore).toBeGreaterThanOrEqual(0);
      expect(result.compositeScore).toBeLessThanOrEqual(100);
    });

    it('scores a well-structured file higher than a poor one', () => {
      const good = analyze(WELL_STRUCTURED_CODEX_MD);
      const bad = analyze(POOR_CODEX_MD);
      expect(good.compositeScore).toBeGreaterThan(bad.compositeScore);
    });

    it('scores a minimal file in the middle range', () => {
      const result = analyze(MINIMAL_CODEX_MD);
      expect(result.compositeScore).toBeLessThan(analyze(WELL_STRUCTURED_CODEX_MD).compositeScore);
      expect(result.compositeScore).toBeGreaterThan(analyze(POOR_CODEX_MD).compositeScore);
    });
  });
  ...
  describe('grade', () => {
    ...
    it('assigns F grade for very poor content', () => {
      const result = analyze(POOR_CODEX_MD);
  ...
  describe('metrics', () => {
    ...
    it('detects @import directives', () => {
      const withImport = WELL_STRUCTURED_CODEX_MD + '\n@~/.codex/my_instructions.md\n';
  ...
  describe('local content overlay', () => {
    it('accepts optional local content', () => {
      const result = analyze(WELL_STRUCTURED_CODEX_MD, LOCAL_OVERLAY);
      expect(result.compositeScore).toBeGreaterThan(0);
    });

    it('compilability dimension uses local overlay', () => {
      const withLocal = analyze(WELL_STRUCTURED_CODEX_MD, LOCAL_OVERLAY);
      const withoutLocal = analyze(WELL_STRUCTURED_CODEX_MD);
...
describe('benchmark', () => {
  it('computes delta between before and after', () => {
    const result = benchmark(POOR_CODEX_MD, WELL_STRUCTURED_CODEX_MD);
  ...
  it('reports regressions when going from good to bad', () => {
    const result = benchmark(WELL_STRUCTURED_CODEX_MD, POOR_CODEX_MD);
    expect(result.delta).toBeLessThan(0);
    expect(result.regressions.length).toBeGreaterThan(0);
  });

  it('shows zero delta for identical content', () => {
    const result = benchmark(WELL_STRUCTURED_CODEX_MD, WELL_STRUCTURED_CODEX_MD);
  ...
  it('accepts optional local content', () => {
    const result = benchmark(POOR_CODEX_MD, WELL_STRUCTURED_CODEX_MD, LOCAL_OVERLAY);
    expect(result.delta).toBeGreaterThan(0);
  });
...
describe('autoOptimize', () => {
  it('improves the score of poor content', () => {
    const result = autoOptimize(POOR_CODEX_MD);
  ...
  it('returns the optimized content', () => {
    ...
    expect(result.optimized.length).toBeGreaterThan(POOR_CODEX_MD.length);
  });
  ...
  it('respects maxIterations', () => {
    const onePass = autoOptimize(POOR_CODEX_MD, undefined, 1);
    const threePasses = autoOptimize(POOR_CODEX_MD, undefined, 3);
  ...
  it('does not regress well-structured content', () => {
    const result = autoOptimize(WELL_STRUCTURED_CODEX_MD);
  ...
  it('accepts local content overlay', () => {
    const result = autoOptimize(POOR_CODEX_MD, LOCAL_OVERLAY);
    expect(result.benchmark.delta).toBeGreaterThan(0);
  });
});
...
describe('edge cases', () => {
  ...
  it('benchmark handles identical content correctly', () => {
    const result = benchmark(MINIMAL_CODEX_MD, MINIMAL_CODEX_MD);
    expect(result.delta).toBe(0);
    expect(result.improvements).toHaveLength(0);
    expect(result.regressions).toHaveLength(0);
  });
  ...
  it('autoOptimize handles already-optimal content gracefully', () => {
    ...
    const result = autoOptimize(WELL_STRUCTURED_CODEX_MD, undefined, 5);
    expect(result.benchmark.delta).toBeGreaterThanOrEqual(0);
    expect(result.optimized.length).toBeGreaterThanOrEqual(WELL_STRUCTURED_CODEX_MD.length);
  });
});
...
// A large, realistic AGENTS.md with enforcement prose and long sections
const LARGE_CODEX_MD = `# My Project
...
When starting work on complex tasks, Codex MUST automatically:
...
When user says "spawn swarm", Codex MUST in ONE message:
...
npx ruflo memory search -q "auth patterns"
...
\`\`\`bash
RUFLO_CONFIG=./codex.config.json
...
describe('optimizeForSize', () => {
  describe('compact context', () => {
    it('produces output within compact line budget', () => {
      const result = optimizeForSize(LARGE_CODEX_MD, { contextSize: 'compact' });
  ...
  describe('standard context', () => {
    it('produces output within standard line budget', () => {
      const result = optimizeForSize(LARGE_CODEX_MD, { contextSize: 'standard' });
      const lineCount = result.optimized.split('\n').length;
      expect(lineCount).toBeLessThanOrEqual(250);
    });

    it('reaches higher score than compact', () => {
      const compact = optimizeForSize(LARGE_CODEX_MD, { contextSize: 'compact' });
      const standard = optimizeForSize(LARGE_CODEX_MD, { contextSize: 'standard' });
  ...
  describe('full context', () => {
    it('keeps most content intact', () => {
      const result = optimizeForSize(LARGE_CODEX_MD, { contextSize: 'full' });
      expect(result.optimized.length).toBeGreaterThanOrEqual(LARGE_CODEX_MD.length * 0.8);
    });
...
Codex MUST automatically spawn agents.
...
`;
  ...
  describe('target score', () => {
    it('stops when target score is reached', () => {
      const result = optimizeForSize(WELL_STRUCTURED_CODEX_MD, {
        contextSize: 'full',
  ...
  describe('proof chain', () => {
    it('generates proof envelopes when proofKey is provided', () => {
      const result = optimizeForSize(LARGE_CODEX_MD, {
...
// ============================================================================
// headlessBenchmark() — codex -p integration
// ============================================================================

describe('headlessBenchmark', () => {
  // Mock executor that simulates codex -p responses
  ...
  it('runs benchmark with mock executor', async () => {
    ...
    expect(result.report).toContain('Headless Codex Benchmark');
  });
  ...
  it('detects improvement after optimization', async () => {
    const optimized = optimizeForSize(POOR_CODEX_MD, { contextSize: 'standard' });
    const result = await headlessBenchmark(
      POOR_CODEX_MD,
      optimized.optimized,
      { executor: new MockHeadlessExecutor() },
...
class CompliantExecutor implements IHeadlessExecutor {
  async execute(prompt: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    if (lower.includes('environment') || lower.includes('env')) {
      return { stdout: 'Required environment variables: OPENAI_API_KEY=your-key NODE_ENV=production', stderr: '', exitCode: 0 };
    }
    return { stdout: '{}', stderr: '', exitCode: 0 };
  }
}
 ...
 * based on the quality of the loaded AGENTS.md content.
 *
 * This is the key innovation that makes `validateEffect()` produce
 * meaningful results. When the AGENTS.md has:
 ...
 *
 * When the AGENTS.md is poor/missing those sections, the executor produces
...
describe('validateEffect', () => {
  ...
  describe('content-aware differential executor', () => {
    it('produces higher adherence for well-structured AGENTS.md than poor', async () => {
...
class ABDifferentialExecutor implements IContentAwareExecutor {
  ...
  async execute(prompt: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    if (lower.includes('publish') && lower.includes('npm')) {
      if (this.hasDeployInfo) {
        return { stdout: 'Set up npm publish workflow with dist-tag management. Publishes to alpha and latest tags. Both @ruflo/cli and codex umbrella package are updated. Tags verified after publish.', stderr: '', exitCode: 0 };
      }
...
describe('abBenchmark', () => {
  ...
  describe('A/B execution with differential executor', () => {
    it('returns a complete ABReport', async () => {
      const report = await abBenchmark(WELL_STRUCTURED_CODEX_MD, {
  ...
  describe('category shift detection', () => {
    it('detects category shift when B beats A by ≥0.2 across ≥3 classes', async () => {
      ...
      // With the differential executor and well-structured AGENTS.md,
  ...
  describe('poor AGENTS.md shows minimal improvement', () => {
    it('poor guidance still shows some improvement over no guidance', async () => {
      const report = await abBenchmark(POOR_CODEX_MD, {
        executor: new ABDifferentialExecutor(),
      });
    ...
    it('poor guidance has lower composite than well-structured guidance', async () => {
      const poor = await abBenchmark(POOR_CODEX_MD, {
        executor: new ABDifferentialExecutor(),
      });
      const good = await abBenchmark(WELL_STRUCTURED_CODEX_MD, {
        executor: new ABDifferentialExecutor(),
      });
