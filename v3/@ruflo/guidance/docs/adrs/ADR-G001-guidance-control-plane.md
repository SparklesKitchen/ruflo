# ADR-G001: Guidance Control Plane -- A Parallel Enforcement Layer Beside Codex
...
Codex loads `AGENTS.md` into its context window as a system prompt at session start. This mechanism has three fundamental limitations:

1. **No enforcement.** Rules in `AGENTS.md` are advisory. The model can forget, ignore, or misinterpret them at any point during a long session. There is no gate that blocks a tool call when a rule is violated.

2. **No retrieval.** Every rule in `AGENTS.md` consumes tokens on every turn, regardless of whether the current task needs it. A 2,000-line guidance file wastes context on irrelevant rules while still missing edge cases because the model's attention degrades over long documents.
...
The existing Ruflo V3 infrastructure provides hooks (`pre-edit`, `pre-command`, `post-task`), a memory subsystem (AgentDB + HNSW), and a headless execution mode (`codex -p --output-format json`). A control plane can leverage all three without modifying Codex itself.

## Decision

Build a separate package, `@ruflo/guidance`, that runs **beside** Codex as a parallel control plane. The control plane has five components:

1. **Compiler** (`GuidanceCompiler` in `src/compiler.ts`) -- Parses `AGENTS.md` and optional `.codex/AGENTS.override.md` into a `PolicyBundle` containing a constitution, rule shards, and a machine-readable manifest. The compiler extracts rule IDs, risk classes, tool classes, intent tags, repo scopes, domain tags, verifiers, and priority annotations using deterministic regex patterns.
...
- **Operational surface.** The control plane is a separate process/module that must be initialized, configured, and kept in sync with `AGENTS.md` changes.
...
### 1. Improve AGENTS.md formatting only
...
Train guidance into model weights. Rejected because fine-tuning is slow (days), expensive, inflexible (cannot change rules without retraining), and unavailable for Codex models via the public API.

### 3. Build enforcement inside Codex via monkey-patching
Intercept tool calls within the Codex process. Rejected because it couples to Codex internals, breaks on updates, and is fragile. A parallel system is decoupled and version-independent.
...
- `v3/@ruflo/guidance/src/index.ts` -- `GuidanceControlPlane` orchestrator class
- `v3/@ruflo/guidance/src/compiler.ts` -- `GuidanceCompiler`
- `v3/@ruflo/guidance/src/retriever.ts` -- `ShardRetriever`
- `v3/@ruflo/guidance/src/gates.ts` -- `EnforcementGates`
- `v3/@ruflo/guidance/src/ledger.ts` -- `RunLedger`
- `v3/@ruflo/guidance/src/optimizer.ts` -- `OptimizerLoop`
- `v3/@ruflo/guidance/src/headless.ts` -- `HeadlessRunner`
- `v3/@ruflo/guidance/src/types.ts` -- All type definitions
