...
The promotion policy is the critical decision: when does a rule change earn the right to be promoted from `local` (.codex/AGENTS.override.md overlay) to `root` (AGENTS.md constitution)?
...
- **Week 1:** Violation observed, new rule proposed and added to .codex/AGENTS.override.md.
...
- **Slow reaction.** A critical new rule takes 2-3 weeks to reach root. If a new vulnerability pattern emerges, the rule must wait. Mitigation: teams can manually add critical rules directly to AGENTS.md, bypassing the optimizer.
...
- `v3/@ruflo/guidance/src/optimizer.ts` -- `OptimizerLoop.runCycle()`, `proposeChanges()`, `evaluateChange()`, `applyPromotions()`, `promotionTracker`
- `v3/@ruflo/guidance/src/types.ts` -- `RuleChange`, `ABTestResult`, `OptimizationMetrics`, `ViolationRanking`, `RuleADR`
- `v3/@ruflo/guidance/src/ledger.ts` -- `RunLedger.rankViolations()`, `computeMetrics()`
- `v3/@ruflo/guidance/src/index.ts` -- `GuidanceControlPlane.optimize()`
