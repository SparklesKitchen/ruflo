# Getting Started with @ruflo/guidance

## Background: How Codex Uses Memory Files

In Codex, `AGENTS.md` and `.codex/AGENTS.override.md` are loaded into the agent's working context at session start. They are the primary mechanism for telling Codex how to behave on your project:

| File | Who it's for | Committed? |
|------|-------------|-----------|
| `AGENTS.md` | The whole team | Yes |
| `.codex/AGENTS.override.md` | You, on this machine | No (auto-gitignored) |

Codex searches **upward** from the current directory and loads every instance it finds. In a monorepo, a child directory's `AGENTS.md` layers on top of the root's. Run `/memory` in Codex to see which files were loaded.

The `@import` pattern offers an alternative to `.codex/AGENTS.override.md` for developers using multiple git worktrees:
...
The Guidance Control Plane takes these plain-text files and turns them into structured, enforceable, auditable policy. Without it, Codex loads `AGENTS.md` as raw text and relies on the model to follow it. With it, rules become typed objects with gates that block violations before they execute.

## Installation

```bash
npm install @ruflo/guidance@v3alpha
```
...
```ts
import { createGuidanceControlPlane } from '@ruflo/guidance';

const plane = createGuidanceControlPlane({
  rootGuidancePath: './AGENTS.md',
  localGuidancePath: './.codex/AGENTS.override.md', // optional, your personal overrides
});
...
1. **Load** — `AGENTS.md` is read as the root guidance. `.codex/AGENTS.override.md` (if present) is read as an overlay. Local rules supplement or override root rules.
...
import { createTrustSystem } from '@ruflo/guidance/trust';
...
## AGENTS.md vs. .codex/AGENTS.override.md

### What goes in AGENTS.md (shared, committed)
...
### What goes in .codex/AGENTS.override.md (private, not committed)
...
If you use multiple git worktrees, `.codex/AGENTS.override.md` is awkward because each worktree needs its own copy. The `@` import pattern inside `AGENTS.md` points to a shared file in your home directory:
...
The optimizer watches which `.codex/AGENTS.override.md` experiments reduce violations. When a local rule consistently outperforms the root, the optimizer proposes promoting it to `AGENTS.md` with an ADR:
...
```ts
import { getKernel, isWasmAvailable } from '@ruflo/guidance/wasm-kernel';
...
@ruflo/guidance/
  src/
    ...
    compiler.ts         # AGENTS.md → PolicyBundle
...
1. Add a unique rule to `AGENTS.md`: `# Test: Always respond in English`
2. Add a different rule to `.codex/AGENTS.override.md`: `# Test: Prefer bullet points`
3. Start Codex and run `/memory` — both files should appear as loaded
4. Ask Codex to restate each unique rule to confirm both were applied
...
const plane = createGuidanceControlPlane({
  rootGuidancePath: './AGENTS.md',
  localGuidancePath: './.codex/AGENTS.override.md',
});
await plane.initialize();
