...
## Starting Point: AGENTS.md as Memory

In Codex, `AGENTS.md` and `.codex/AGENTS.override.md` are loaded into the agent's working context as raw text. The agent reads them and tries to follow them. This works for simple rules but breaks down at scale:
...
```mermaid
graph TD
    CWD["Working directory<br/>/repo/packages/api"] -->|search upward| P1["./AGENTS.md"]
    CWD -->|search upward| P2["./.codex/AGENTS.override.md"]
    P1 -->|parent| P3["/repo/packages/AGENTS.md"]
    P3 -->|parent| P4["/repo/AGENTS.md"]
    P4 -->|"@ import"| P5["~/.codex/my_instructions.md"]
...
Codex searches upward from the current directory. Each level can add or override rules. `.codex/AGENTS.override.md` overlays at the same level. The `@import` pattern pulls in files from outside the repo.
...
| Module | Purpose |
|--------|---------|
| **GuidanceCompiler** | Parses AGENTS.md into PolicyBundle |
...
| **HeadlessRunner** | Automated compliance testing via `codex -p` |
...
graph LR
    subgraph "Compile Time (once per session)"
        MD["AGENTS.md<br/>(team rules)"] --> C[Compiler]
        LMD[".codex/AGENTS.override.md<br/>(your overrides)"] --> C
        C --> PB[PolicyBundle]
...
The key lifecycle: `AGENTS.md` is compiled once at session start. Rules are retrieved per task. The optimizer watches run outcomes and proposes changes back to `AGENTS.md`, generating an ADR for each change. `.codex/AGENTS.override.md` experiments that consistently reduce violations get promoted to the shared root.
