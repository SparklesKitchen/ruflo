...
You've written 50 rules in `AGENTS.md`. How do you know agents actually follow them? Manual testing is slow and inconsistent. The headless runner automates this: it runs Codex in headless mode against test tasks and evaluates the results.

## Step 1: Define Test Tasks

```ts
import { createComplianceSuite } from '@ruflo/guidance/headless';
import type { TestTask } from '@ruflo/guidance/headless';
...
```ts
import { createHeadlessRunner } from '@ruflo/guidance/headless';
import { createLedger } from '@ruflo/guidance/ledger';
...
```ts
import { createOptimizer } from '@ruflo/guidance/optimizer';
...
```ts
import { createPersistentLedger, createEventStore } from '@ruflo/guidance/persistence';

const store = createEventStore({ dataDir: './.codex/guidance/events' });
...
// Event is now on disk at .codex/guidance/events/
