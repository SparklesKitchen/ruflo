...
All exports from `@ruflo/guidance`. Each module is also available as a standalone import.

## Import Map

| Import Path | Key Exports |
|-------------|-------------|
| `@ruflo/guidance` | `GuidanceControlPlane`, `createGuidanceControlPlane` |
| `@ruflo/guidance/compiler` | `GuidanceCompiler`, `createCompiler` |
| `@ruflo/guidance/retriever` | `ShardRetriever`, `createRetriever`, `HashEmbeddingProvider` |
| `@ruflo/guidance/gates` | `EnforcementGates`, `createGates` |
| `@ruflo/guidance/hooks` | `GuidanceHookProvider`, `createGuidanceHooks` |
| `@ruflo/guidance/ledger` | `RunLedger`, `createLedger`, `TestsPassEvaluator`, `ForbiddenCommandEvaluator`, `ForbiddenDependencyEvaluator`, `ViolationRateEvaluator`, `DiffQualityEvaluator` |
| `@ruflo/guidance/optimizer` | `OptimizerLoop`, `createOptimizer` |
| `@ruflo/guidance/persistence` | `PersistentLedger`, `EventStore`, `createPersistentLedger`, `createEventStore` |
| `@ruflo/guidance/headless` | `HeadlessRunner`, `createHeadlessRunner`, `createComplianceSuite` |
| `@ruflo/guidance/gateway` | `DeterministicToolGateway`, `createToolGateway` |
| `@ruflo/guidance/artifacts` | `ArtifactLedger`, `createArtifactLedger` |
| `@ruflo/guidance/evolution` | `EvolutionPipeline`, `createEvolutionPipeline` |
| `@ruflo/guidance/manifest-validator` | `ManifestValidator`, `ConformanceSuite`, `createManifestValidator`, `createConformanceSuite` |
| `@ruflo/guidance/proof` | `ProofChain`, `createProofChain` |
| `@ruflo/guidance/memory-gate` | `MemoryWriteGate`, `createMemoryWriteGate`, `createMemoryEntry` |
| `@ruflo/guidance/coherence` | `CoherenceScheduler`, `EconomicGovernor`, `createCoherenceScheduler`, `createEconomicGovernor` |
| `@ruflo/guidance/capabilities` | `CapabilityAlgebra`, `createCapabilityAlgebra` |
| `@ruflo/guidance/conformance-kit` | `SimulatedRuntime`, `MemoryClerkCell`, `ConformanceRunner`, `createMemoryClerkCell`, `createConformanceRunner` |
| `@ruflo/guidance/ruvbot-integration` | `RuvBotGuidanceBridge`, `AIDefenceGate`, `RuvBotMemoryAdapter`, `createRuvBotBridge`, `createAIDefenceGate`, `createRuvBotMemoryAdapter` |
| `@ruflo/guidance/meta-governance` | `MetaGovernor`, `createMetaGovernor` |
| `@ruflo/guidance/adversarial` | `ThreatDetector`, `CollusionDetector`, `MemoryQuorum`, `createThreatDetector`, `createCollusionDetector`, `createMemoryQuorum` |
| `@ruflo/guidance/trust` | `TrustAccumulator`, `TrustScoreLedger`, `TrustSystem`, `getTrustBasedRateLimit`, `createTrustAccumulator`, `createTrustSystem` |
| `@ruflo/guidance/truth-anchors` | `TruthAnchorStore`, `TruthResolver`, `createTruthAnchorStore`, `createTruthResolver` |
| `@ruflo/guidance/uncertainty` | `UncertaintyLedger`, `UncertaintyAggregator`, `createUncertaintyLedger`, `createUncertaintyAggregator` |
| `@ruflo/guidance/temporal` | `TemporalStore`, `TemporalReasoner`, `createTemporalStore`, `createTemporalReasoner` |
| `@ruflo/guidance/authority` | `AuthorityGate`, `IrreversibilityClassifier`, `createAuthorityGate`, `createIrreversibilityClassifier`, `isHigherAuthority`, `getAuthorityHierarchy` |
| `@ruflo/guidance/continue-gate` | `ContinueGate`, `createContinueGate` |
| `@ruflo/guidance/wasm-kernel` | `getKernel`, `isWasmAvailable`, `resetKernel` |
| `@ruflo/guidance/generators` | `generateCodexMd`, `generateCodexLocalMd`, `generateSkillMd`, `generateAgentMd`, `generateAgentIndex`, `scaffold` |
| `@ruflo/guidance/analyzer` | `analyze`, `benchmark`, `autoOptimize`, `optimizeForSize`, `headlessBenchmark`, `validateEffect`, `abBenchmark`, `getDefaultABTasks`, `formatReport`, `formatBenchmark` |
...
|--------|---------|-------------|
| `initialize()` | `Promise<void>` | Read AGENTS.md, compile, load shards, activate gates |
...
```ts
import { generateCodexMd, scaffold } from '@ruflo/guidance/generators';
```

| Function | Returns | Description |
|----------|---------|-------------|
| `generateCodexMd(profile)` | `string` | Generate AGENTS.md from a `ProjectProfile` |
| `generateCodexLocalMd(profile)` | `string` | Generate .codex/AGENTS.override.md from a `LocalProfile` |
...
```ts
import { analyze, validateEffect } from '@ruflo/guidance/analyzer';
```
...
| `benchmark(before, after, local?)` | `BenchmarkResult` | Compare two AGENTS.md versions |
...
| Function | Returns | Description |
|----------|---------|-------------|
| `headlessBenchmark(original, optimized, options?)` | `Promise<HeadlessBenchmarkResult>` | Run compliance tasks via `codex -p` |
...
**IContentAwareExecutor**: Extends `IHeadlessExecutor` with `setContext(codexMdContent)` — called before each phase so the executor can vary behavior based on guidance quality.
...
| Function | Returns | Description |
|----------|---------|-------------|
| `abBenchmark(codexMd, options?)` | `Promise<ABReport>` | Run 20 tasks under Config A (no guidance) vs Config B (with guidance) |
