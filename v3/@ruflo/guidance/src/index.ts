/**
 * @ruflo/guidance - Guidance Control Plane
 *
 * Sits beside Codex (not inside it) to:
 * 1. Compile AGENTS.md into constitution + shards + manifest
 ...
 *
 * Architecture:
 * - Root AGENTS.md → Repo constitution (rare changes)
 * - .codex/AGENTS.override.md → Overlay / experiment sandbox (frequent changes)
 * - Optimizer → Promotes winning local rules to root
 *
 * Integration with Codex:
 * - Headless mode (codex -p --output-format json) for automated testing
 * - Hook system for enforcement gates
 * - RuVector/HNSW for semantic shard retrieval
 *
 * @module @ruflo/guidance
 */
