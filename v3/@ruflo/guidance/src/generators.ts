 ...
 * AGENTS.md Generator
 *
 * Generates a structured AGENTS.md file optimized for the Guidance Control Plane.
 ...
 *
 * @module @ruflo/guidance/generators
 */
...
export function generateCodexMd(profile: ProjectProfile): string {
  ...
  sections.push('');
  if (profile.monorepo) {
    sections.push('This is a monorepo. Each package has its own AGENTS.md that layers on top of this root file.');
  }
  if (profile.srcDir) {
  ...
  if (profile.guidanceControlPlane) {
    ...
    sections.push('');
    sections.push('This project uses `@ruflo/guidance` to enforce these rules programmatically.');
    ...
    sections.push('The optimizer watches violations and promotes winning .codex/AGENTS.override.md experiments here.');
    sections.push('');
...
export function generateCodexLocalMd(local: LocalProfile): string {
  const sections: string[] = [];

  sections.push('# Local Development Notes');
  sections.push('');
  sections.push('> This file is auto-gitignored by Codex. It stays on this machine only.');
  sections.push('');
...
// ============================================================================
// Scaffold Generator (creates full .codex/ directory structure)
// ============================================================================

export interface ScaffoldOptions {
  /** Project profile for AGENTS.md */
  project: ProjectProfile;
  /** Local profile for .codex/AGENTS.override.md (optional) */
  local?: LocalProfile;
  /** Skills to generate */
...
export function scaffold(options: ScaffoldOptions): ScaffoldResult {
  ...
  // AGENTS.md
  files.set('AGENTS.md', generateCodexMd(options.project));

  // .codex/AGENTS.override.md
  if (options.local) {
    files.set('.codex/AGENTS.override.md', generateCodexLocalMd(options.local));
  }
  ...
  for (const agent of agents) {
    ...
    const path = `.codex/agents/${category}/${agent.name}.md`;
    files.set(path, generateAgentMd(agent));
  }

  // Generate agent index
  if (agents.length > 0) {
    files.set('.codex/agents/index.yaml', generateAgentIndex(agents));
  }

  // Generate skill files
  for (const skill of skills) {
    const path = `.codex/skills/${skill.name}/SKILL.md`;
  ...
  if (profile.guidanceControlPlane) {
    skills.push({
      ...
      instructions: [
        ...
        '',
        'This project uses `@ruflo/guidance` to enforce AGENTS.md rules programmatically.',
        '',
