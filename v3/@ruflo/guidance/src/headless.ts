 ...
 *
 * Integrates with Codex headless mode (codex -p --output-format json)
 ...
 *
 * @module @ruflo/guidance/headless
 */
...
/**
 * Parsed output from Codex headless mode
 */
...
export class ProcessExecutor implements ICommandExecutor {
  async execute(command: string, timeoutMs: number): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    // Commands follow the pattern: codex -p '<prompt>' --output-format json
  ...
  private parseCommand(command: string): string[] {
    // Extract prompt from: codex -p '<prompt>' --output-format json 2>/dev/null
    const match = command.match(/^codex\s+-p\s+'((?:[^']|'\\'')*?)'\s+--output-format\s+json/);
    if (match) {
...
export class HeadlessRunner {
  ...
  /**
   * Build the Codex headless command
   */
  private buildCommand(task: TestTask): string {
    // Escape the prompt for shell safety
    const escapedPrompt = task.prompt.replace(/'/g, "'\\''");
    return `codex -p '${escapedPrompt}' --output-format json 2>/dev/null`;
  }
