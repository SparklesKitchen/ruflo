/**
 * AGENTS.md Analyzer & Auto-Optimizer
 *
 * Quantifiable, verifiable analysis of AGENTS.md files.
 ...
 *
 * @module @ruflo/guidance/analyzer
 */
...
/** Result of headless benchmark via codex -p */
...
/**
 * Analyze a AGENTS.md file and produce quantifiable scores.
 *
...
/**
 * Auto-optimize a AGENTS.md file by applying high-priority suggestions.
 ...
 *
 * @param content - AGENTS.md content
...
/**
 * Run a headless benchmark using `codex -p` to measure actual agent
 * compliance before and after optimization.
 *
 * Requires `codex` CLI to be installed. Uses the proof chain to create
 * tamper-evident records of each test run.
 *
 * @param originalContent - Original AGENTS.md
 * @param optimizedContent - Optimized AGENTS.md
...
/** Executor interface for headless codex commands */
...
/**
 * Content-aware executor that adapts behavior based on AGENTS.md content.
 *
 ...
 * based on the quality of the loaded AGENTS.md. This is the key mechanism
...
export interface IContentAwareExecutor extends IHeadlessExecutor {
  /** Set the AGENTS.md content that the executor should use as behavioral context */
...
class DefaultHeadlessExecutor implements IContentAwareExecutor {
  ...
  setContext(codexMdContent: string): void {
    this.contextContent = codexMdContent;
  }
  ...
  async execute(prompt: string, workDir: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    ...
    const codexMdPath = join(workDir, 'AGENTS.md');
    const backupPath = join(workDir, '.AGENTS.md.ab-backup');
    let swapped = false;

    if (this.contextContent !== null) {
      try { await fs.copyFile(codexMdPath, backupPath); } catch { /* no file to back up */ }

      if (this.contextContent.length > 0) {
        await fs.writeFile(codexMdPath, this.contextContent, 'utf-8');
      } else {
        await fs.unlink(codexMdPath).catch(() => {});
      }
      swapped = true;
    }
    ...
    try {
      ...
      if (swapped) {
        try {
          await fs.copyFile(backupPath, codexMdPath);
...
function formatHeadlessBenchmarkReport(result: HeadlessBenchmarkResult): string {
  const lines: string[] = [];
  lines.push('Headless Codex Benchmark (codex -p)');
  lines.push('======================================');
  lines.push('');
...
export function formatReport(result: AnalysisResult): string {
  ...
  lines.push(`AGENTS.md Analysis Report`);
  lines.push(`========================`);
  lines.push(``);
...
/**
 * A single validation run against one AGENTS.md version.
 */
export interface ValidationRun {
  /** Analysis of the AGENTS.md used */
  analysis: AnalysisResult;
  /** Per-task results */
...
export interface ValidationReport {
  /** Run against original AGENTS.md */
  before: ValidationRun;
  /** Run against optimized AGENTS.md */
  after: ValidationRun;
 ...
 * AGENTS.md, then computes statistical correlations between per-dimension
 ...
 * `setContext()` is called before each phase with the corresponding AGENTS.md
 ...
 *
 * @param originalContent - Original AGENTS.md content
 * @param optimizedContent - Optimized AGENTS.md content
...
export async function validateEffect(
  ...
  if (chain) {
    const event: RunEvent = {
      ...
      retrievedRuleIds: [],
      toolsUsed: ['codex -p', 'analyzer.validateEffect'],
      filesTouched: ['AGENTS.md'],
      diffSummary: { linesAdded: 0, linesRemoved: 0, filesChanged: 0 },
      testResults: {
        ran: true,
...
/** A single benchmark task representing a real Ruflo scenario */
...
// ── 20 Representative Tasks from Ruflo History ───────────────────────
...
function getABTasks(): ABTask[] {
  ...
  return [
    ...
    // ── Deployment (2 tasks) ──────────────────────────────────────────
    {
      ...
      prompt: 'Create a multi-stage Dockerfile for the Ruflo CLI. Include a build stage and a minimal runtime stage. Never include dev dependencies in production.',
      assertions: [
 ...
 * **Config B** (treatment): With guidance — executor gets setContext(codexMd) +
 *   gate simulation on every output
 *
 * The 20 tasks span 7 task classes drawn from real Ruflo repo history:
 ...
 * = "category shift"
 *
 * @param codexMdContent - The AGENTS.md content used for Config B
...
export async function abBenchmark(
  ...
  // #1652: a non-content-aware executor reads AGENTS.md from disk for both
  // configs, so the delta is architecturally guaranteed to be zero — yet
  // the verdict implies the user's AGENTS.md is ineffective. Detect and
  ...
  if (!contentAware) {
    throw new Error(
      'abBenchmark requires a content-aware executor. The provided IHeadlessExecutor lacks `setContext()`, so Config A and Config B will both read the same on-disk AGENTS.md and the delta is guaranteed to be zero. Either use the DefaultHeadlessExecutor (content-aware as of @ruflo/guidance@3.0.0-alpha.2) or implement IContentAwareExecutor on your custom executor.',
    );
  }
  ...
  if (contentAware) executor.setContext(codexMdContent);
  ...
  if (proofKey) {
    ...
    const event: RunEvent = {
      ...
      intent: 'testing' as TaskIntent,
      guidanceHash: createHash('sha256').update(codexMdContent).digest('hex').slice(0, 16),
      retrievedRuleIds: [],
