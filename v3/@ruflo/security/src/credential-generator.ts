...
export class CredentialGenerator {
  ...
  createEnvScript(credentials: GeneratedCredentials): string {
    return `# Ruflo V3 - Generated Credentials
# Generated: ${credentials.generatedAt.toISOString()}
# IMPORTANT: Store these securely and delete this file after use

export RUFLO_ADMIN_PASSWORD="${credentials.adminPassword}"
export RUFLO_SERVICE_PASSWORD="${credentials.servicePassword}"
export RUFLO_JWT_SECRET="${credentials.jwtSecret}"
export RUFLO_SESSION_SECRET="${credentials.sessionSecret}"
export RUFLO_ENCRYPTION_KEY="${credentials.encryptionKey}"
`;
  }
  ...
  createJsonConfig(credentials: GeneratedCredentials): string {
    return JSON.stringify({
      'codex/admin-password': credentials.adminPassword,
      'codex/service-password': credentials.servicePassword,
      'codex/jwt-secret': credentials.jwtSecret,
      'codex/session-secret': credentials.sessionSecret,
      'codex/encryption-key': credentials.encryptionKey,
      'codex/generated-at': credentials.generatedAt.toISOString(),
      'codex/expires-at': credentials.expiresAt?.toISOString() ?? null,
    }, null, 2);
  }
}
