...
describe('CredentialGenerator', () => {
  ...
  describe('Environment Script Generation', () => {
    it('should generate valid shell script', () => {
      ...
      expect(script).toContain('export RUFLO_ADMIN_PASSWORD=');
      expect(script).toContain('export RUFLO_SERVICE_PASSWORD=');
      expect(script).toContain('export RUFLO_JWT_SECRET=');
      expect(script).toContain('export RUFLO_SESSION_SECRET=');
      expect(script).toContain('export RUFLO_ENCRYPTION_KEY=');
    });
  ...
  describe('JSON Config Generation', () => {
    ...
    it('should include all credentials', () => {
      ...
      expect(parsed['codex/admin-password']).toBe(credentials.adminPassword);
      expect(parsed['codex/service-password']).toBe(credentials.servicePassword);
      expect(parsed['codex/jwt-secret']).toBe(credentials.jwtSecret);
      expect(parsed['codex/session-secret']).toBe(credentials.sessionSecret);
      expect(parsed['codex/encryption-key']).toBe(credentials.encryptionKey);
    });
  });
