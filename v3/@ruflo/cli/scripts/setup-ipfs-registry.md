...
```bash
# Create new project
gcloud projects create codex-registry --name="Ruflo Registry"

# Set as active project
gcloud config set project codex-registry

# Enable billing (required for Cloud Functions)
gcloud beta billing projects link codex-registry \
...
gcloud storage buckets create gs://codex-plugin-registry \
  --location=US \
  --uniform-bucket-level-access

# Enable versioning for rollback
gsutil versioning set on gs://codex-plugin-registry

# Make registry.json publicly readable (optional)
gsutil iam ch allUsers:objectViewer gs://codex-plugin-registry
```
...
# Grant permissions
gcloud projects add-iam-policy-binding codex-registry \
  --member="serviceAccount:plugin-registry-publisher@codex-registry.iam.gserviceaccount.com" \
...
async function getSecret(name) {
  const [version] = await secretManager.accessSecretVersion({
    name: `projects/codex-registry/secrets/${name}/versions/latest`,
  });
...
export async function publishRegistry(req, res) {
  try {
    ...
    const bucket = storage.bucket('codex-plugin-registry');
...
gcloud functions deploy publish-registry \
  ...
  --service-account=plugin-registry-publisher@codex-registry.iam.gserviceaccount.com \
  --set-env-vars=GCP_PROJECT=codex-registry
```
...
# Upload to GCS
gsutil cp registry.json gs://codex-plugin-registry/registry.json
```
...
steps:
  ...
  - name: 'gcr.io/cloud-builders/gsutil'
    args: ['cp', 'registry.json', 'gs://codex-plugin-registry/registry.json']
...
substitutions:
  _FUNCTION_URL: 'https://us-central1-codex-registry.cloudfunctions.net/publish-registry'
...
gcloud scheduler jobs create http publish-registry-daily \
  ...
  --schedule="0 2 * * *" \
  --uri="https://us-central1-codex-registry.cloudfunctions.net/publish-registry" \
  --http-method=POST
```
