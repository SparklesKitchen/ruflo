...
async function generateRegistry(): Promise<PluginRegistry> {
  ...
  for (const pkg of officialPackages) {
    ...
    plugins.push({
      id: pkg,
      name: pkg,
      displayName: pkg.replace('@ruflo/plugin-', '').replace('@ruflo/', ''),
      description: `Official Ruflo plugin: ${pkg}`,
  ...
  return {
    ...
    plugins,
    categories: [
      { id: 'official', name: 'Official', description: 'Official Ruflo plugins', pluginCount: plugins.length },
    ],
...
async function main() {
  ...
  try {
    const result = await pinToIPFS(registry, 'codex-plugin-registry', jwt);
    ...
    console.log('   3. Test with: npx ruflo@latest plugins list');
  } catch (error) {
