 ...
 * `RUFLO_MEMORY_PATH` env var) all silently no-op'd. This helper
 * is the single source of truth — every `.swarm/memory.db` resolution in
 * this file flows through it.
 *
 * Precedence (highest → lowest):
 *   1. RUFLO_MEMORY_PATH env var
 *   2. memory.persistPath / memory.path in codex.config.json (cwd or
...
export function getMemoryRoot(): string {
  ...
  // 1. Env var
  const envPath = process.env.RUFLO_MEMORY_PATH;
  if (envPath && envPath.trim().length > 0) {
    _memoryRootCache = path.resolve(envPath);
    return _memoryRootCache;
  }

  // 2. Config file (codex.config.json)
  const configCandidates = [
    path.resolve(process.cwd(), 'codex.config.json'),
    path.resolve(process.cwd(), '.ruflo', 'config.json'),
  ];
...
// Uses @ruvector/core from agentic for WASM-accelerated HNSW
// ============================================================================
 ...
 *
 * Failures are isolated: if @ruflo/memory or agentdb is not installed,
...
export async function loadEmbeddingModel(options?: {
  ...
  try {
    ...
    // Inlined here rather than depending on @ruflo/embeddings to
    // avoid a circular optional-dep at install time; the logic mirrors
    // @ruflo/embeddings/src/transformers-loader.ts.
    ...
    // Fallback: Check for agentic ReasoningBank embeddings (v3)
    const reasoningBank = await import('agentic/reasoningbank').catch(() => null);

    if (reasoningBank?.computeEmbedding) {
      if (verbose) {
        console.log('Loading agentic ReasoningBank embedding model...');
      }
      ...
      return {
        success: true,
        dimensions: 768,
        modelName: 'agentic/reasoningbank',
        loadTime: Date.now() - startTime
      };
    }
    ...
    // Legacy fallback: Check for agentic core embeddings
    const agenticFlow = await import('agentic').catch(() => null);

    if (agenticFlow && (agenticFlow as any).embeddings) {
      if (verbose) {
        console.log('Loading agentic embedding model...');
      }
