 ...
 * Returns null if @ruflo/memory is not available.
 */
async function getRegistry(dbPath?: string): Promise<any | null> {
  ...
  if (!registryPromise) {
    registryPromise = (async () => {
      try {
        const { ControllerRegistry } = await import('@ruflo/memory');
...
export async function bridgeDeleteHierarchical(options: {
  ...
  try {
    ...
    //    See agentic#150/#151 (closes ruvnet/RuVector#427 the cli-visible way).
