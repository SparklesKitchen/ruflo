...
export class WorkerDaemon extends EventEmitter {
  ...
  constructor(projectRoot: string, config?: Partial<DaemonConfig>) {
    ...
    const codexFlowDir = join(projectRoot, '.ruflo');

    // Read daemon config from .ruflo/config.json (Layer B)
    const fileConfig = this.readDaemonConfigFromFile(codexFlowDir);
    ...
    this.config = {
      ...
      logDir: config?.logDir ?? join(codexFlowDir, 'logs'),
      stateFile: config?.stateFile ?? join(codexFlowDir, 'daemon-state.json'),
    ...
    if (!existsSync(codexFlowDir)) {
      mkdirSync(codexFlowDir, { recursive: true });
    }
   ...
   */
  private readDaemonConfigFromFile(codexFlowDir: string): {
    autoStart?: boolean;
    maxConcurrent?: number;
    workerTimeoutMs?: number;
    maxCpuLoad?: number;
    ...
    const jsonPath = join(codexFlowDir, 'config.json');
    const yamlPath = join(codexFlowDir, 'config.yaml');
    const ymlPath = join(codexFlowDir, 'config.yml');
  ...
  private async runMapWorker(): Promise<unknown> {
    ...
    const map = {
      ...
      structure: {
        ...
        hasCodexConfig: existsSync(join(this.projectRoot, '.codex')),
        hasCodexFlow: existsSync(join(this.projectRoot, '.ruflo')),
      },
      scannedAt: Date.now(),
    };
