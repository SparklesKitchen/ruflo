...
export const STATE_DIR = '.ruflo/data';
...
export function discoverTasks(sources: string[]): TaskInfo[] {
  ...
  for (const source of validSources) {
    if (source === 'team-tasks') {
      const tasksDir = join(homedir(), '.codex', 'tasks');
      try {
    ...
    if (source === 'swarm-tasks') {
      const swarmFile = resolve('.ruflo/swarm-tasks.json');
      try {
    ...
    if (source === 'file-checklist') {
      const checklistFile = resolve('.ruflo/data/checklist.json');
      try {
...
export async function tryLoadLearning(): Promise<{ initialize: () => Promise<boolean>; [key: string]: unknown } | null> {
  try {
    const modPath = 'agentic/dist/coordination/autopilot-learning.js';
