...
export class RvfaBuilder {
  ...
  async build(): Promise<BuildResult> {
    ...
    const stages: { id: SectionId; raw: Buffer; label: string }[] = [
      ...
      { id: 'runtime', raw: this.buildRuntimeSection(), label: 'Runtime (Node.js + Codex)' },
  ...
  private buildRuntimeSection(): Buffer {
    ...
    return jsonBuf({
      ...
      codexCode: { name: 'codex-code-cli', entrypoint: '/usr/local/bin/codex' },
      paths: { node: '/usr/local/bin/node', codex: '/usr/local/bin/codex' },
    });
  }
  ...
  private buildHeaderPartial(): Partial<RvfaHeader> {
    ...
    const boot: RvfaBootConfig = {
      ...
      env: { NODE_ENV: 'production', RUFLO_MEMORY_BACKEND: 'hybrid', RUFLO_LOG_LEVEL: 'info' },
