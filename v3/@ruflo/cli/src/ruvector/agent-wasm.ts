 ...
 *
 * @module @ruflo/cli/ruvector/agent-wasm
 */
...
export async function createWasmAgent(config: WasmAgentConfig = {}): Promise<WasmAgentInfo> {
  ...
  // #1810 — was hardcoded `openai:codex-sonnet-4-20250514`. Updated to
  ...
  const configJson = JSON.stringify({
    model: config.model ?? 'openai:codex-sonnet-4-6',
 ...
 * detect that stub output and route the prompt through OpenAI's
 ...
 *
 * If OPENAI_API_KEY is not set, returns the stub output verbatim so
...
export async function promptWasmAgent(agentId: string, input: string): Promise<string> {
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      ...
      return `${wasmResult}\n[NOTE: bundled WASM agent has no LLM; set OPENAI_API_KEY to enable real responses via OpenAI Messages API]`;
    }

    const { callOpenAIMessages, resolveOpenAIModel } = await import('../mcp-tools/agent-execute-core.js');
    const model = resolveOpenAIModel(entry.info.config.model);
    ...
    if (!result.success) {
      return `${wasmResult}\n[NOTE: bundled WASM agent has no LLM; OpenAI fallback failed: ${result.error}]`;
    }
