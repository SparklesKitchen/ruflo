...
import { CodexModel, getModelRouter, ModelRouter, ModelRoutingResult } from './model-router.js';
...
export class EnhancedModelRouter {
  ...
  private async tryAgentBooster(
    ...
    try {
      ...
      // Try local agentic agent-booster (v3 — no npx needed)
      // Note: agent-booster export declared but dist missing in alpha.1; use intelligence path as fallback
      const boosterModule = await import('agentic/agent-booster')
        .catch(() => import(/* @vite-ignore */ 'agentic/intelligence/agent-booster-enhanced'))
        .catch(() => null);
