 ...
 *
 * Dynamically routes requests to optimal Codex model (haiku/sonnet/opus)
...
/**
 * Available Codex models for routing
 */
export type CodexModel = 'haiku' | 'sonnet' | 'opus' | 'inherit';

/**
 * Model capabilities and characteristics
 */
export const MODEL_CAPABILITIES: Record<CodexModel, {
...
export interface ModelRoutingResult {
  ...
  alternatives: Array<{ model: CodexModel; score: number }>;
 ...
 */
const BANDIT_REWARDS: Record<CodexModel, Record<'success' | 'failure' | 'escalated', number>> = {
...
interface RouterState {
  ...
  modelDistribution: Record<CodexModel, number>;
   ...
   */
  priors?: Record<CodexModel, BetaPrior>;
}
 ...
 */
function defaultBanditPriors(): Record<CodexModel, BetaPrior> {
  return {
...
export class ModelRouter {
  ...
  private consecutiveFailures: Record<CodexModel, number> = {
    haiku: 0,
    sonnet: 0,
    opus: 0,
    inherit: 0,
  ...
  async route(task: string, embedding?: number[]): Promise<ModelRoutingResult> {
    ...
    const result: ModelRoutingResult = {
      ...
      alternatives: Object.entries(adjustedScores)
        .filter(([m]) => m !== model)
        .map(([m, score]) => ({ model: m as CodexModel, score }))
   ...
   */
  private computeModelScores(complexity: ComplexityAnalysis): Record<CodexModel, number> {
   ...
   * Apply circuit breaker adjustments
   */
  private applyCircuitBreaker(scores: Record<CodexModel, number>): Record<CodexModel, number> {
    if (!this.config.enableCircuitBreaker) {
      return scores;
    }

    const adjusted = { ...scores };
    for (const model of Object.keys(adjusted) as CodexModel[]) {
   ...
   */
  private selectModel(
    scores: Record<CodexModel, number>,
    complexityScore: number
  ): { model: CodexModel; confidence: number; uncertainty: number } {
    // Thompson sampling: combine deterministic score with bandit posterior
    const priors = this.state.priors ?? defaultBanditPriors();
    const sampledScores: Record<CodexModel, number> = {
    ...
    const sorted = (Object.entries(sampledScores) as [CodexModel, number][])
      .filter(([m]) => m !== 'inherit')
      .sort((a, b) => b[1] - a[1]);

    const [bestModel, bestScore] = sorted[0];
    const [, secondScore] = sorted[1] || ['sonnet' as CodexModel, 0];
  ...
  getStats(): {
    ...
    avgConfidence: number;
    circuitBreakerTrips: number;
    consecutiveFailures: Record<CodexModel, number>;
  } {
    return {
   ...
   */
  getBanditPriors(): Record<CodexModel, BetaPrior> {
 ...
 */
export async function routeToModel(task: string): Promise<CodexModel> {
