...
const DEFAULT_OPTIONS: Required<MCPServerOptions> = {
  ...
  host: 'localhost',
  port: 3000,
  pidFile: path.join(os.tmpdir(), 'codex-mcp.pid'),
  logFile: path.join(os.tmpdir(), 'codex-mcp.log'),
  tools: 'all',
  daemonize: false,
  timeout: 30000,
};
...
export class MCPServerManager extends EventEmitter {
  ...
  async getStatus(): Promise<MCPServerStatus> {
    ...
    if (!pid) {
      ...
      // (e.g., launched by Codex via `codex mcp add`).
      const isStdio = !process.stdin.isTTY;
      const envTransport = process.env.RUFLO_MCP_TRANSPORT;
  ...
  private async startStdioServer(): Promise<void> {
    ...
    console.error(
      `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Starting in stdio mode`
    );
    ...
    try {
      ...
      if (!status.initialized) {
        console.error(
          `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Auto-initializing memory database...`
        );
        const result = await initializeMemoryDatabase({ force: false, verbose: false });
        if (result.success) {
          console.error(
            `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Memory database initialized at ${result.dbPath}`
          );
        } else if (result.error && !result.error.includes('already exists')) {
          console.error(
            `[${new Date().toISOString()}] WARN [codex-mcp] (${sessionId}) Memory database init returned: ${result.error}`
          );
        }
      } else {
        console.error(
          `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Memory database already initialized (v${status.version || 'unknown'})`
        );
      }
    } catch (memInitError) {
      ...
      console.error(
        `[${new Date().toISOString()}] WARN [codex-mcp] (${sessionId}) Memory auto-init failed (tools will retry on first call): ${memInitError instanceof Error ? memInitError.message : String(memInitError)}`
      );
    }
    ...
    process.stdin.on('data', async (chunk) => {
      ...
      if (buffer.length > MAX_BUFFER_SIZE) {
        console.error(
          `[${new Date().toISOString()}] ERROR [codex-mcp] Buffer exceeded ${MAX_BUFFER_SIZE} bytes, rejecting`
        );
        buffer = '';
        console.log(JSON.stringify({
          jsonrpc: '2.0',
      ...
      for (const line of lines) {
        if (line.trim()) {
          try {
            ...
            console.error(
              `[${new Date().toISOString()}] ERROR [codex-mcp] Failed to parse message:`,
    ...
    process.stdin.on('end', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) stdin closed, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGINT', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Received SIGINT, shutting down...`
      );
      process.exit(0);
    });
    ...
    process.on('SIGTERM', () => {
      console.error(
        `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Received SIGTERM, shutting down...`
      );
      process.exit(0);
    });
    ...
    try {
      switch (message.method) {
        ...
        case 'notifications/initialized':
          ...
          console.error(
            `[${new Date().toISOString()}] INFO [codex-mcp] (${sessionId}) Client initialized`
          );
          return null;
      ...
      console.error(
        `[${new Date().toISOString()}] ERROR [codex-mcp] Error handling ${message.method}:`,
        error
      );
      return {
        jsonrpc: '2.0',
  ...
  private async startHttpServer(): Promise<void> {
    ...
    const { createMCPServer } = await import('@ruflo/mcp');
  ...
  private async removePidFile(): Promise<void> {
    ...
    try {
      const legacyPath = path.join(process.env.RUFLO_CWD || process.cwd(), '.codex', 'mcp-server.pid');
  ...
  /**
   * Check if process is running AND is a node/codex process.
  ...
  private isProcessRunning(pid: number): boolean {
    ...
    try {
      ...
      return cmdline.includes('node') || cmdline.includes('codex') || cmdline.includes('npx');
    } catch {
