...
export const browserSessionTools: MCPTool[] = [
  ...
  // ==========================================================================
  {
    ...
    handler: async (input) => {
      ...
      // 3. AgentDB index — best-effort via memory store (codex bridges)
      ...
      const idx = await shell('npx', ['-y', '@ruflo/cli@latest', 'memory', 'store',
  ...
  // ==========================================================================
  {
    ...
    handler: async (input) => {
      ...
      const r = await shell('npx', ['-y', '@ruflo/cli@latest', 'memory', 'retrieve',
