 ...
 * - Uses @ruflo/embeddings for REAL ML embeddings when available
 ...
 *
 * Note: For production neural features, use @ruflo/neural module
 */
...
// Try to import real embeddings — prefer agentic v3 ReasoningBank, then @ruflo/embeddings
let realEmbeddings: { embed: (text: string) => Promise<number[]> } | null = null;
let embeddingServiceName: string = 'none';
try {
  // Tier 1: agentic v3 ReasoningBank (fastest — WASM-accelerated)
  ...
  if (rb?.computeEmbedding) {
    ...
    embeddingServiceName = 'agentic/reasoningbank';
  }

  // Tier 2: @ruflo/embeddings with agentic provider
  if (!realEmbeddings) {
    ...
    if (embeddingsModule?.createEmbeddingService) {
      try {
        ...
        embeddingServiceName = 'agentic';
      } catch {
        // agentic provider not available, try ONNX
      }
    }
  }

  // Tier 3: @ruflo/embeddings with ONNX provider
  if (!realEmbeddings) {
    const embeddingsModule = await import('@ruflo/embeddings').catch(() => null);
    if (embeddingsModule?.createEmbeddingService) {
      try {
  ...
  // No Tier 4 mock fallback. If Tier 1 (agentic) and Tier 3 (onnx)
    ...
    handler: async (input) => {
      ...
      return {
        ...
        ...(embeddingServiceName === 'hash-fallback' || embeddingServiceName === 'none' ? {
          platformNote: 'ONNX embeddings not available — using hash-based fallback. Install @ruflo/embeddings and run "embeddings init --download" for semantic search.',
        } : {}),
      };
    },
  },
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      return {
        ...
        embeddingProvider: realEmbeddings ? `@ruflo/embeddings (${embeddingServiceName})` : 'hash-based (deterministic)',
        models: {
          total: models.length,
        ...
        features: {
          ...
          flashAttention: await (async () => {
            try {
              // #1773 item 4 — flash-attention now lives in @ruflo/neural
              const { getFlashAttention } = await import('@ruflo/neural');
