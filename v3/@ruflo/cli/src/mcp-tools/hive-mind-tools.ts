...
function loadAgentStore(): { agents: Record<string, unknown> } {
  const storePath = join(getProjectCwd(), '.codex', 'agents.json');
  try {
...
function saveAgentStore(store: { agents: Record<string, unknown> }): void {
  const storeDir = join(getProjectCwd(), '.codex');
...
export const hiveMindTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      const taskStorePath = join(getProjectCwd(), '.codex', 'tasks', 'store.json');
