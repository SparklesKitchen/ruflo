...
type AIDefenceInstance = ReturnType<typeof import('@ruflo/aidefence').createAIDefence>;
...
async function getAIDefence(): Promise<AIDefenceInstance> {
  ...
  const packageName = '@ruflo/aidefence';
  ...
  if (installAttempted) {
    throw new Error('AIDefence package not available. Install with: npm install @ruflo/aidefence');
  }
  installAttempted = true;

  // Second attempt - auto-install and retry
  console.error(`[codex] ${packageName} not found, attempting auto-install...`);
  ...
  try {
    ...
    if (instance) {
      aidefenceInstance = instance;
      console.error(`[codex] ${packageName} loaded after install (resolver path)`);
      return instance;
    }
  ...
  try {
    ...
    aidefenceInstance = instance;
    console.error(`[codex] ${packageName} loaded after install (file:// path)`);
    return instance;
  } catch (retryError) {
    throw new Error(
      ...
      `(common with global installs of \`codex\`). Recovery options:\n` +
      `  1. Run \`npm install --save @ruflo/aidefence\` in your project's working directory.\n` +
...
const aidefenceIsSafeTool: MCPTool = {
  ...
  handler: async (args: Record<string, unknown>): Promise<MCPToolResult> => {
    ...
    try {
      ...
      const defender = await getAIDefence();
      const { isSafe } = await import('@ruflo/aidefence');
