 ...
 * to dispatch a prompt to an agent's configured OpenAI model. This
...
const STORAGE_DIR = '.codex';
const AGENT_DIR = 'agents';
const AGENT_FILE = 'store.json';

type CodexModel = 'haiku' | 'sonnet' | 'opus' | 'inherit';
...
const MODEL_MAP: Record<string, string> = {
  haiku: 'codex-3-5-haiku-latest',
  sonnet: 'codex-3-5-sonnet-latest',
  opus: 'codex-3-opus-latest',
  inherit: 'codex-3-5-sonnet-latest',
};

export interface OpenAICallInput {
  prompt: string;
  systemPrompt?: string;
  model?: string;          // already-resolved OpenAI model id (e.g. 'codex-3-5-sonnet-latest')
  maxTokens?: number;
  temperature?: number;
  timeoutMs?: number;
}
...
/**
 * Generic OpenAI Messages API call. No agent registry coupling — used
 ...
 * OPENAI_API_KEY is unset and OLLAMA_API_KEY is present, or when
 * RUFLO_PROVIDER=ollama is explicitly set. Response shape is normalized
 * to the OpenAI-flavored OpenAICallResult so existing callers
 * don't need to know which provider answered.
 */
export async function callOpenAIMessages(input: OpenAICallInput): Promise<OpenAICallResult> {
  const explicitProvider = (process.env.RUFLO_PROVIDER || '').toLowerCase();
  const ollamaKey = process.env.OLLAMA_API_KEY;
  const openaiKey = process.env.OPENAI_API_KEY;
  const useOllama =
    explicitProvider === 'ollama' || (!openaiKey && !!ollamaKey);
  ...
  if (!openaiKey) {
    return {
      success: false,
      error:
        'No LLM provider configured. Set OPENAI_API_KEY (Tier-3) or OLLAMA_API_KEY (Tier-2 Ollama Cloud — see issue #1725).',
    };
  }
  const model = input.model || 'codex-3-5-sonnet-latest';
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { success: false, model, error: `OpenAI API error ${res.status}: ${errText.slice(0, 400)}` };
    }
 ...
 *
 * Translates the OpenAI-flavored input shape onto OpenAI chat-completions
...
async function callOllamaCompat(
  input: OpenAICallInput & { apiKey: string },
...
/**
 * Resolve a model identifier to an OpenAI model ID. Accepts:
 * - logical names: 'haiku', 'sonnet', 'opus', 'inherit'
 * - prefixed: 'openai:codex-3-5-sonnet-latest'
 * - direct: 'codex-3-5-sonnet-latest'
 */
export function resolveOpenAIModel(input: string | undefined): string {
  if (!input) return 'codex-3-5-sonnet-latest';
  if (input in MODEL_MAP) return MODEL_MAP[input];
  if (input.startsWith('openai:')) return input.slice('openai:'.length);
  return input;
}
...
export async function executeAgentTask(input: AgentExecuteInput): Promise<AgentExecuteResult> {
  ...
  const openaiModel = MODEL_MAP[agent.model || 'sonnet'] || 'codex-3-5-sonnet-latest';
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        agentId: input.agentId,
        model: openaiModel,
        error: `OpenAI API error ${res.status}: ${errText.slice(0, 400)}`,
      };
    }
