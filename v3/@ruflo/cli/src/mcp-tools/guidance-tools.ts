 ...
 *
 * @module @ruflo/cli/mcp-tools/guidance
 */
...
/**
 * Find the project root by looking for .codex/ directory.
...
function findProjectRoot(): string {
  ...
  if (existsSync(join(getProjectCwd(), '.codex'))) {
    return getProjectCwd();
  }

  // Strategy 2: Walk up from CLI package location
  // CLI is at v3/@ruflo/cli/ — project root is 4 levels up
  const fromPackage = join(CLI_ROOT, '../../../..');
  if (existsSync(join(fromPackage, '.codex'))) {
    return fromPackage;
  }
  ...
  for (let i = 0; i < 10; i++) {
    if (existsSync(join(dir, '.codex'))) return dir;
...
const CAPABILITY_CATALOG: Record<string, CapabilityArea> = {
  ...
  'swarm-orchestration': {
    ...
    skills: ['swarm-orchestration', 'swarm-advanced', 'codex-swarm'],
  ...
  'memory-knowledge': {
    ...
    tools: ['memory_store', 'memory_retrieve', 'memory_search', 'memory_list', 'memory_delete', 'memory_init', 'memory_export', 'memory_import_codex', 'memory_stats', 'memory_compact', 'memory_namespace'],
...
function discoverSkills(): string[] {
  const skillsDir = join(PROJECT_ROOT, '.codex/skills');
...
const guidanceDiscover: MCPTool = {
  ...
  description: 'Discover all available agents and skills from the .codex/ directory. Returns live filesystem data.',
  inputSchema: {
    type: 'object',
    properties: {
      type: {
