...
const LEGACY_MEMORY_DIR = '.codex/memory';
...
/**
 * Check if legacy JSON store exists in old .codex/memory/ location
 */
...
async function ensureInitialized(): Promise<void> {
  try {
    ...
    // Migrate legacy JSON data if exists (from old .codex/memory/ location)
    if (hasLegacyStore()) {
...
export const memoryTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      try {
        ...
        // The export landed in @ruflo/memory@>3.0.0-alpha.14 — older
        ...
        if (input.smart) {
          ...
          let memMod: any;
          try {
            memMod = await import('@ruflo/memory');
          } catch (err) {
            smartFallbackReason = `@ruflo/memory failed to load: ${(err as Error).message}`;
          }
          ...
          smartFallbackReason = smartFallbackReason
            ?? 'smartSearch is not exported by the installed @ruflo/memory build (likely a release lag — see #1846). Falling back to standard semantic search.';
        }
  ...
  // ===== Codex Memory Bridge Tools =====

  {
    name: 'memory_import_codex',
    description: 'Import Codex auto-memory files into AgentDB with ONNX vector embeddings. Reads ~/.codex/projects/*/memory/*.md files, parses YAML frontmatter, splits into sections, and stores with 384-dim embeddings for semantic search. Use allProjects=true to import from ALL Codex projects.',
    category: 'memory',
    inputSchema: {
      type: 'object',
      properties: {
        allProjects: { type: 'boolean', description: 'Import from all Codex projects (default: current project only)' },
        namespace: { type: 'string', description: 'Target namespace (default: "codex-memories")' },
      },
    },
    handler: async (input) => {
      await ensureInitialized();
      ...
      const ns = (input.namespace as string) || 'codex-memories';
      ...
      if (allProjects) {
        ...
        if (existsSync(codexProjectsDir)) {
          try {
            for (const project of readdirSync(codexProjectsDir, { withFileTypes: true })) {
              if (!project.isDirectory()) continue;
              const memDir = join(codexProjectsDir, project.name, 'memory');
        ...
        const memDir = join(codexProjectsDir, projectHash, 'memory');
      ...
      if (memoryFiles.length === 0) {
        return { success: true, imported: 0, message: 'No Codex memory files found' };
      }

      let imported = 0;
      let skipped = 0;
      // #1791.8 — Codex's `~/.codex/projects/` accumulates historical
      ...
      for (const memFile of memoryFiles) {
        ...
        try {
          ...
          if (sections.length === 0 && body.length > 10) {
            await storeEntry({ key: `codex:${memFile.project}:${name}`, value: body.slice(0, 4096), namespace: ns, generateEmbeddingFlag: true });
            imported++;
          } else {
            for (const section of sections) {
              ...
              await storeEntry({ key: `codex:${memFile.project}:${name}:${sectionTitle.slice(0, 50)}`, value: sectionBody.slice(0, 4096), namespace: ns, generateEmbeddingFlag: true });
              imported++;
            }
          }
        } catch {
...

  {
    ...
    description: 'Show Codex memory bridge status — AgentDB vectors, SONA learning, intelligence patterns, and connection health.',
    category: 'memory',
    inputSchema: { type: 'object', properties: {} },
    handler: async () => {
      await ensureInitialized();

      // Count Codex memory files
      const codexProjectsDir = join(homedir(), '.codex', 'projects');
      let codexFiles = 0;
      let codexProjects = 0;
      if (existsSync(codexProjectsDir)) {
        try {
          for (const project of readdirSync(codexProjectsDir, { withFileTypes: true })) {
            ...
            if (files.length > 0) { codexProjects++; codexFiles += files.length; }
          }
        } catch { /* ignore */ }
      }
      ...
      let codexMemoryEntries = 0;
      try {
        const { listEntries } = await getMemoryFunctions();
        const allEntries = await listEntries({});
        agentdbEntries = allEntries?.entries?.length ?? 0;
        const codexEntries = await listEntries({ namespace: 'codex-memories' });
        codexMemoryEntries = codexEntries?.entries?.length ?? 0;
      } catch { /* ignore */ }
      ...
      return {
        codexCode: { memoryFiles: codexFiles, projects: codexProjects },
        agentdb: { totalEntries: agentdbEntries, codexMemoryEntries, backend: 'sql.js + ONNX' },
        intelligence,
        bridge: { status: codexMemoryEntries > 0 ? 'connected' : 'not-synced', embedding: 'all-MiniLM-L6-v2 (384-dim)' },
      };
    },
  },

  {
    ...
    description: 'Search across both Codex memories and AgentDB entries using semantic vector similarity. Returns merged, deduplicated results from all namespaces.',
    category: 'memory',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (input) => {
      ...
      const namespaces = ns ? [ns] : ['default', 'codex-memories', 'auto-memory', 'patterns', 'tasks', 'feedback'];
          ...
          if (r?.results) {
            for (const entry of r.results) {
              allResults.push({
                ...
                score: entry.score || 0,
                namespace: searchNs,
                source: searchNs === 'codex-memories' ? 'codex-code' : searchNs === 'auto-memory' ? 'auto-memory' : 'agentdb',
              });
            }
          }
