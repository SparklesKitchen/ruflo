  ...
  try {
    ...
    for (const depth of ['../..', '../../..']) {
      ...
      if (existsSync(pkgPath)) {
        ...
        if (pkg.name?.includes('codex') || pkg.name === 'ruflo') {
...
export const systemTools: MCPTool[] = [
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      // not just the legacy `.codex/memory/*` triple.
      {
        const t0 = performance.now();
        const memoryCandidates = [
          join(projectCwd, '.codex', 'memory', 'store.json'),       // legacy
          join(projectCwd, '.codex', 'memory', 'agentdb.sqlite'),
          join(projectCwd, '.codex', 'memory', 'store.rvf'),
          join(projectCwd, '.codex', 'memory', 'codex.db'),   // sql.js
      ...
      // #1843: also accept YAML config (.codex/config.yaml) which
      // the rest of v3 treats as canonical; previous code only counted
      // .json variants and reported `degraded` when YAML was used.
      {
        const t0 = performance.now();
        const configCandidates = [
          join(projectCwd, '.codex', 'config.json'),
          join(projectCwd, '.codex', 'config.yaml'),
          join(projectCwd, '.codex', 'config.yml'),
          join(projectCwd, 'codex.config.json'),
          join(projectCwd, 'codex.config.yaml'),
          join(projectCwd, 'codex.config.yml'),
        ];
      ...
      if (input.deep) {
        ...
        {
          const t0 = performance.now();
          const coordPath = join(projectCwd, '.codex', 'coordination', 'store.json');
  ...
  },
  {
    ...
    handler: async () => {
      ...
      // When Codex launches us via `codex mcp add`, stdin is piped (not a TTY)
      // and the process IS the MCP server, so it is running.
      const isStdio = !process.stdin.isTTY;
      const transport = process.env.RUFLO_MCP_TRANSPORT || (isStdio ? 'stdio' : 'http');
      const port = parseInt(process.env.RUFLO_MCP_PORT || '3000', 10);
      ...
      const host = process.env.RUFLO_MCP_HOST || 'localhost';
      try {
  ...
  },
  {
    ...
    handler: async () => {
      ...
      const storePath = join(getProjectCwd(), '.codex', 'tasks', 'store.json');
