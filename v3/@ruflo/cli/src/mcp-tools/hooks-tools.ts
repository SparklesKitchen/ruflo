...
// #1773 item 4 — moe-router migrated to @ruflo/neural
let moeRouter: Awaited<ReturnType<typeof import('@ruflo/neural').getMoERouter>> | null = null;
async function getMoERouter() {
  if (!moeRouter) {
    try {
      const { getMoERouter: getMoE } = await import('@ruflo/neural');
...
// #1773 item 4 — flash-attention migrated to @ruflo/neural
let flashAttention: Awaited<ReturnType<typeof import('@ruflo/neural').getFlashAttention>> | null = null;
async function getFlashAttention() {
  if (!flashAttention) {
    try {
      const { getFlashAttention: getFlash } = await import('@ruflo/neural');
...
export const hooksRoute: MCPTool = {
  name: 'hooks_route',
  description: 'Get a 3-tier routing recommendation for a task: Tier 1 (Agent Booster, 0ms / $0 — for var-to-const, add-types, etc.), Tier 2 (Haiku — simple), Tier 3 (Sonnet/Opus — complex). Use this BEFORE spawning an agent to avoid sending simple transforms to Sonnet. Native tools have no equivalent — Codex does not introspect its own model-selection cost. Returns the recommended model + a `[AGENT_BOOSTER_AVAILABLE]` literal when the WASM bypass applies.',
  inputSchema: {
    type: 'object',
    properties: {
...
export const hooksSessionStart: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    try {
      const statuslinePath = join(getProjectCwd(), '.codex', 'helpers', 'statusline.cjs');
...
export const hooksSessionEnd: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      statePath: saveState ? `.codex/sessions/${sessionId}.json` : undefined,
...
export const hooksInit: MCPTool = {
  name: 'hooks_init',
  description: 'Initialize hooks in project with .codex/settings.json',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    return {
      ...
      template,
      created: {
        settingsJson: `${path}/.codex/settings.json`,
        hooksDir: `${path}/.codex/hooks`,
      },
      hooks: {
...
export const hooksIntelligenceAttention: MCPTool = {
  ...
  handler: async (params: Record<string, unknown>) => {
    ...
    async function getQueryEmbedding(text: string, dims: number): Promise<{ embedding: Float32Array; source: 'onnx' | 'hash-fallback' }> {
      // Try ONNX via @ruflo/embeddings
      try {
        const embeddingsModule = await import('@ruflo/embeddings').catch(() => null);
        ...
        // ONNX not available, try agentic
      }

      // Try agentic embeddings
      try {
        const embeddingsModule = await import('@ruflo/embeddings').catch(() => null);
        if (embeddingsModule?.createEmbeddingService) {
          const service = embeddingsModule.createEmbeddingService({ provider: 'agentic' });
          const result = await service.embed(text);
          const arr = new Float32Array(dims);
          for (let i = 0; i < Math.min(dims, result.embedding.length); i++) {
            arr[i] = result.embedding[i];
    ...
    return {
      ...
      stats: {
        ...
        ...(embeddingSource === 'hash-fallback' && implementation !== 'none'
          ? { _embeddingNote: 'Query embeddings are hash-based (not semantic). Install @ruflo/embeddings for real ONNX embeddings.' }
          : {}),
      },
      implementation,
    };
...
/**
 * Worker trigger types matching agentic v3
 */
type WorkerTrigger =
