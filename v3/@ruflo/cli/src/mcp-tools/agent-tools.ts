...
// Model types matching Codex Agent SDK
...
interface AgentRecord {
  ...
  createdAt: string;
  domain?: string;
  model?: CodexModel;  // Model assigned to this agent
...
const AGENT_TYPE_MODEL_DEFAULTS: Record<string, CodexModel> = {
  // Complex agents → opus
  'architect': 'opus',
...
export const agentTools: MCPTool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: {
          ...
          description: 'Codex model to use (haiku=fast/cheap, sonnet=balanced, opus=most capable)'
        },
    ...
    handler: async (input) => {
      ...
      const response: Record<string, unknown> = {
        ...
        note: 'Agent registered for coordination. Three execution paths: ' +
          '(1) call agent_execute(agentId, prompt) — direct LLM call via OpenAI Messages API (requires OPENAI_API_KEY); ' +
          '(2) Codex Task tool — spawns a real subagent; ' +
          '(3) codex -p — headless background instance.',
      };
  ...
  },
  {
    ...
    // provider — the wire between OpenAIProvider and the agent
    ...
    // model, calling the OpenAI Messages API directly via fetch, and
    // updating the agent record with lastResult / taskCount / status.
    // No mock — actual HTTP request to api.openai.com.
    name: 'agent_execute',
    description: 'Execute a task on a spawned agent — calls the OpenAI Messages API with the agent\'s configured model. Requires OPENAI_API_KEY in env.',
    category: 'agent',
    inputSchema: {
      type: 'object',
      properties: {
