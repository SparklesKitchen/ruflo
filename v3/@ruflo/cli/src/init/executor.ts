...
import { generateCodexMd } from './codexmd-generator.js';
...
const SKILLS_MAP: Record<string, string[]> = {
  ...
  dualMode: ['dual-mode'],  // Codex + Codex hybrid execution
  agentdb: [
    'agentdb-advanced',
    'agentdb-learning',
...
const COMMANDS_MAP: Record<string, string[]> = {
  core: ['codex-help.md', 'codex-swarm.md', 'codex-memory.md'],
  analysis: ['analysis'],
...
export async function executeInit(options: InitOptions): Promise<InitResult> {
  ...
  try {
    ...
    // Generate AGENTS.md
    if (options.components.codexMd) {
      await writeCodexMd(targetDir, options, result);
    }
...
function mergeSettingsForUpgrade(existing: Record<string, unknown>): Record<string, unknown> {
  ...
  // TeammateIdle/TaskCompleted are not valid Codex hook events and caused warnings.
  // Agent Teams hook config lives in codexFlow.agentTeams.hooks instead.
  ...
  merged.env = {
    ...
    RUFLO_V3_ENABLED: existingEnv.RUFLO_V3_ENABLED || 'true',
    RUFLO_HOOKS_ENABLED: existingEnv.RUFLO_HOOKS_ENABLED || 'true',
  };
  ...
  const autoMemoryScript = '.codex/helpers/auto-memory-hook.mjs';
  ...
  // NOTE: TeammateIdle and TaskCompleted are NOT valid Codex hook events.
  ...
  // Their configuration lives in codexFlow.agentTeams.hooks instead.

  // 3. Fix statusLine config (remove invalid fields, ensure correct format)
  // Codex only supports: type, command, padding
  const existingStatusLine = existing.statusLine as Record<string, unknown> | undefined;
  if (existingStatusLine) {
    merged.statusLine = {
      type: 'command',
      command: existingStatusLine.command || `node -e "var c=require('child_process'),p=require('path'),r;try{r=c.execSync('git rev-parse --show-toplevel',{encoding:'utf8'}).trim()}catch(e){r=process.cwd()}var s=p.join(r,'.codex/helpers/statusline.cjs');process.argv.splice(1,0,s);require(s)"`,
      // Remove invalid fields: refreshMs, enabled (not supported by Codex)
    };
  }

  // 4. Merge codexFlow settings (preserve existing, add agentTeams + memory)
  const existingCodexFlow = (existing.codexFlow as Record<string, unknown>) || {};
  const existingMemory = (existingCodexFlow.memory as Record<string, unknown>) || {};
  merged.codexFlow = {
    ...existingCodexFlow,
    version: existingCodexFlow.version || '3.0.0',
    enabled: existingCodexFlow.enabled !== false,
    agentTeams: {
      enabled: true,
      teammateMode: 'auto',
  ...
  try {
    ...
    if (sourceHelpersForUpgrade) {
      ...
      for (const helperName of criticalHelpers) {
        const targetPath = path.join(targetDir, '.codex', 'helpers', helperName);
        const sourcePath = path.join(sourceHelpersForUpgrade, helperName);
        if (fs.existsSync(sourcePath)) {
          if (fs.existsSync(targetPath)) {
            result.updated.push(`.codex/helpers/${helperName}`);
          } else {
            result.created.push(`.codex/helpers/${helperName}`);
          }
    ...
    const statuslinePath = path.join(targetDir, '.codex', 'helpers', 'statusline.cjs');
    ...
    if (fs.existsSync(statuslinePath)) {
      result.updated.push('.codex/helpers/statusline.cjs');
    } else {
      result.created.push('.codex/helpers/statusline.cjs');
    }
    fs.writeFileSync(statuslinePath, statuslineContent, 'utf-8');

    // 2. Create MISSING metrics files only (preserve existing data)
    const metricsDir = path.join(targetDir, '.codex', 'metrics');
    const securityDir = path.join(targetDir, '.codex', 'security');
    ...
    if (!fs.existsSync(progressPath)) {
      ...
      result.created.push('.codex/metrics/v3-progress.json');
    } else {
      result.preserved.push('.codex/metrics/v3-progress.json');
    }
    ...
    if (!fs.existsSync(activityPath)) {
      ...
      result.created.push('.codex/metrics/swarm-activity.json');
    } else {
      result.preserved.push('.codex/metrics/swarm-activity.json');
    }
    ...
    if (!fs.existsSync(learningPath)) {
      ...
      result.created.push('.codex/metrics/learning.json');
    } else {
      result.preserved.push('.codex/metrics/learning.json');
    }
    ...
    if (!fs.existsSync(auditPath)) {
      const audit = {
        ...
        totalCves: 3,
        lastScan: null,
        _note: 'Run: npx @ruflo/cli@latest security scan'
      };
      fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
      result.created.push('.codex/security/audit-status.json');
    } else {
      result.preserved.push('.codex/security/audit-status.json');
    }

    // 3. Merge settings if requested
    if (upgradeSettings) {
      const settingsPath = path.join(targetDir, '.codex', 'settings.json');
      if (fs.existsSync(settingsPath)) {
        try {
          ...
          result.updated.push('.codex/settings.json');
          result.settingsUpdated = [
            'env.CODEX_EXPERIMENTAL_AGENT_TEAMS',
            'hooks.SessionStart (auto-memory import)',
            'hooks.SessionEnd (auto-memory sync)',
            'hooks.TeammateIdle (removed — not a valid Codex hook)',
            'hooks.TaskCompleted (removed — not a valid Codex hook)',
            'codexFlow.agentTeams',
            'codexFlow.memory (learningBridge, memoryGraph, agentScopes)',
          ];
        } catch (settingsError) {
        ...
        result.created.push('.codex/settings.json');
...
export async function executeUpgradeWithMissing(targetDir: string, upgradeSettings = false): Promise<UpgradeResult> {
  ...
  try {
    ...
    if (process.env.DEBUG || process.env.RUFLO_DEBUG) {
    ...
    if (sourceSkillsDir) {
      ...
      const debugMode = process.env.DEBUG || process.env.RUFLO_DEBUG;
      if (debugMode) {
      ...
      for (const skillName of [...new Set(allSkills)]) {
        ...
        if (sourceExists && !targetExists) {
          ...
          result.created.push(`.codex/skills/${skillName}`);
        }
      }
    }
    ...
    if (sourceAgentsDir) {
      ...
      for (const agentCategory of [...new Set(allAgents)]) {
        ...
        if (fs.existsSync(sourcePath) && !fs.existsSync(targetPath)) {
          ...
          result.created.push(`.codex/agents/${agentCategory}`);
        }
      }
    }
    ...
    if (sourceCommandsDir) {
      ...
      for (const cmdName of [...new Set(allCommands)]) {
        ...
        if (fs.existsSync(sourcePath) && !fs.existsSync(targetPath)) {
          ...
          result.created.push(`.codex/commands/${cmdName}`);
        }
      }
    }
...
async function writeSettings(
  ...
  if (fs.existsSync(settingsPath) && !options.force) {
    ...
    try {
      ...
      // Merge env vars (for CODEX_EXPERIMENTAL_AGENT_TEAMS etc.)
      if (generated.env) {
      ...
      if (merged) {
        ...
        result.created.files.push('.codex/settings.json (merged hooks)');
      } else {
        result.skipped.push('.codex/settings.json');
      }
    } catch {
      // Existing file is corrupt — overwrite
      fs.writeFileSync(settingsPath, JSON.stringify(generated, null, 2), 'utf-8');
      result.created.files.push('.codex/settings.json');
    }
    return;
  }
...
/**
 * #1779 — Walk parents of `targetDir` plus the user-global Codex
 * config locations, looking for any `.mcp.json` (or `~/.codex.json`)
 * that already declares a `ruflo`-keyed MCP server. We use this to skip
 * writing our own `codex`-keyed entry when the user has already
...
function detectExistingRufloMCP(targetDir: string): string | null {
  ...
  // User-global Codex config locations
  if (home) {
    candidates.add(path.join(home, '.codex.json'));
    candidates.add(path.join(home, '.codex', 'mcp.json'));
  }
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // (b) #1840: Codex project-scoped registrations under
      //     parsed.projects[<projectPath>].mcpServers.ruflo. Match by
      //     normalized path against targetDir or any of its ancestors so
      //     a `codex mcp add ruflo` in this repo is detected even when
      //     Codex stored the key with different casing/slash style.
 ...
 * Codex stores Windows paths like "C:/Users/.../Project" while
...
async function writeMCPConfig(
  ...
  // server registered elsewhere (parent .mcp.json, ~/.codex.json, etc).
  // Writing our `codex`-keyed entry on top of that produces the
...
async function copySkills(
  ...
  const targetSkillsDir = path.join(targetDir, '.codex', 'skills');
  ...
  for (const skillName of [...new Set(skillsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        copyDirRecursive(sourcePath, targetPath);
        result.created.files.push(`.codex/skills/${skillName}`);
        result.summary.skillsCount++;
      } else {
        result.skipped.push(`.codex/skills/${skillName}`);
      }
    }
  }
}
...
async function copyCommands(
  ...
  const targetCommandsDir = path.join(targetDir, '.codex', 'commands');
  ...
  for (const cmdName of [...new Set(commandsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        ...
        result.created.files.push(`.codex/commands/${cmdName}`);
        result.summary.commandsCount++;
      } else {
        result.skipped.push(`.codex/commands/${cmdName}`);
      }
    }
  }
}
...
async function copyAgents(
  ...
  if (agentsConfig.all) {
    ...
    // Dual-mode agents (Codex + Codex hybrid)
  ...
  for (const agentCategory of [...new Set(agentsToCopy)]) {
    ...
    if (fs.existsSync(sourcePath)) {
      if (!fs.existsSync(targetPath) || options.force) {
        ...
        result.created.files.push(`.codex/agents/${agentCategory}`);
      } else {
        result.skipped.push(`.codex/agents/${agentCategory}`);
      }
    }
  }
}
...
function findSourceHelpersDir(sourceBaseDir?: string): string | null {
  ...
  if (sourceBaseDir) {
    possiblePaths.push(path.join(sourceBaseDir, '.codex', 'helpers'));
  }

  // Strategy 1: require.resolve to find package root (most reliable for npx)
  try {
    const esmRequire = createRequire(import.meta.url);
    const pkgJsonPath = esmRequire.resolve('@ruflo/cli/package.json');
    const pkgRoot = path.dirname(pkgJsonPath);
    possiblePaths.push(path.join(pkgRoot, '.codex', 'helpers'));
  } catch {
  ...
  const packageHelpers = path.join(packageRoot, '.codex', 'helpers');
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const helpersPath = path.join(parentDir, '.codex', 'helpers');
  ...
  const cwdBased = [
    path.join(process.cwd(), '.codex', 'helpers'),
    path.join(process.cwd(), '..', '.codex', 'helpers'),
    path.join(process.cwd(), '..', '..', '.codex', 'helpers'),
  ];
...
async function writeHelpers(
  ...
  const helpersDir = path.join(targetDir, '.codex', 'helpers');
  ...
  if (sourceHelpersDir && fs.existsSync(sourceHelpersDir)) {
    ...
    for (const file of helperFiles) {
      ...
      if (!fs.existsSync(destPath) || options.force) {
        ...
        result.created.files.push(`.codex/helpers/${file}`);
        copiedCount++;
      } else {
        result.skipped.push(`.codex/helpers/${file}`);
      }
    }
  ...
  for (const [name, content] of Object.entries(helpers)) {
    ...
    if (!fs.existsSync(filePath) || options.force) {
      ...
      result.created.files.push(`.codex/helpers/${name}`);
    } else {
      result.skipped.push(`.codex/helpers/${name}`);
    }
  }
}

/**
 * Find source .codex directory for statusline files
 */
function findSourceCodexDir(sourceBaseDir?: string): string | null {
  const possiblePaths: string[] = [];

  // If explicit source base directory is provided, check it first
  if (sourceBaseDir) {
    possiblePaths.push(path.join(sourceBaseDir, '.codex'));
  }

  // IMPORTANT: Check the package's own .codex directory
  // Go up 3 levels: dist/src/init -> dist/src -> dist -> root
  const packageRoot = path.resolve(__dirname, '..', '..', '..');
  const packageCodex = path.join(packageRoot, '.codex');
  if (fs.existsSync(packageCodex)) {
    possiblePaths.unshift(packageCodex); // Add to beginning (highest priority)
  }
  ...
  for (let i = 0; i < 10; i++) {
    const parentDir = path.dirname(currentDir);
    const codexPath = path.join(parentDir, '.codex');
    if (fs.existsSync(codexPath)) {
      possiblePaths.push(codexPath);
    }
    currentDir = parentDir;
  }
...
async function writeStatusline(
  ...
  // Find source .codex directory (works for npm package and local dev)
  const sourceCodexDir = findSourceCodexDir(options.sourceBaseDir);

  // Try to copy existing advanced statusline files from source
  const advancedStatuslineFiles = [
    { src: 'statusline.sh', dest: 'statusline.sh', dir: codexDir },
    { src: 'statusline.mjs', dest: 'statusline.mjs', dir: codexDir },
  ];

  if (sourceCodexDir) {
    for (const file of advancedStatuslineFiles) {
      const sourcePath = path.join(sourceCodexDir, file.src);
      ...
      if (fs.existsSync(sourcePath)) {
        if (!fs.existsSync(destPath) || options.force) {
          ...
          result.created.files.push(`.codex/${file.dest}`);
        } else {
          result.skipped.push(`.codex/${file.dest}`);
        }
      }
    }
  }
  ...
  result.created.files.push('.codex/helpers/statusline.cjs');
}

/**
 * Write runtime configuration (.codex/)
 */
async function writeRuntimeConfig(
  ...
  const configPath = path.join(targetDir, '.codex', 'config.yaml');

  if (fs.existsSync(configPath) && !options.force) {
    result.skipped.push('.codex/config.yaml');
    return;
  }
...
`;
  ...
  result.created.files.push('.codex/config.yaml');

  // Write .gitignore
  const gitignorePath = path.join(targetDir, '.codex', '.gitignore');
...
`;
  ...
  if (!fs.existsSync(gitignorePath) || options.force) {
    ...
    result.created.files.push('.codex/.gitignore');
  }
...
async function writeInitialMetrics(
  ...
  const metricsDir = path.join(targetDir, '.codex', 'metrics');
  const learningDir = path.join(targetDir, '.codex', 'learning');
  const securityDir = path.join(targetDir, '.codex', 'security');
  ...
  if (!fs.existsSync(progressPath) || options.force) {
    ...
    fs.writeFileSync(progressPath, JSON.stringify(progress, null, 2), 'utf-8');
    result.created.files.push('.codex/metrics/v3-progress.json');
  }
  ...
  if (!fs.existsSync(activityPath) || options.force) {
    ...
    fs.writeFileSync(activityPath, JSON.stringify(activity, null, 2), 'utf-8');
    result.created.files.push('.codex/metrics/swarm-activity.json');
  }
  ...
  if (!fs.existsSync(learningPath) || options.force) {
    ...
    fs.writeFileSync(learningPath, JSON.stringify(learning, null, 2), 'utf-8');
    result.created.files.push('.codex/metrics/learning.json');
  }
  ...
  if (!fs.existsSync(auditPath) || options.force) {
    ...
    fs.writeFileSync(auditPath, JSON.stringify(audit, null, 2), 'utf-8');
    result.created.files.push('.codex/security/audit-status.json');
  }
}
...
async function writeCapabilitiesDoc(
  ...
  const capabilitiesPath = path.join(targetDir, '.codex', 'CAPABILITIES.md');

  if (fs.existsSync(capabilitiesPath) && !options.force) {
    result.skipped.push('.codex/CAPABILITIES.md');
    return;
  }

  const capabilities = `# RuFlo V3 - Complete Capabilities Reference
> Generated: ${new Date().toISOString()}
> Full documentation: https://github.com/SparklesKitchen/ruflo
...
# Check status
npx @ruflo/cli@latest swarm status

# Monitor activity
npx @ruflo/cli@latest swarm monitor
\`\`\`
...
# Memory operations
npx @ruflo/cli@latest memory store --key "pattern" --value "data" --namespace patterns
npx @ruflo/cli@latest memory search --query "authentication"
...
**AgentMemoryScope** - Maps Codex 3-scope directories:
- \`project\`: \`<gitRoot>/.codex/agent-memory/<agent>/\`
- \`local\`: \`<gitRoot>/.codex/agent-memory-local/<agent>/\`
- \`user\`: \`~/.codex/agent-memory/<agent>/\`
...
\`\`\`bash
# Store pattern
npx @ruflo/cli@latest memory store --key "name" --value "data" --namespace patterns

# Semantic search
npx @ruflo/cli@latest memory search --query "authentication"

# List entries
npx @ruflo/cli@latest memory list --namespace patterns

# Initialize database
npx @ruflo/cli@latest memory init --force
\`\`\`
...
\`\`\`bash
# Initialize
npx @ruflo/cli@latest hive-mind init --queen-type strategic

# Status
npx @ruflo/cli@latest hive-mind status

# Spawn workers
npx @ruflo/cli@latest hive-mind spawn --count 5 --type worker

# Consensus
npx @ruflo/cli@latest hive-mind consensus --propose "task"
\`\`\`
...
|---------|---------|---------|
| agentic | 3.0.0-alpha.1 | Core coordination + ReasoningBank + Router |
...
\`\`\`bash
# Add Ruflo MCP
codex mcp add ruflo -- npx -y ruflo@latest

# Optional servers
codex mcp add ruv-swarm -- npx -y ruv-swarm mcp start
codex mcp add flow-nexus -- npx -y flow-nexus@latest mcp start
\`\`\`
...
`;
  ...
  result.created.files.push('.codex/CAPABILITIES.md');
}

/**
 * Write AGENTS.md with swarm guidance
 */
async function writeCodexMd(
  targetDir: string,
  options: InitOptions,
  result: InitResult
): Promise<void> {
  const codexMdPath = path.join(targetDir, 'AGENTS.md');

  if (fs.existsSync(codexMdPath) && !options.force) {
    ...
    const content = generateCodexMd(options, inferredTemplate);

    fs.writeFileSync(codexMdPath, content, 'utf-8');
    result.created.files.push('AGENTS.md');
  }

  // Also write/append global ~/.codex/AGENTS.md so ruflo tools are used automatically (#1497).
  // Opt-out via --no-global / options.skipGlobalCodexMd (#1744 — keeps global rules file pristine
  // for users who don't want a per-machine pointer block).
  const homeDir = process.env.HOME || process.env.USERPROFILE || '';
  if (homeDir && !options.skipGlobalCodexMd) {
    const globalCodexDir = path.join(homeDir, '.codex');
    const globalCodexMd = path.join(globalCodexDir, 'AGENTS.md');
    const rufloBlock = [
      '',
    ...
    try {
      ...
      if (fs.existsSync(globalCodexMd)) {
        const existing = fs.readFileSync(globalCodexMd, 'utf-8');
        if (!existing.includes('Ruflo Integration')) {
          fs.appendFileSync(globalCodexMd, rufloBlock);
          result.created.files.push('~/.codex/AGENTS.md (appended ruflo block)');
        }
      } else {
        fs.writeFileSync(globalCodexMd, rufloBlock.trimStart(), 'utf-8');
        result.created.files.push('~/.codex/AGENTS.md');
      }
    } catch {
      // Non-critical — global AGENTS.md is best-effort
    }
  } else if (options.skipGlobalCodexMd) {
    result.skipped.push('~/.codex/AGENTS.md (--no-global)');
  }
}
...
function findSourceDir(type: 'skills' | 'commands' | 'agents', sourceBaseDir?: string): string | null {
  ...
  if (sourceBaseDir) {
    possiblePaths.push(path.join(sourceBaseDir, '.codex', type));
  }

  // IMPORTANT: Check the package's own .codex directory first
  // This is the primary path when running as an npm package
  // __dirname is typically /path/to/node_modules/@ruflo/cli/dist/src/init
  // We need to go up 3 levels to reach the package root (dist/src/init -> dist/src -> dist -> root)
  const packageRoot = path.resolve(__dirname, '..', '..', '..');
  const packageDotCodex = path.join(packageRoot, '.codex', type);
  if (fs.existsSync(packageDotCodex)) {
    possiblePaths.unshift(packageDotCodex); // Add to beginning (highest priority)
  }

  // From dist/src/init -> go up to project root
  const distPath = __dirname;

  // Try to find the project root by looking for .codex directory
  let currentDir = distPath;
  for (let i = 0; i < 10; i++) {
    const parentDir = path.dirname(currentDir);
    const dotCodexPath = path.join(parentDir, '.codex', type);
    if (fs.existsSync(dotCodexPath)) {
      possiblePaths.push(dotCodexPath);
    }
    currentDir = parentDir;
  }
  ...
  const cwdBased = [
    path.join(process.cwd(), '.codex', type),
    path.join(process.cwd(), '..', '.codex', type),
    path.join(process.cwd(), '..', '..', '.codex', type),
  ];
  possiblePaths.push(...cwdBased);
  ...
  if (type === 'agents') {
    possiblePaths.push(
      path.join(process.cwd(), 'v2', '.codex', type),
      path.join(process.cwd(), '..', 'v2', '.codex', type),
    );
  }
