/**
 * AGENTS.md Generator
 * Generates lean, enforceable Codex configuration optimized for token efficiency.
 *
...
import type { InitOptions, CodexMdTemplate } from './types.js';
...
### Before Any Task
\`\`\`bash
npx @ruflo/cli@latest memory search --query "[task keywords]" --namespace patterns
npx @ruflo/cli@latest hooks route --task "[task description]"
\`\`\`

### After Success
\`\`\`bash
npx @ruflo/cli@latest memory store --namespace patterns --key "[name]" --value "[what worked]"
npx @ruflo/cli@latest hooks post-task --task-id "[id]" --success true --store-results true
\`\`\`
...
| **Bridge** | \`memory_import_codex\`, \`memory_bridge_status\` |
...
\`\`\`bash
npx @ruflo/cli@latest hooks worker dispatch --trigger audit
\`\`\``;
}
...
\`\`\`bash
npx @ruflo/cli@latest init --wizard           # Setup
npx @ruflo/cli@latest swarm init --v3-mode     # Start swarm
npx @ruflo/cli@latest memory search --query "" # Vector search
npx @ruflo/cli@latest hooks route --task ""    # Route to agent
npx @ruflo/cli@latest doctor --fix             # Diagnostics
npx @ruflo/cli@latest security scan            # Security scan
npx @ruflo/cli@latest performance benchmark    # Benchmarks
\`\`\`
...
\`\`\`bash
codex mcp add codex -- npx -y @ruflo/cli@latest
...
\`\`\`bash
npx @ruflo/cli@latest security scan --depth full
npx @ruflo/cli@latest security audit --report
\`\`\`
...
\`\`\`bash
npx @ruflo/cli@latest performance benchmark --suite all
npx @ruflo/cli@latest performance profile --target "[component]"
\`\`\`
...
\`\`\`bash
npx @ruflo/cli@latest hooks pre-task --description "[task]"
npx @ruflo/cli@latest hooks post-task --task-id "[id]" --success true
npx @ruflo/cli@latest hooks session-start --session-id "[id]"
npx @ruflo/cli@latest hooks route --task "[task]"
npx @ruflo/cli@latest hooks worker dispatch --trigger audit
\`\`\``;
}
...
- **Codex Bridge**: Auto-imports \`~/.codex/projects/*/memory/*.md\` into AgentDB`;
}
...
\`\`\`bash
npx @ruflo/cli@latest federation init
npx @ruflo/cli@latest federation join wss://peer:8443
npx @ruflo/cli@latest federation send --to peer --type task-request --message "..."
npx @ruflo/cli@latest federation status
\`\`\`
...
const TEMPLATE_SECTIONS: Record<CodexMdTemplate, Array<(opts: InitOptions) => string>> = {
  minimal: [
    behavioralRules,
    (_opts) => agentComms(),
    swarmConfig,
...
export function generateCodexMd(options: InitOptions, template?: CodexMdTemplate): string {
  const tmpl = template ?? options.runtime.codexMdTemplate ?? 'standard';
  const sections = TEMPLATE_SECTIONS[tmpl] ?? TEMPLATE_SECTIONS.standard;

  const header = `# Ruflo — Codex Configuration\n`;
  const body = sections.map(fn => fn(options)).join('\n\n');

  return `${header}\n${body}\n`;
}

export function generateMinimalCodexMd(options: InitOptions): string {
  return generateCodexMd(options, 'minimal');
}

export const CODEX_MD_TEMPLATES: Array<{ name: CodexMdTemplate; description: string }> = [
...
export default generateCodexMd;
