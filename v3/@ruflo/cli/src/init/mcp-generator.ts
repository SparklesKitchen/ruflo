 ...
 * Creates .mcp.json for Codex MCP server integration
  ...
  if (config.codexFlow) {
    mcpServers['ruflo'] = createMCPServerEntry(
      ['ruflo@latest', 'mcp', 'start'],
      {
        ...
        RUFLO_TOPOLOGY: options.runtime.topology,
        RUFLO_MAX_AGENTS: String(options.runtime.maxAgents),
        RUFLO_MEMORY_BACKEND: options.runtime.memoryBackend,
      },
...
export function generateMCPCommands(options: InitOptions): string[] {
  ...
  if (isWindows()) {
    if (config.codexFlow) {
      commands.push('codex mcp add ruflo -- cmd /c npx -y ruflo@latest mcp start');
    }
    if (config.ruvSwarm) {
      commands.push('codex mcp add ruv-swarm -- cmd /c npx -y ruv-swarm mcp start');
    }
    if (config.flowNexus) {
      commands.push('codex mcp add flow-nexus -- cmd /c npx -y flow-nexus@latest mcp start');
    }
  } else {
    if (config.codexFlow) {
      commands.push("codex mcp add ruflo -- npx -y ruflo@latest mcp start");
    }
    if (config.ruvSwarm) {
      commands.push("codex mcp add ruv-swarm -- npx -y ruv-swarm mcp start");
    }
    if (config.flowNexus) {
      commands.push("codex mcp add flow-nexus -- npx -y flow-nexus@latest mcp start");
    }
  }
