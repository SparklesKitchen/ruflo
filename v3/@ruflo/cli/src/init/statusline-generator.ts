...
function getSettings() {
  ...
  _settingsCache = readJSON(path.join(CWD, '.codex', 'settings.json'))
                || readJSON(path.join(CWD, '.codex', 'settings.local.json'))
                || null;
...
// Detect model name from Codex config (pure file reads, no exec)
function getModelName() {
  try {
    const codexConfig = readJSON(path.join(os.homedir(), '.codex.json'));
    if (codexConfig && codexConfig.projects) {
      for (const [projectPath, projectConfig] of Object.entries(codexConfig.projects)) {
...
function getLearningStats() {
  ...
  try {
    const sessDir = path.join(CWD, '.codex', 'sessions');
    if (fs.existsSync(sessDir)) {
      sessions = fs.readdirSync(sessDir).filter(f => f.endsWith('.json')).length;
    }
  } catch { /* ignore */ }

  // 5. Count session files from codex
  if (sessions === 0) {
    try {
...
function getSecurityStatus() {
  ...
  let scanCount = 0;
  try {
    const scanDir = path.join(CWD, '.codex', 'security-scans');
...
function getAgentDBStats() {
  ...
  if (!hasHnsw) {
    const memPkgPaths = [
      path.join(CWD, 'v3', '@codex', 'memory', 'dist'),
      path.join(CWD, 'node_modules', '@codex', 'memory'),
    ];
...
function getIntegrationStatus() {
  ...
  if (mcpServers.total === 0) {
    const mcpConfig = readJSON(path.join(CWD, '.mcp.json'))
                   || readJSON(path.join(os.homedir(), '.codex', 'mcp.json'));
  ...
  const hasApi = !!(process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY);
...
function getSessionStats() {
  var sessionPaths = ['.ruflo/session.json', '.codex/session.json'];
...
function generateStatusline() {
  ...
  // Prefer model name from Codex stdin data, fallback to file-based detection
  ...
  let pkgVersion = '3.6';
  try {
    const pkgPath = path.join(CWD, 'node_modules', '@codex', 'cli', 'package.json');
    if (fs.existsSync(pkgPath)) {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
      if (pkg.version) pkgVersion = pkg.version;
    } else {
      // Try npx-installed location
      const npxPkg = path.join(CWD, 'v3', '@codex', 'cli', 'package.json');
...
// ─── Stdin reader (Codex pipes session JSON) ──────────────

// Codex sends session JSON via stdin (model, context, cost, etc.)
// Read it synchronously so the script works both:
//   1. When invoked by Codex (stdin has JSON)
...
// Override model detection to prefer stdin data from Codex
...
// Get context window info from Codex session
...
// Get cost info from Codex session
...
# "statusLine": {
#   "type": "command",
#   "command": "node .codex/helpers/statusline.cjs 2>/dev/null"
#   "when": "test -f .codex/helpers/statusline.cjs"
# }
`;
}
