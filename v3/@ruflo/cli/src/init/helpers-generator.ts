/**
 * Helpers Generator
 * Creates utility scripts in .codex/helpers/
 */
...
for FILE in $STAGED_FILES; do
  if [[ "$FILE" =~ \\.(ts|js|tsx|jsx)$ ]]; then
    ...
    npx @ruflo/cli hooks pre-edit --file "$FILE" --validate-syntax 2>/dev/null || true
  fi
done
...
export function generateHookHandler(): string {
  ...
  const lines = [
    ...
    'const [,, command, ...args] = process.argv;',
    '',
    '// Read stdin with timeout — Codex sends hook data as JSON via stdin.',
    ...
    "    console.log('IMPORTANT: Review AGENTS.md in project root for:');",
    ...
    "    console.log('   - All agents available in .codex/agents/ directory');",
    "    console.log('   - Concurrent execution patterns from AGENTS.md');",
 ...
 * @ruflo/memory is not installed. Gets overwritten when source copy succeeds.
 */
...
async function loadMemoryPackage() {
  ...
  // when installed as a transitive dependency via npx ruflo / npx ruflo)
  try {
  ...
  // Strategy 2: ESM import (works when @ruflo/memory is a direct dependency)
  try { return await import('@ruflo/memory'); } catch { /* fall through */ }

  // Strategy 3: Walk up from PROJECT_ROOT looking for the package in any node_modules
  let searchDir = PROJECT_ROOT;
  const { parse } = await import('path');
  while (searchDir !== parse(searchDir).root) {
    const candidate = join(searchDir, 'node_modules', '@codex', 'memory', 'dist', 'index.js');
...
try {
  ...
  // Hooks must never crash Codex - fail silently
  dim(\`Error (non-critical): \${err.message}\`);
}
// Ensure clean exit for Codex hooks (exit 0 = success)
process.exit(0);
`;
}
...
$CodexFlowDir = Join-Path $PWD '.ruflo'
$PidDir = Join-Path $CodexFlowDir 'pids'
...
function getDataDir() {
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env.APPDATA || homeDir, 'codex', 'sessions');
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'codex', 'sessions');
    default:
