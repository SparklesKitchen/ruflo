 ...
 * Creates .codex/settings.json with V3-optimized hook configurations
 */
...
export function generateSettings(options: InitOptions): object {
  ...
  // .codex/helpers/hook-handler.cjs; if that file isn't created (as in
  ...
  // Note: Codex expects 'model' to be a string, not an object
  // Model preferences are stored in codexFlow settings instead
  // settings.model = 'codex-sonnet-4-5-20250929'; // Uncomment if you want to set a default model

  // Add Agent Teams configuration (experimental feature)
  settings.env = {
    // Enable Codex Agent Teams for multi-agent coordination
    CODEX_EXPERIMENTAL_AGENT_TEAMS: '1',
    // Ruflo specific environment
    RUFLO_V3_ENABLED: 'true',
  ...
  settings.codexFlow = {
    ...
    modelPreferences: {
      default: 'codex-opus-4-7',
      routing: 'codex-haiku-4-5-20251001',
    },
    agentTeams: {
      enabled: true,
...
/**
 * Build a hook command with reliable $CODEX_PROJECT_DIR expansion.
 * Wraps in `sh -c` to guarantee shell expansion on all platforms (macOS zsh,
 * Linux bash). Falls back to "." if CODEX_PROJECT_DIR is unset, since
 * Codex runs hooks from the project root.
 * On Windows, uses `cmd /c` with %CODEX_PROJECT_DIR%.
 */
function hookCmd(script: string, subcommand: string): string {
  if (IS_WINDOWS) {
    return `cmd /c node %CODEX_PROJECT_DIR%/${script} ${subcommand}`.trim();
  }
  // Use sh -c to ensure $CODEX_PROJECT_DIR is expanded by a real shell,
  // even if Codex doesn't invoke hooks through a shell on macOS.
  // eslint-disable-next-line no-template-curly-in-string
  const dir = '${CODEX_PROJECT_DIR:-.}';
...
function hookHandlerCmd(subcommand: string): string {
  return hookCmd('.codex/helpers/hook-handler.cjs', subcommand);
}

/** Shorthand for ESM auto-memory-hook commands */
function autoMemoryCmd(subcommand: string): string {
  return hookCmd('.codex/helpers/auto-memory-hook.mjs', subcommand);
}

/**
 * Generate statusLine configuration for Codex
...
function generateHooksConfig(config: HooksConfig): object {
  ...
  // Notification — capture Codex notifications for logging
  ...
  // Codex's settings.json validator (rejected as "Invalid key in record").
  // Agent Teams coordination lives in codexFlow.agentTeams.hooks instead.
