...
export class PluginManager {
  ...
  constructor(baseDir: string = process.cwd()) {
    const pluginsDir = path.join(baseDir, '.ruflo', 'plugins');
    this.config = {
      pluginsDir,
  ...
  async installFromNpm(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        // Check for codex plugin metadata
        if (pkg['codex']) {
          commands = pkg['codex'].commands || [];
          hooks = pkg['codex'].hooks || [];
        }
      }
  ...
  async upgrade(
    ...
    try {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        existing.commands = pkg['codex']?.commands || existing.commands;
        existing.hooks = pkg['codex']?.hooks || existing.hooks;
      }
...
export function getPluginManager(baseDir?: string): PluginManager {
  ...
  } else if (baseDir && defaultManager.getPluginsDir() !== path.join(baseDir, '.ruflo', 'plugins')) {
