/**
 * V3 CLI Init Command
 * Comprehensive initialization for Ruflo with Codex integration
 */
...
async function initCodexAction(
  ...
  try {
    ...
    // Try multiple resolution strategies for the @ruflo/codex package
    // Use a variable to prevent TypeScript from statically resolving the optional module
    const codexModuleId = '@ruflo/codex';
    const resolutionStrategies = [
      ...
      async () => {
        const projectPath = path.join(ctx.cwd, 'node_modules', '@codex', 'codex', 'dist', 'index.js');
      ...
      async () => {
        ...
        const codexPath = path.join(globalPath, '@codex', 'codex', 'dist', 'index.js');
    ...
    if (!CodexInitializer) {
      throw new Error('Cannot find module @ruflo/codex');
    }
    ...
    output.printBox(
      [
        ...
        dualMode ? `AGENTS.md: Codex compatibility` : '',
      ].filter(Boolean).join('\n'),
    ...
    output.printList([
      ...
      dualMode ? `Codex users can use ${output.highlight('AGENTS.md')}` : '',
    ].filter(Boolean));
    ...
    if (errorMessage.includes('Cannot find module') || errorMessage.includes('@ruflo/codex')) {
      output.printError('The @ruflo/codex package is not installed.');
      output.printInfo('Install it with: npm install @ruflo/codex');
      output.writeln();
      output.printInfo('Alternatively, copy skills manually from .codex/skills/ to .agents/skills/');
    } else {
...
function isInitialized(cwd: string): { codex: boolean; codexFlow: boolean } {
  const codexPath = path.join(cwd, '.codex', 'settings.json');
  const codexFlowPath = path.join(cwd, '.codex', 'config.yaml');
  return {
    codex: fs.existsSync(codexPath),
    codexFlow: fs.existsSync(codexFlowPath),
  };
}
...
const initAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  const skipCodex = ctx.flags['skip-codex'] as boolean;
  const onlyCodex = ctx.flags['only-codex'] as boolean;
  ...
  const hasExisting = initialized.codex || initialized.codexFlow;

  if (hasExisting && !force) {
    output.printWarning('RuFlo appears to be already initialized');
    if (initialized.codex) output.printInfo('  Found: .codex/settings.json');
    if (initialized.codexFlow) output.printInfo('  Found: .codex/config.yaml');
  ...
  // Handle --skip-codex and --only-codex flags
  if (skipCodex) {
    ...
    options.components.codexMd = false;
  }
  ...
  // #1744 — opt-out of the user-global ~/.codex/AGENTS.md "Ruflo Integration"
  ...
  if (noGlobal) {
    options.skipGlobalCodexMd = true;
  }
  ...
  try {
    ...
    // Show what was created
    if (options.components.codexMd || options.components.settings || options.components.skills || options.components.commands || options.components.agents) {
      output.printBox(
        [
          options.components.codexMd ? `AGENTS.md:   Swarm guidance & configuration` : '',
          options.components.settings ? `Settings:    .codex/settings.json` : '',
          options.components.skills ? `Skills:      .codex/skills/ (${result.summary.skillsCount} skills)` : '',
          options.components.commands ? `Commands:    .codex/commands/ (${result.summary.commandsCount} commands)` : '',
          options.components.agents ? `Agents:      .codex/agents/ (${result.summary.agentsCount} agents)` : '',
          options.components.helpers ? `Helpers:     .codex/helpers/` : '',
    ...
    if (startDaemon || startAll) {
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @ruflo/cli@latest memory init 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
      ...
      if (startDaemon) {
        try {
          ...
          execSync('npx @ruflo/cli@latest daemon start 2>/dev/null &', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 10000
          });
      ...
      if (startAll) {
        try {
          ...
          execSync('npx @ruflo/cli@latest swarm init --topology hierarchical 2>/dev/null', {
            stdio: 'pipe',
            cwd: ctx.cwd,
            timeout: 30000
          });
    ...
    if (withEmbeddings) {
      ...
      try {
        ...
        execFileInit('npx', [
          '@ruflo/cli@latest', 'embeddings', 'init',
    ...
    if (!startDaemon && !startAll) {
      const bin = (process.argv[1] || '').includes('ruflo') ? 'ruflo' : 'codex';
      output.writeln(output.bold('Next steps:'));
      output.printList([
        ...
        options.components.settings ? `Review ${output.highlight('.codex/settings.json')} for hook configurations` : '',
      ].filter(Boolean));
    }
    ...
    try {
      ...
      if (preset === 'minimal') {
        ...
        const components = await multiSelect({
          ...
          options: [
            { value: 'codexMd', label: 'AGENTS.md', hint: 'Swarm guidance and project configuration', selected: true },
            { value: 'settings', label: 'settings.json', hint: 'Codex hooks configuration', selected: true },
            { value: 'skills', label: 'Skills', hint: 'Codex skills in .codex/skills/', selected: true },
            { value: 'commands', label: 'Commands', hint: 'Codex commands in .codex/commands/', selected: true },
            { value: 'agents', label: 'Agents', hint: 'Agent definitions in .codex/agents/', selected: true },
            { value: 'helpers', label: 'Helpers', hint: 'Utility scripts in .codex/helpers/', selected: true },
            ...
            { value: 'runtime', label: 'Runtime', hint: '.codex/ directory for V3 runtime', selected: true },
          ],
        });

        options.components.codexMd = components.includes('codexMd');
        ...
        if (options.components.settings) {
          const hooks = await multiSelect({
            ...
            options: [
              ...
              { value: 'permissionRequest', label: 'PermissionRequest', hint: 'Auto-allow codex tools', selected: true },
            ],
          });
...
const checkCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const result = {
      initialized: initialized.codex || initialized.codexFlow,
      codex: initialized.codex,
      codexFlow: initialized.codexFlow,
      paths: {
        codexSettings: initialized.codex ? path.join(ctx.cwd, '.codex', 'settings.json') : null,
        codexFlowConfig: initialized.codexFlow ? path.join(ctx.cwd, '.codex', 'config.yaml') : null,
      },
    };
    ...
    if (result.initialized) {
      ...
      if (initialized.codex) {
        output.printInfo(`  Codex: .codex/settings.json`);
      }
      if (initialized.codexFlow) {
        output.printInfo(`  V3 Runtime: .codex/config.yaml`);
      }
    } else {
...
export const initCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'skip-codex',
      description: 'Skip .codex/ directory creation (runtime only)',
      type: 'boolean',
      default: false,
    },
    {
      name: 'only-codex',
      description: 'Only create .codex/ directory (skip runtime)',
      type: 'boolean',
      default: false,
    },
    {
      name: 'no-global',
      description: 'Skip the ~/.codex/AGENTS.md "Ruflo Integration" pointer block (#1744)',
      type: 'boolean',
      default: false,
    },
    {
    ...
    {
      name: 'dual',
      description: 'Initialize for both Codex and OpenAI Codex',
      type: 'boolean',
      default: false,
    },
  ],
  examples: [
    { command: 'codex init', description: 'Initialize with default configuration' },
    { command: 'codex init --start-all', description: 'Initialize and start daemon, memory, swarm' },
    { command: 'codex init --start-daemon', description: 'Initialize and start daemon only' },
    { command: 'codex init --minimal', description: 'Initialize with minimal configuration' },
    { command: 'codex init --full', description: 'Initialize with all components' },
    { command: 'codex init --force', description: 'Reinitialize and overwrite existing config' },
    { command: 'codex init --only-codex', description: 'Only create Codex integration' },
    { command: 'codex init --skip-codex', description: 'Only create V3 runtime' },
    { command: 'codex init wizard', description: 'Interactive setup wizard' },
    { command: 'codex init --with-embeddings', description: 'Initialize with ONNX embeddings' },
    { command: 'codex init --with-embeddings --embedding-model Xenova/all-mpnet-base-v2', description: 'Use larger embedding model' },
    { command: 'codex init skills --all', description: 'Install all available skills' },
    { command: 'codex init hooks --minimal', description: 'Create minimal hooks configuration' },
    { command: 'codex init upgrade', description: 'Update helpers while preserving data' },
    { command: 'codex init upgrade --settings', description: 'Update helpers and merge new settings (Agent Teams)' },
    { command: 'codex init upgrade --verbose', description: 'Show detailed upgrade info' },
    { command: 'codex init --codex', description: 'Initialize for OpenAI Codex (AGENTS.md)' },
    { command: 'codex init --codex --full', description: 'Codex init with all 137+ skills' },
    { command: 'codex init --dual', description: 'Initialize for both Codex and Codex' },
  ],
  action: initAction,
};
