...
function updateSwarmActivityMetrics(agentCountDelta: number): void {
  try {
    const metricsDir = path.join(process.cwd(), '.codex', 'metrics');
...
const spawnCommand: Command = {
  ...
  options: [
    ...
    },
    {
      name: 'provider',
      short: 'p',
      description: 'Provider to use (openai, openrouter, ollama)',
      type: 'string',
      default: 'openai'
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const result = await callMCPTool<{
        ...
        id: agentName,
        config: {
          provider: ctx.flags.provider || 'openai',
...
export const agentCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex agent <subcommand> [options]');
    output.writeln();
