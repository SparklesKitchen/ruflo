...
async function checkConfigFile(): Promise<HealthCheck> {
  ...
  // pre-v3 init flows; v3 init writes only `.codex/config.yaml`.
  const jsonPaths = [
    '.codex/config.json',
    'codex.config.json',
    '.codex.json'
  ...
  return { name: 'Config File', status: 'warn', message: 'No config file (using defaults)', fix: 'codex config init' };
}

// Check daemon status
async function checkDaemonStatus(): Promise<HealthCheck> {
  try {
    const pidFile = '.codex/daemon.pid';
    if (existsSync(pidFile)) {
      ...
      try {
        ...
        return { name: 'Daemon Status', status: 'warn', message: 'Stale PID file', fix: 'rm .codex/daemon.pid && codex daemon start' };
      }
    }
    return { name: 'Daemon Status', status: 'warn', message: 'Not running', fix: 'codex daemon start' };
  } catch {
    return { name: 'Daemon Status', status: 'warn', message: 'Unable to check', fix: 'codex daemon status' };
  }
}
...
async function checkMemoryDatabase(): Promise<HealthCheck> {
  ...
  return { name: 'Memory Database', status: 'warn', message: 'Not initialized', fix: 'codex memory configure --backend hybrid' };
}

// Check API keys
async function checkApiKeys(): Promise<HealthCheck> {
  const keys = ['OPENAI_API_KEY', 'CODEX_API_KEY', 'OPENAI_API_KEY'];
  const found: string[] = [];
  ...
  // Detect Codex environment — API keys are managed internally
  const inCodexCode = !!(process.env.CODEX || process.env.CODEX_PROJECT_DIR || process.env.MCP_SESSION_ID);

  if (found.includes('OPENAI_API_KEY') || found.includes('CODEX_API_KEY')) {
    return { name: 'API Keys', status: 'pass', message: `Found: ${found.join(', ')}` };
  } else if (inCodexCode) {
    return { name: 'API Keys', status: 'pass', message: 'Codex (managed internally)' };
  } else if (found.length > 0) {
    return { name: 'API Keys', status: 'warn', message: `Found: ${found.join(', ')} (no Codex key)`, fix: 'export OPENAI_API_KEY=your_key' };
  } else {
    return { name: 'API Keys', status: 'warn', message: 'No API keys found', fix: 'export OPENAI_API_KEY=your_key' };
  }
}
...
async function checkAIDefence(): Promise<HealthCheck> {
  try {
    ...
    await import('@ruflo/aidefence');
    return {
      name: 'AIDefence',
      status: 'pass',
      message: '@ruflo/aidefence loadable — aidefence_* MCP tools functional',
    };
  } catch {
    return {
      name: 'AIDefence',
...
async function checkMcpServers(): Promise<HealthCheck> {
  ...
  // #1842: ~/.codex.json holds project-scoped registrations under
  ...
  const mcpConfigPaths = [
    ...
    join(home, '.codex/codex_desktop_config.json'),
    join(home, '.config/codex/mcp.json'),
    '.mcp.json',
  ];

  const isRufloKey = (k: string) =>
    k === 'ruflo' || k === 'ruflo_alpha' || k === 'codex' || k === 'codex_alpha';
  ...
  for (const configPath of mcpConfigPaths) {
    ...
    try {
      ...
      // Project-scoped (Codex shape): projects[*].mcpServers.ruflo
      let projectHits = 0;
      ...
      if (totalServers > 0) {
        return {
          ...
          fix: 'codex mcp add ruflo -- npx -y ruflo@latest mcp start',
        };
      }
    } catch {
...
async function checkVersionFreshness(): Promise<HealthCheck> {
  try {
    ...
    try {
      ...
      // package.json that belongs to @ruflo/cli (or codex/cli).
        ...
        try {
          if (existsSync(candidate)) {
            ...
            if (
              ...
              (pkg.name === '@ruflo/cli' || pkg.name === 'codex' || pkg.name === 'ruflo')
            ) {
    ...
    try {
      const npmInfo = await runCommand('npm view @ruflo/cli@alpha version', 5000);
    ...
    if (isOutdated) {
      const fix = isNpx
        ? 'rm -rf ~/.npm/_npx/* && npx -y @ruflo/cli@latest'
        : 'npm update @ruflo/cli';
...
// Check Codex CLI (async with proper env inheritance)
async function checkCodexCode(): Promise<HealthCheck> {
  try {
    const version = await runCommand('codex --version');
    // Parse version from output like "codex 1.0.0" or "Codex v1.0.0"
    const versionMatch = version.match(/v?(\d+\.\d+\.\d+)/);
    const versionStr = versionMatch ? `v${versionMatch[1]}` : version;
    return { name: 'Codex CLI', status: 'pass', message: versionStr };
  } catch {
    return {
      name: 'Codex CLI',
      status: 'warn',
      message: 'Not installed',
      fix: 'npm install -g @openai-ai/codex-code'
    };
  }
}
...
async function installCodexCode(): Promise<boolean> {
  try {
    output.writeln();
    output.writeln(output.bold('Installing Codex CLI...'));
    execSync('npm install -g @openai-ai/codex-code', {
      encoding: 'utf8',
      stdio: 'inherit'
    });
    output.writeln(output.success('Codex CLI installed successfully!'));
    return true;
  } catch (error) {
    output.writeln(output.error('Failed to install Codex CLI'));
...
// Check agentic v3 integration (filesystem-based to avoid slow WASM/DB init)
async function checkAgenticFlow(): Promise<HealthCheck> {
  try {
    // Walk common node_modules paths to find agentic/package.json
    const candidates = [
      join(process.cwd(), 'node_modules', 'agentic', 'package.json'),
      join(process.cwd(), '..', 'node_modules', 'agentic', 'package.json'),
    ];
    ...
    if (!pkgJsonPath) {
      return {
        ...
        fix: 'npm install agentic@latest'
      };
    }
    ...
    return { name: 'agentic', status: 'warn', message: 'Check failed' };
  }
}
...
//   1. Gate status — is RUFLO_ENCRYPT_AT_REST set?
//   2. Key resolution — does RUFLO_ENCRYPTION_KEY resolve to a valid
...
async function checkEncryptionAtRest(): Promise<HealthCheck> {
  if (!isEncryptionEnabled()) {
    return {
      ...
      fix: 'export RUFLO_ENCRYPT_AT_REST=1 && export RUFLO_ENCRYPTION_KEY=<64-char-hex>',
    };
  }

  // Gate is on — try to resolve the key. Fail-closed if missing or malformed.
  const rawKey = process.env.RUFLO_ENCRYPTION_KEY;
  if (!rawKey) {
    return {
      name: 'Encryption at Rest',
      status: 'fail',
      message: 'Gate is on but RUFLO_ENCRYPTION_KEY is unset (fail-closed)',
      fix: 'Generate a key: openssl rand -hex 32 → export RUFLO_ENCRYPTION_KEY=<value>',
    };
  }
  ...
  try {
    ...
    return {
      name: 'Encryption at Rest',
      status: 'fail',
      message: `RUFLO_ENCRYPTION_KEY invalid: ${err instanceof Error ? err.message : String(err)}`,
  ...
  const stores: Array<{ label: string; path: string }> = [
    { label: 'sessions/', path: join(cwd, '.codex', 'sessions') },
    { label: 'terminals', path: join(cwd, '.codex', 'terminals', 'store.json') },
...
export const doctorCommand: Command = {
  ...
  options: [
    ...
    },
    {
      name: 'install',
      short: 'i',
      description: 'Auto-install missing dependencies (Codex CLI)',
      type: 'boolean',
      default: false
    },
    {
      name: 'component',
      short: 'c',
      description: 'Check specific component (version, node, npm, config, daemon, memory, api, git, mcp, codex, disk, typescript)',
      type: 'string'
    },
    {
      name: 'verbose',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (autoInstall) {
      const codexCodeResult = results.find(r => r.name === 'Codex CLI');
      if (codexCodeResult && codexCodeResult.status !== 'pass') {
        const installed = await installCodexCode();
        if (installed) {
          // Re-check Codex after installation
          const newCheck = await checkCodexCode();
          const idx = results.findIndex(r => r.name === 'Codex CLI');
          if (idx !== -1) {
            results[idx] = newCheck;
            // Update fixes list
            const fixIdx = fixes.findIndex(f => f.startsWith('Codex CLI:'));
