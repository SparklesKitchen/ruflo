...
services:
  postgres:
    ...
    environment:
      POSTGRES_USER: codex
      POSTGRES_PASSWORD: codex-test
      POSTGRES_DB: codex_flow
    ports:
      - "5432:5432"
    volumes:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U codex -d codex_flow"]
      interval: 5s
      timeout: 5s
      retries: 10
    command: >
  ...
  pgadmin:
    ...
    environment:
      PGADMIN_DEFAULT_EMAIL: admin@codex.local
...
-- from ruvnet/ruvector-postgres with Codex-Flow V3 integration.
--
...
-- Create the codex_flow schema
CREATE SCHEMA IF NOT EXISTS codex_flow;

-- Grant permissions
GRANT ALL ON SCHEMA codex_flow TO codex;

-- Set search path
SET search_path TO codex_flow, public;
...
CREATE TABLE IF NOT EXISTS codex_flow.embeddings (
...
CREATE TABLE IF NOT EXISTS codex_flow.patterns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS codex_flow.agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS codex_flow.trajectories (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Memory entries table (main storage for Codex-Flow memory)
CREATE TABLE IF NOT EXISTS codex_flow.memory_entries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS codex_flow.hyperbolic_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    content TEXT NOT NULL,
    ...
    parent_id UUID REFERENCES codex_flow.hyperbolic_embeddings(id),
    metadata JSONB DEFAULT '{}',
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Graph nodes for GNN operations
CREATE TABLE IF NOT EXISTS codex_flow.graph_nodes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS codex_flow.graph_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    source_id UUID REFERENCES codex_flow.graph_nodes(id),
    target_id UUID REFERENCES codex_flow.graph_nodes(id),
...
ON codex_flow.graph_nodes
USING hnsw (embedding ruvector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Additional indices for common queries
CREATE INDEX IF NOT EXISTS idx_embeddings_namespace ON codex_flow.embeddings(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_namespace ON codex_flow.memory_entries(namespace);
CREATE INDEX IF NOT EXISTS idx_memory_entries_key ON codex_flow.memory_entries(key);
...
CREATE OR REPLACE FUNCTION codex_flow.search_similar(
...
CREATE OR REPLACE FUNCTION codex_flow.search_memory(
    query_embedding ruvector(384),
...
BEGIN
    ...
    SELECT
        ...
        m.metadata
    FROM codex_flow.memory_entries m
...
CREATE OR REPLACE FUNCTION codex_flow.search_patterns(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION codex_flow.find_agents(
    query_embedding ruvector(384),
...
CREATE OR REPLACE FUNCTION codex_flow.to_poincare(
    euclidean real[],
    curvature FLOAT DEFAULT -1.0
)
RETURNS real[] AS $$
...
CREATE OR REPLACE FUNCTION codex_flow.hyperbolic_search(
    query ruvector(384),
    limit_count INT DEFAULT 10,
    curvature FLOAT DEFAULT -1.0
)
...
BEGIN
    ...
    query_poincare := codex_flow.to_poincare(query_arr, curvature);
    ...
    SELECT
        ...
        he.hierarchy_level,
        he.metadata
    FROM codex_flow.hyperbolic_embeddings he
...
CREATE OR REPLACE FUNCTION codex_flow.ruvector_info()
RETURNS TABLE (
    version TEXT,
    simd_info TEXT
) AS $$
...
-- L2 distance helper
CREATE OR REPLACE FUNCTION codex_flow.l2_distance(
    a ruvector,
    b ruvector
)
RETURNS FLOAT AS $$
...
-- Upsert memory entry
CREATE OR REPLACE FUNCTION codex_flow.upsert_memory(
    p_key VARCHAR(255),
    p_value TEXT,
...
DECLARE
    v_id UUID;
BEGIN
    INSERT INTO codex_flow.memory_entries (key, value, embedding, namespace, metadata, ttl, updated_at)
    VALUES (p_key, p_value, p_embedding, p_namespace, p_metadata, p_ttl, NOW())
    ON CONFLICT (key, namespace) DO UPDATE SET
        value = EXCLUDED.value,
        embedding = COALESCE(EXCLUDED.embedding, codex_flow.memory_entries.embedding),
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'Schema: codex_flow';
    ...
    RAISE NOTICE '  - codex_flow.search_similar(embedding, limit, min_sim)';
    RAISE NOTICE '  - codex_flow.search_memory(embedding, namespace, limit)';
    RAISE NOTICE '  - codex_flow.search_patterns(embedding, type, limit)';
    RAISE NOTICE '  - codex_flow.find_agents(embedding, type, limit)';
    RAISE NOTICE '  - codex_flow.hyperbolic_search(embedding, limit, curvature)';
    RAISE NOTICE '  - codex_flow.upsert_memory(key, value, embedding, namespace)';
    RAISE NOTICE '';
...
This directory contains the Docker configuration for RuVector PostgreSQL with Codex-Flow V3.
...
docker exec ruvector-postgres psql -U codex -d codex_flow -c "SELECT ruvector_version();"
\`\`\`
...
\`\`\`bash
# Export current Codex-Flow memory
...
- [Codex-Flow Documentation](https://github.com/SparklesKitchen/ruflo)
`;
