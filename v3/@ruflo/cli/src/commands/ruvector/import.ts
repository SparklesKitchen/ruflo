...
function generateInsertSQL(entry: MemoryEntry): string {
  ...
  return `INSERT INTO codex_flow.memory_entries (key, value, embedding, namespace, metadata, created_at, updated_at)
VALUES (
  '${key}',
  '${value}',
  ${embeddingClause},
...
export const importCommand: Command = {
  ...
  options: [
    ...
    {
      name: 'from-memory',
      description: 'Export from current Codex-Flow memory and import',
      type: 'boolean',
      default: false,
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!inputFile && !fromMemory) {
      ...
      output.writeln('  codex ruvector import --input memory-export.json');
      output.writeln('  codex ruvector import --from-memory');
    ...
    if (fromMemory) {
      output.printInfo('Exporting from current Codex-Flow memory...');
    ...
    if (outputFile) {
      try {
        ...
        output.writeln(`  docker exec -i ${containerName} psql -U codex -d codex_flow < ${outputFile}`);
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
      ...
      try {
        ...
        output.writeln(output.dim(`  docker exec -i ${containerName} psql -U codex -d codex_flow < ${tempFile}`));
        output.writeln();
        ...
        try {
          ...
          output.writeln(`  docker exec -i ${containerName} psql -U codex -d codex_flow < ${tempFile}`);
    ...
    output.writeln(`  docker exec ${containerName} psql -U codex -d codex_flow -c "SELECT COUNT(*) FROM codex_flow.memory_entries;"`);
    output.writeln();
