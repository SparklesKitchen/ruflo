...
async function getEmbeddings() {
  try {
    return await import('@ruflo/embeddings');
  } catch {
    return null;
  }
}
...
const generateCommand: Command = {
  ...
  options: [
    ...
    { name: 'provider', short: 'p', type: 'string', description: 'Provider: openai, transformers, agentic, local', default: 'local' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (!fs.existsSync(fullDbPath)) {
        ...
        output.printInfo('Run: codex memory init');
        return { success: false, exitCode: 1 };
      }
      ...
      if (topResults.length === 0) {
        ...
        output.printInfo(`Try: codex memory store -k "key" --value "your data"`);
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (collections.length === 0) {
        ...
        output.writeln(output.highlight('  codex memory store -k "key" --value "data"'));
        return { success: true, data: [] };
      }
    ...
    try {
      ...
      if (action === 'status') {
        ...
        if (status.available && status.entryCount > 0) {
          ...
          output.printInfo('Run: codex memory store -k "key" --value "text"');
        } else {
          output.writeln();
...
const initCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Create directories
      const configDir = path.join(process.cwd(), '.codex');
      ...
      if (download) {
        ...
        if (embeddings) {
          ...
          output.writeln(output.dim('  (Skipped — @ruflo/embeddings not installed)'));
        }
      }
...
const chunkCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!embeddings) {
      output.printWarning('@ruflo/embeddings not installed, showing preview');
      output.writeln();
      output.printBox([
        `Strategy: ${strategy}`,
...
const hyperbolicCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const hyperbolic = await import('@ruflo/embeddings').then(m => m).catch(() => null);

      if (!hyperbolic || !hyperbolic.euclideanToPoincare) {
        output.printWarning('@ruflo/embeddings hyperbolic module not available');
        output.printInfo('Install with: npm install @ruflo/embeddings');
        return { success: false, exitCode: 1 };
      }
...
const neuralCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const fs = await import('fs');
    const path = await import('path');
    const configPath = path.join(process.cwd(), '.codex', 'embeddings.json');
    ...
    if (!neuralConfig.enabled) {
      ...
      output.writeln(output.dim('Configuration: .codex/embeddings.json'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (download) {
      ...
      if (embeddings) {
        ...
        await new Promise(r => setTimeout(r, 500));
        spinner.succeed(`Download skipped — @ruflo/embeddings not installed`);
      }
      return { success: true };
    }
