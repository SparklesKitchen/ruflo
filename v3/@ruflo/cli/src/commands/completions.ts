...
function generateBashCompletion(): string {
  return `# codex bash completion
# Generated by codex completions bash
# Add this to ~/.bashrc or ~/.bash_completion

_codex_flow_completions() {
...
complete -F _codex_flow_completions codex
complete -F _codex_flow_completions npx\\ @ruflo/cli@v3alpha
`;
}

// Generate zsh completion script
function generateZshCompletion(): string {
  return `#compdef codex
# codex zsh completion
# Generated by codex completions zsh
# Add to ~/.zfunc/_codex or ~/.zsh/completions/_codex
...
function generateFishCompletion(): string {
  return `# codex fish completion
# Generated by codex completions fish
# Save to ~/.config/fish/completions/codex.fish

# Disable file completion by default
complete -c codex -f

# Top-level commands
${TOP_LEVEL_COMMANDS.map(cmd => `complete -c codex -n "__fish_use_subcommand" -a "${cmd}"`).join('\n')}

# Swarm subcommands
${SWARM_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from swarm" -a "${sub}"`).join('\n')}

# Agent subcommands
${AGENT_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from agent" -a "${sub}"`).join('\n')}

# Task subcommands
${TASK_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from task" -a "${sub}"`).join('\n')}

# Memory subcommands
${MEMORY_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from memory" -a "${sub}"`).join('\n')}

# Hive-mind subcommands
${HIVE_MIND_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from hive-mind hive" -a "${sub}"`).join('\n')}

# Hooks subcommands
${HOOKS_SUBCOMMANDS.map(sub => `complete -c codex -n "__fish_seen_subcommand_from hooks" -a "${sub}"`).join('\n')}

# Neural subcommands
complete -c codex -n "__fish_seen_subcommand_from neural" -a "train status patterns predict optimize"

# Security subcommands
complete -c codex -n "__fish_seen_subcommand_from security" -a "scan cve threats audit secrets"

# Performance subcommands
complete -c codex -n "__fish_seen_subcommand_from performance" -a "benchmark profile metrics optimize bottleneck"

# Plugins subcommands
complete -c codex -n "__fish_seen_subcommand_from plugins" -a "list install uninstall toggle info create"

# Deployment subcommands
complete -c codex -n "__fish_seen_subcommand_from deployment deploy" -a "deploy status rollback history environments logs"

# Claims subcommands
complete -c codex -n "__fish_seen_subcommand_from claims" -a "list check grant revoke roles policies"

# Embeddings subcommands
complete -c codex -n "__fish_seen_subcommand_from embeddings" -a "generate search compare collections index providers"

# Daemon subcommands
complete -c codex -n "__fish_seen_subcommand_from daemon" -a "start stop status trigger enable"

# Providers subcommands
complete -c codex -n "__fish_seen_subcommand_from providers" -a "list configure test models usage"
`;
}

// Generate PowerShell completion script
function generatePowerShellCompletion(): string {
  return `# codex PowerShell completion
# Generated by codex completions powershell
# Add to $PROFILE or save to a separate file and dot-source it

$script:CodexFlowCommands = @(
...
Register-ArgumentCompleter -Native -CommandName codex -ScriptBlock {
    ...
    if ($tokens.Count -eq 2) {
        ...
        $script:CodexFlowCommands | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {
...
export const completionsCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'codex completions bash > ~/.bash_completion.d/codex', description: 'Install bash completions' },
    { command: 'codex completions zsh > ~/.zfunc/_codex', description: 'Install zsh completions' },
    { command: 'codex completions fish > ~/.config/fish/completions/codex.fish', description: 'Install fish completions' },
    { command: 'codex completions powershell >> $PROFILE', description: 'Install PowerShell completions' }
  ],
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  codex completions bash > ~/.bash_completion.d/codex'));
    output.writeln(output.dim('  source ~/.bash_completion.d/codex'));
    output.writeln();
    output.writeln(output.bold('Zsh:'));
    output.writeln(output.dim('  mkdir -p ~/.zfunc'));
    output.writeln(output.dim('  codex completions zsh > ~/.zfunc/_codex'));
    output.writeln(output.dim('  # Add to ~/.zshrc: fpath=(~/.zfunc $fpath); autoload -Uz compinit && compinit'));
    output.writeln();
    output.writeln(output.bold('Fish:'));
    output.writeln(output.dim('  codex completions fish > ~/.config/fish/completions/codex.fish'));
    output.writeln();
    output.writeln(output.bold('PowerShell:'));
    output.writeln(output.dim('  codex completions powershell >> $PROFILE'));
