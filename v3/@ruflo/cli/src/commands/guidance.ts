...
/** Task intent categories matching @ruflo/guidance TaskIntent */
type TaskIntent = 'bug-fix' | 'feature' | 'refactor' | 'security' | 'performance' | 'testing' | 'docs' | 'deployment' | 'architecture' | 'debug' | 'general';

/** Gate evaluation result matching @ruflo/guidance GateResult */
...
const compileCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { GuidanceCompiler } = await import('@ruflo/guidance/compiler');
...
const retrieveCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { GuidanceCompiler } = await import('@ruflo/guidance/compiler');
      const { ShardRetriever, HashEmbeddingProvider } = await import('@ruflo/guidance/retriever');
...
const gatesCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { EnforcementGates } = await import('@ruflo/guidance/gates');
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const localExists = existsSync('./.codex/AGENTS.override.md');

      const statusData = {
        rootGuidance: rootExists ? 'found' : 'not found',
        localOverlay: localExists ? 'found' : 'not configured',
        dataDir: existsSync('./.codex/guidance') ? 'exists' : 'not created',
      };
      ...
      if (jsonOutput) {
        ...
        output.writeln(`  Local overlay:  ${localExists ? output.success('.codex/AGENTS.override.md found') : output.dim('not configured')}`);
...
const optimizeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      const { analyze, formatReport, optimizeForSize, formatBenchmark } = await import('@ruflo/guidance/analyzer');
...
const abTestCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const { readFile } = await import('node:fs/promises');
      const { abBenchmark, getDefaultABTasks } = await import('@ruflo/guidance/analyzer');
...
export const guidanceCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim('Use codex guidance <subcommand> --help for details'));
