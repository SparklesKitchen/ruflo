...
const searchCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // @ruflo/memory don't expose it. Fall through to plain
      ...
      if (useSmart) {
        try {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const memMod: any = await import('@ruflo/memory');
        ...
        if (!smartSearchFn) {
          output.printWarning(
            'Smart search requested but smartSearch is not available on the installed @ruflo/memory build (#1846). Falling back to standard semantic search.',
          );
        }
      }
      ...
      if (results.length === 0) {
        ...
        output.writeln(output.dim('Try: codex memory store -k "key" --value "data"'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        output.printInfo('Store data: codex memory store -k "key" --value "data"');
        return { success: true, data: [] };
      }
...
const initMemoryCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printList([
        `Store data: ${output.highlight('codex memory store -k "key" --value "data"')}`,
        `Search: ${output.highlight('codex memory search -q "query"')}`,
        `Train patterns: ${output.highlight('codex neural train -p coordination')}`,
        `View stats: ${output.highlight('codex memory stats')}`
      ]);

      // Also sync to .codex directory
      const fs = await import('fs');
      const path = await import('path');
      const codexDir = path.join(process.cwd(), '.codex');
      const codexDbPath = path.join(codexDir, 'memory.db');

      if (!fs.existsSync(codexDir)) {
        fs.mkdirSync(codexDir, { recursive: true });
      }

      if (fs.existsSync(result.dbPath) && (!fs.existsSync(codexDbPath) || force)) {
        fs.copyFileSync(result.dbPath, codexDbPath);
        output.writeln();
        output.writeln(output.dim(`Synced to: ${codexDbPath}`));
      }
...
export const memoryCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex memory <subcommand> [options]');
    output.writeln();
