...
function isInitialized(cwd: string): boolean {
  const configPath = path.join(cwd, '.codex', 'config.yaml');
...
const startAction = async (ctx: CommandContext): Promise<CommandResult> => {
  ...
  try {
    ...
    output.printList([
      `${output.highlight('codex status')} - View system status`,
      `${output.highlight('codex agent spawn -t coder')} - Spawn an agent`,
      `${output.highlight('codex swarm status')} - View swarm details`,
      `${output.highlight('codex stop')} - Stop the system`
    ]);

    // Daemon mode
    if (daemon) {
      output.writeln();
      output.printInfo('Running in daemon mode. Use "codex stop" to stop.');

      // Store PID for daemon management
      const daemonPidPath = path.join(cwd, '.codex', 'daemon.pid');
...
const stopCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      // Clean up daemon PID
      const daemonPidPath = path.join(ctx.cwd, '.codex', 'daemon.pid');
      if (fs.existsSync(daemonPidPath)) {
