...
function getSwarmStatus(swarmId?: string) {
  ...
  const sessionDir = path.join(process.cwd(), '.codex', 'sessions');
  const memoryPaths = [
...
const startCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  - Codex Agent tool (interactive)'));
    output.writeln(output.dim('  - codex -p (headless background)'));
    output.writeln(output.dim('  - hive-mind spawn --codex (autonomous)'));
    output.writeln(output.dim(`  Monitor: codex swarm status ${swarmId}`));
...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!status.hasActiveSwarm) {
      ...
      output.writeln(output.dim('  npx @ruflo/cli@latest swarm init'));
      output.writeln(output.dim('  npx @ruflo/cli@latest swarm start'));
      output.writeln();
      return { success: true, data: status };
    }
...
const coordinateCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(output.dim('Note: Use Codex Task tool or hive-mind spawn --codex to'));
...
export const swarmCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex swarm <subcommand> [options]');
    output.writeln();
