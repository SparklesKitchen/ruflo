...
/**
 * Ruflo-owned subdirectories within .codex/ that are safe to delete.
 * Everything else in .codex/ (agents, skills, commands, settings.local.json,
 * memory.db, worktrees, launch.json) belongs to Codex and must be preserved.
 * See: https://github.com/ruvnet/ruflo/issues/1557
 */
const CODEX_OWNED_SUBDIRS = [
  { path: join('.codex', 'helpers'), description: 'Ruflo hook scripts' },
];
...
const ARTIFACT_FILES = [
  { path: 'codex.config.json', description: 'Ruflo configuration' },
];
...
export const cleanupCommand: Command = {
  ...
  options: [
    ...
    },
    {
      name: 'keep-config',
      short: 'k',
      description: 'Preserve codex.config.json and .codex/settings.json',
      type: 'boolean',
      default: false,
    },
  ],
  examples: [
    ...
    },
    {
      ...
      description: 'Remove all codex artifacts',
    },
    {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Scan ruflo-owned subdirs within .codex/ (surgical — preserves Codex files)
    for (const artifact of CODEX_OWNED_SUBDIRS) {
    ...
    // Check if .codex/settings.json has ruflo hooks/codexFlow blocks to clean
    const settingsPath = join(cwd, '.codex', 'settings.json');
    if (existsSync(settingsPath)) {
      found.push({ path: join('.codex', 'settings.json'), description: 'Remove ruflo hooks/codexFlow blocks (preserves rest)', size: 0, type: 'file' });
    }
    ...
    if (found.length === 0) {
      output.writeln(output.info('No codex artifacts found in the current directory.'));
    ...
    for (const item of found) {
      ...
      if (dryRun) {
        ...
        try {
          ...
          if (item.path === join('.codex', 'settings.json')) {
            try {
              const raw = JSON.parse(readFileSync(fullPath, 'utf-8'));
              delete raw.hooks;
              delete raw.codexFlow;
