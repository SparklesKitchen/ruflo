...
const providersCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const defaultProviders = [
      { name: 'openai', model: 'codex-3-5-sonnet-20241022', priority: 1, enabled: true, status: 'Active' },
      { name: 'openrouter', model: 'codex-3.5-sonnet', priority: 2, enabled: false, status: 'Disabled' },
...
const exportCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      const exportPath = (ctx.flags.output as string) || ctx.args[0] || 'codex.config.export.json';
...
export const configCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex config <subcommand> [options]');
    output.writeln();
