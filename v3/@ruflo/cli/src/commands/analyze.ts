...
export const analyzeCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln(`  ${output.dim('codex analyze ast src/')}                  # Full AST analysis`);
    output.writeln(`  ${output.dim('codex analyze ast src/index.ts -c')}       # Include complexity`);
    output.writeln(`  ${output.dim('codex analyze complexity src/ -t 15')}     # Flag high complexity`);
    output.writeln(`  ${output.dim('codex analyze symbols src/ --type fn')}    # Extract functions`);
    output.writeln(`  ${output.dim('codex analyze imports src/ --external')}   # Only npm imports`);
    output.writeln();

    output.writeln(output.bold('Graph Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('codex analyze boundaries src/')}            # Find natural code boundaries`);
    output.writeln(`  ${output.dim('codex analyze modules src/')}               # Detect module communities`);
    output.writeln(`  ${output.dim('codex analyze dependencies -f dot src/')}   # Export to DOT format`);
    output.writeln(`  ${output.dim('codex analyze circular src/')}              # Find circular deps`);
    output.writeln();

    output.writeln(output.bold('Diff Analysis Examples:'));
    output.writeln();
    output.writeln(`  ${output.dim('codex analyze diff --risk')}              # Risk assessment`);
    output.writeln(`  ${output.dim('codex analyze diff HEAD~1 --classify')}   # Classify changes`);
    output.writeln(`  ${output.dim('codex analyze diff main..feature')}       # Compare branches`);
    output.writeln();
