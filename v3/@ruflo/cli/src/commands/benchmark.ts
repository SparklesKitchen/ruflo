...
const pretrainCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (saveFile) {
        const resultsDir = join(process.cwd(), '.codex', 'benchmarks');
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      try {
        ...
        // @ruflo/memory not available — return null metrics instead of fake numbers
...
export const benchmarkCommand: Command = {
  ...
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln('  codex benchmark pretrain -i 200');
    output.writeln('  codex benchmark all --save results.json');
    output.writeln();
