 ...
 *
 * Updated to support --codex flag for launching interactive Codex sessions
 * PR: Fix #955 - Implement --codex flag for hive-mind spawn command
 */
...
/**
 * Generate comprehensive Hive Mind prompt for Codex
...
• Do NOT use Codex native Task/Agent tools for swarm coordination — use mcp__ruflo__agent_spawn, mcp__ruflo__task_assign, etc.
• Native Codex tools (Read, Write, Edit, Bash, Grep, Glob) should ONLY be used for file operations and shell commands
...
/**
 * Spawn Codex with Hive Mind coordination instructions
 * Ported from v2.7.47 spawnCodexCodeInstances function
 */
async function spawnCodexCodeInstance(
  swarmId: string,
  swarmName: string,
  objective: string,
  workers: HiveWorker[],
  ...
  output.writeln();
  output.writeln(output.bold('🚀 Launching Codex with Hive Mind Coordination'));
  ...
  try {
    ...
    output.printList([
      ...
      `MCP Tools: ${output.success('Full Codex-Flow integration enabled')}`
    ]);
    ...
    try {
      ...
      output.writeln();
      output.printWarning('Codex CLI not found in PATH');
      output.writeln(output.dim('Install it with: npm install -g @openai-ai/codex-code'));
      output.writeln(output.dim('Falling back to displaying instructions...'));
    }

    const dryRun = flags.dryRun || flags['dry-run'];

    if (codexAvailable && !dryRun) {
      ...
      //   3. ~/.codex.json or ~/.codex/mcp.json (user-global)
      ...
      if (!mcpConfigPath) {
        const candidates = [
          ...
          join(process.env.HOME || process.env.USERPROFILE || '', '.codex.json'),
          join(process.env.HOME || process.env.USERPROFILE || '', '.codex', 'mcp.json'),
        ];
      ...
      if (mcpConfigPath) {
        // #1780 — Codex's `--mcp-config` is variadic; passing it as two
        ...
        codexArgs.push(`--mcp-config=${mcpConfigPath}`);
        output.printInfo(`Spawned worker MCP config: ${mcpConfigPath}`);
      } else {
        output.printWarning('No .mcp.json or ~/.codex.json found — spawned worker will not have mcp__ruflo__* tools (#1748 Issue 2). Pass --mcp-config <path> or run "ruflo init" to generate one.');
      }

      // Check for non-interactive mode
      const isNonInteractive = flags['non-interactive'] || flags.nonInteractive;
      if (isNonInteractive) {
        codexArgs.push('-p'); // Print mode
      ...
      output.writeln();
      output.printInfo('Launching Codex...');
      output.writeln(output.dim('Press Ctrl+C to pause the session'));

      // Spawn codex with properly ordered arguments
      ...
      const sigintHandler = () => {
        ...
        output.writeln();
        output.writeln();
        output.printWarning('Pausing session and terminating Codex...');
        ...
        output.writeln(output.dim('To resume, run codex with the saved prompt file'));
      ...
      codexProcess.on('exit', (code) => {
        ...
        if (code === 0) {
          output.writeln();
          output.printSuccess('Codex completed successfully');
        } else if (code !== null) {
          output.writeln();
          output.printError(`Codex exited with code ${code}`);
        }
      });

      output.writeln();
      output.printSuccess('Codex launched with Hive Mind coordination');
      ...
      output.writeln();
      output.printInfo('Dry run - would execute Codex with prompt:');
      ...
      output.printList([
        'Install Codex: npm install -g @openai-ai/codex-code',
        `Run with saved prompt: codex < ${promptFile}`,
        `Or copy manually: cat ${promptFile} | codex`,
        `With auto-permissions: codex --dangerously-skip-permissions < ${promptFile}`
      ]);
    ...
    try {
      ...
      output.writeln(output.dim('You can run Codex manually with the saved prompt'));
...
const initCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim('  Use "codex hive-mind spawn" to add workers'));
      output.writeln(output.dim('  Use "codex hive-mind spawn --codex" to launch Codex'));
...
// Spawn subcommand - UPDATED with --codex flag
const spawnCommand: Command = {
  name: 'spawn',
  description: 'Spawn worker agents into the hive (use --codex to launch Codex)',
  options: [
    {
      name: 'count',
      short: 'n',
    ...
    // NEW: --codex flag for launching Codex
    {
      name: 'codex',
      description: 'Launch Codex with hive-mind coordination prompt',
      type: 'boolean',
      default: false
    },
    {
      name: 'objective',
      short: 'o',
      description: 'Objective for the hive mind (used with --codex)',
      type: 'string'
    },
    {
      name: 'dangerously-skip-permissions',
      description: 'Skip permission prompts in Codex (use with caution)',
      type: 'boolean',
      default: true
    },
    {
    ...
    {
      name: 'dry-run',
      description: 'Show what would be done without launching Codex',
      type: 'boolean',
      default: false
    },
    {
    ...
    {
      name: 'mcp-config',
      description: 'Path to .mcp.json for the spawned worker (auto-detects ./.mcp.json or ~/.codex.json if omitted) — fixes #1748 Issue 2',
      type: 'string'
    }
  ],
  examples: [
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const launchCodex = ctx.flags.codex as boolean;
    ...
    try {
      ...
      if (ctx.flags.format === 'json' && !launchCodex) {
        output.printJson(result);
        return { success: true, data: result };
      }
      ...
      if (launchCodex) {
        ...
        if (!objective && ctx.interactive) {
          objective = await input({
            ...
            validate: (v) => v.length > 0 || 'Objective is required when using --codex'
          });
        }
        ...
        const codexResult = await spawnCodexCodeInstance(
          swarmId,
          swarmName,
          objective,
          workers,
        ...
        if (!codexResult.success) {
          return { success: false, exitCode: 1, data: { spawn: result, codex: codexResult } };
        }

        return { success: true, data: { spawn: result, codex: codexResult } };
      }
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (workerData.length === 0) {
        output.printInfo('No workers in hive. Use "codex hive-mind spawn" to add workers.');
      } else {
        output.printTable({
          columns: [
...
const taskCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.writeln(output.dim(`  Track with: codex hive-mind task-status ${result.taskId}`));
...
export const hiveMindCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex hive-mind <subcommand> [options]');
    output.writeln();
    output.writeln('Subcommands:');
    output.printList([
      `${output.highlight('init')}            - Initialize hive mind`,
      `${output.highlight('spawn')}           - Spawn worker agents (use --codex to launch Codex)`,
    ...
    output.printList([
      ...
      output.success('NEW: --codex flag to launch interactive Codex sessions')
    ]);
    output.writeln();
    output.writeln('Quick Start with Codex:');
    output.writeln(output.dim('  codex hive-mind init'));
    output.writeln(output.dim('  codex hive-mind spawn -n 5 --codex -o "Your objective here"'));
