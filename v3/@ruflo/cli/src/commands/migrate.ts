...
const statusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Check v2 config: codex.config.json with version "2" or missing version
    const v2ConfigPath = path.join(cwd, 'codex.config.json');
    const v3ConfigDir = path.join(cwd, '.codex');
    ...
    const migrationStatePath = path.join(cwd, '.codex', 'migration-state.json');
...
const runCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const v3Dir = path.join(cwd, '.codex');
      ...
      if (fs.existsSync(v2ConfigPath)) {
        ...
        if (parsed.version === '2' || parsed.version === 2 || !parsed.version) {
          if (dryRun) {
            ...
            // Backup
            if (!skipBackup) {
              fs.copyFileSync(v2ConfigPath, path.join(backupDir, 'codex.config.json'));
            }
...
const rollbackCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      // Restore config
      const backupConfig = path.join(backupPath, 'codex.config.json');
      if (fs.existsSync(backupConfig)) {
        const destConfig = path.join(cwd, 'codex.config.json');
    ...
    const changes = [
      {
        ...
        changes: [
          { change: 'Config file renamed', from: 'codex.json', to: 'codex.config.json' },
          { change: 'Swarm config restructured', from: 'swarm.mode', to: 'swarm.topology' },
          { change: 'Provider config format', from: 'provider: "openai"', to: 'providers: [...]' }
        ]
      },
      {
        category: 'Memory',
        changes: [
          ...
          { change: 'Storage path changed', from: '.codex/memory', to: 'data/memory' }
        ]
      },
      {
        category: 'CLI',
...
export const migrateCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    output.writeln();
    output.writeln('Usage: codex migrate <subcommand> [options]');
    output.writeln();
...
function getMigrationSteps(target: string): Array<{ name: string; description: string; source: string; dest: string }> {
  const allSteps = [
    { name: 'Configuration Files', description: 'Migrate config schema to V3 format', source: './codex.json', dest: './codex.config.json' },
    { name: 'Memory Backend', description: 'Upgrade to hybrid backend with AgentDB', source: './.codex/memory', dest: './data/memory' },
    { name: 'Agent Definitions', description: 'Convert agent configs to V3 format', source: './.codex/agents', dest: './v3/agents' },
    { name: 'Hook Registry', description: 'Migrate hooks to V3 hook system', source: './src/hooks', dest: './v3/hooks' },
    { name: 'Workflow Definitions', description: 'Convert workflows to event-sourced format', source: './.codex/workflows', dest: './data/workflows' },
    { name: 'Embeddings System', description: 'Migrate to ONNX with hyperbolic (Poincaré ball)', source: 'OpenAI/TF.js embeddings', dest: '.codex/embeddings.json' }
  ];
