  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      if (existingPlugin) {
        ...
        output.writeln();
        output.writeln(output.dim('Use "codex plugins upgrade -n ' + name + '" to update'));
        return { success: false, exitCode: 1 };
      }
...
export const pluginsCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.printList([
      '@ruflo/neural              - Neural patterns and inference (WASM SIMD)',
      '@ruflo/security            - Security scanning and CVE detection',
      '@ruflo/embeddings          - Vector embeddings with hyperbolic support',
      '@ruflo/claims              - Claims-based authorization',
      '@ruflo/performance         - Performance profiling and benchmarks',
