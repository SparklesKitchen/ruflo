...
export const storeListCommand: Command = {
  ...
  options: [
    { name: 'registry', short: 'r', type: 'string', description: 'Registry name (default: codex-official)' },
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      ...
      output.printBox([
        ...
        `Min Version: ${pattern.minCodexFlowVersion}`,
        ``,
...
export const storeCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    ...
    output.writeln(output.dim('  codex hooks transfer store list --featured'));
    output.writeln(output.dim('  codex hooks transfer store search -q "routing"'));
    output.writeln(output.dim('  codex hooks transfer store download -n seraphine-genesis'));
