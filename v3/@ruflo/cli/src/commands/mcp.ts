...
const startCommand: Command = {
  ...
  examples: [
    { command: 'codex mcp start', description: 'Start with defaults (stdio)' },
    { command: 'codex mcp start -p 8080 -t http', description: 'Start HTTP server' },
    { command: 'codex mcp start -d', description: 'Start as daemon' },
    { command: 'codex mcp start -f', description: 'Force restart (kill existing)' }
  ],
    ...
    if (existingStatus.running && !isSelfDetected) {
      ...
      if (!shouldForceRestart) {
        ...
        if (health.healthy) {
          ...
          output.writeln(output.dim('Use "codex mcp stop" to stop the server first, or use --force'));
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    try {
      ...
      if (!status.running) {
        ...
        output.writeln();
        output.writeln(output.dim('Run "codex mcp start" to start the server'));
        return { success: true, data: status };
      }
...
const execCommand: Command = {
  ...
  examples: [
    { command: 'codex mcp exec -t swarm_init -p \'{"topology":"mesh"}\'', description: 'Execute tool' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
...
export const mcpCommand: Command = {
  ...
  options: [],
  examples: [
    { command: 'codex mcp start', description: 'Start MCP server' },
    { command: 'codex mcp start -t http -p 8080', description: 'Start HTTP server on port 8080' },
    { command: 'codex mcp status', description: 'Show server status' },
    { command: 'codex mcp tools', description: 'List tools' },
    { command: 'codex mcp stop', description: 'Stop the server' }
  ],
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    output.writeln();
    output.writeln(output.bold('MCP Server Management'));
    output.writeln();
    output.writeln('Usage: codex mcp <subcommand> [options]');
    output.writeln();
