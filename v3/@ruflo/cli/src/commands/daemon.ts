...
const startCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const isDaemonProcess = process.env.RUFLO_DAEMON === '1';
    ...
    try {
      const stateDir = join(projectRoot, '.codex');
...
function validatePath(path: string, label: string): void {
  ...
  if (!resolved.includes('.codex') && !resolved.includes('bin')) {
...
async function startBackgroundDaemon(projectRoot: string, quiet: boolean, maxCpuLoad?: string, minFreeMemory?: string): Promise<CommandResult> {
  ...
  const stateDir = join(resolvedRoot, '.codex');
  const pidFile = join(stateDir, 'daemon.pid');
  ...
  const forkOpts: Record<string, unknown> = {
    ...
    // daemon writes its own logs via appendFileSync to .codex/logs/.
  ...
  if (!quiet) {
    ...
    output.printInfo(`Stop with: codex daemon stop`);
  }
...
async function killBackgroundDaemon(projectRoot: string): Promise<boolean> {
  const pidFile = join(projectRoot, '.codex', 'daemon.pid');
...
async function killStaleDaemonsPosix(projectRoot: string, quiet: boolean): Promise<void> {
  try {
    ...
    for (const line of lines) {
      ...
      if (!line.includes('codex') && !line.includes('@ruflo/cli')) continue;
...
const installSupervisorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (platform === 'darwin') {
      ...
      const logDir = join(projectRoot, '.codex', 'logs');
...
<dict>
    ...
    <dict>
        <key>RUFLO_DAEMON</key><string>1</string>
    </dict>
</dict>
</plist>
`;
