...
const daemonCommand: Command = {
  ...
  options: [
    ...
    },
    {
      ...
      default: '.codex/daemon.pid',
    },
    {
      name: 'log-file',
      type: 'string',
      description: 'Log file location',
      default: '.codex/daemon.log',
    },
    {
      name: 'detach',
      type: 'boolean',
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    const pidFile = (ctx.flags?.['pid-file'] as string) || '.codex/daemon.pid';
    const logFile = (ctx.flags?.['log-file'] as string) || '.codex/daemon.log';
    ...
    switch (action) {
      ...
      case 'status':
        ...
        console.log('  └─────────────────────────────────────────┘');
        if (!existingDaemon) {
          console.log('\n  To start: codex process daemon --action start');
        }
        break;
    }
...
const monitorCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const agentStorePath = resolve('.codex/agents/store.json');
    ...
    try {
      const taskStorePath = resolve('.codex/tasks/store.json');
    ...
    const metrics = {
      ...
      agents: {
        total: agentCount,
        _note: agentCount === 0 ? 'No agent store found at .codex/agents/store.json' : null,
      },
      tasks: {
        ...taskCounts,
        _note: (taskCounts.running + taskCounts.queued + taskCounts.completed + taskCounts.failed) === 0
          ? 'No task store found at .codex/tasks/store.json' : null,
      },
      memory: {
    ...
    // Dashboard format
    console.log('\n╔══════════════════════════════════════════════════════════════╗');
    console.log('║            🖥️  RUFLO PROCESS MONITOR                    ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
...
const logsCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    // Read actual log files from .codex/logs/ if they exist
    const logsDir = resolve('.codex/logs');
...
export const processCommand: Command = {
  ...
  action: async (_ctx: CommandContext): Promise<CommandResult> => {
    ...
    console.log('\nExamples:');
    console.log('  codex process daemon --action start');
    console.log('  codex process monitor --watch');
    console.log('  codex process workers --action spawn --type task --count 3');
    console.log('  codex process logs --follow --level error');
