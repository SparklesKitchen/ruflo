/**
 * Ruflo Plugin Registry Cloud Function
 *
 ...
 *     --service-account=plugin-registry-publisher@codex.iam.gserviceaccount.com
 */
...
const PROJECT_ID = process.env.GCP_PROJECT || 'codex';
const BUCKET_NAME = 'codex-plugin-registry';
...
async function exportModel(modelData, jwt) {
  const result = await pinToIPFS(modelData, `codex-model-${Date.now()}`, jwt);
  return {
    cid: result.IpfsHash,
    size: result.PinSize,
    gateways: [
  ...
  try {
    ...
    switch (action) {
      case 'publish': {
        ...
        // Pin to IPFS
        const pinResult = await pinToIPFS(registry, 'codex-plugin-registry', pinataJwt);
