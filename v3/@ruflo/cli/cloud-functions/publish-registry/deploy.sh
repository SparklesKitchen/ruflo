#!/bin/bash
# Deploy Ruflo Registry Cloud Function

set -e

PROJECT_ID="${GCP_PROJECT:-codex}"
...
gsutil mb -p $PROJECT_ID gs://codex-plugin-registry 2>/dev/null || true
...
echo ""
echo "Rate a plugin:"
echo "  curl -X POST '$FUNCTION_URL?action=rate' -H 'Content-Type: application/json' -d '{\"itemId\":\"@ruflo/embeddings\",\"rating\":5}'"
