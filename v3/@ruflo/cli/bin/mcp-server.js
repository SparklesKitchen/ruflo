#!/usr/bin/env node
/**
 * @ruflo/cli - MCP Server Entry Point
 *
 * Direct stdio MCP server for Codex integration.
  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] ERROR [codex-mcp] Failed to parse:`,
...
process.on('SIGINT', () => {
  console.error(`[${new Date().toISOString()}] INFO [codex-mcp] Received SIGINT`);
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.error(`[${new Date().toISOString()}] INFO [codex-mcp] Received SIGTERM`);
  process.exit(0);
});
  ...
  try {
    switch (message.method) {
      ...
      case 'notifications/initialized':
        console.error(`[${new Date().toISOString()}] INFO [codex-mcp] Client initialized`);
    ...
    console.error(`[${new Date().toISOString()}] ERROR [codex-mcp] ${message.method}:`, error);
    return {
      jsonrpc: '2.0',
      id: message.id,
      error: {
