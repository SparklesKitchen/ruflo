...
1. **`src/config-adapter.ts`** (NEW)
   - Converts between `SystemConfig` (from `@ruflo/shared`) and `V3Config` (CLI-specific format)
...
- `RUFLO_MAX_AGENTS` - Maximum concurrent agents
- `RUFLO_DATA_DIR` - Data directory path
- `RUFLO_MEMORY_TYPE` - Memory backend type
- `RUFLO_MCP_TRANSPORT` - MCP transport type
- `RUFLO_MCP_PORT` - MCP server port
- `RUFLO_SWARM_TOPOLOGY` - Swarm topology type
...
codex agent spawn -t coder

# Use specific config file
codex agent spawn -t coder --config ./custom-config.json

# Override with environment variables
RUFLO_MAX_AGENTS=20 codex swarm init
```
...
3. **Validation** - Uses existing Zod schemas from `@ruflo/shared`
...
- [ ] Config validation command (`codex config validate`)
