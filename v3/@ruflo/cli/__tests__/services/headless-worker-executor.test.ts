 ...
 * runs Codex in headless mode for background worker tasks.
 *
 * Tests cover:
 * 1. HeadlessWorkerExecutor instantiation
 * 2. isAvailable() - Codex availability detection
 ...
 * 7. Timeout handling
 * 8. Error handling for missing Codex
...
class HeadlessWorkerExecutor extends EventEmitter {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      const result = (execSync as Mock)('codex --version', { encoding: 'utf-8' });
      this.codexPath = 'codex';
      return result.includes('codex') || result.includes('Codex');
    } catch {
      // Try alternative path
      try {
        const altResult = (execSync as Mock)('npx ruflo-code --version', { encoding: 'utf-8' });
        this.codexPath = 'npx ruflo-code';
        return altResult.includes('codex') || altResult.includes('Codex');
      } catch {
        return false;
      }
    }
  ...
  async execute(config: HeadlessWorkerConfig): Promise<HeadlessExecutionResult> {
    ...
    if (!(await this.isAvailable())) {
      const error = 'Codex is not available. Install with: npm install -g @openai-ai/codex-code';
    ...
    return new Promise((resolve) => {
      ...
      const childProcess = (spawn as Mock)(this.codexPath || 'codex', args, {
...
describe('HeadlessWorkerExecutor', () => {
  ...
  describe('isAvailable', () => {
    it('should return true when codex --version succeeds', async () => {
      (execSync as Mock).mockReturnValue('codex version 1.0.0');

      const result = await executor.isAvailable();

      expect(result).toBe(true);
      expect(execSync).toHaveBeenCalledWith('codex --version', { encoding: 'utf-8' });
    });

    it('should return true with Codex capitalized', async () => {
      (execSync as Mock).mockReturnValue('Codex CLI v1.0.0');
    ...
    it('should try npx ruflo-code when codex fails', async () => {
      (execSync as Mock)
        .mockImplementationOnce(() => {
          throw new Error('Command not found');
        })
        .mockReturnValue('codex-code version 1.0.0');
    ...
    it('should return false when both codex and npx ruflo-code fail', async () => {
  ...
  describe('execute', () => {
    ...
    it('should pass sandbox option to codex', async () => {
      const config: HeadlessWorkerConfig = {
        workerId: 'test-6',
        workerType: 'audit',
    ...
    it('should return error when Codex is not available', async () => {
      (execSync as Mock).mockImplementation(() => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Codex is not available');
