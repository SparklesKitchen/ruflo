...
const ROOT = resolve(__dirname, '..', '..', '..', '..');            // /workspaces/codex
const CLI_DIR = resolve(__dirname, '..');                             // v3/@ruflo/cli
...
describe('Docker Compose Configuration', () => {
  ...
  it('MCP tool group env vars are defined for mcp-bridge', () => {
    const groups = [
      ...
      'MCP_GROUP_CODEX', 'MCP_GROUP_GEMINI',
      'MCP_GROUP_CODEX',
    ];
...
describe('MCP Bridge Dockerfile', () => {
  ...
  it('creates writable .codex directories', () => {
    expect(dockerContent).toContain('/app/.codex/tasks');
    expect(dockerContent).toContain('/app/.codex/memory');
    expect(dockerContent).toContain('/app/.codex/sessions');
  });
...
describe('CLI Dockerfile (ruflo:lite)', () => {
  ...
  it('prunes heavy optional dependencies to reduce image size', () => {
    const prunedPackages = [
      'agentic', '@opentelemetry', 'onnxruntime-node',
      '@openai-ai', 'agentdb',
    ];
...
describe('CLI Build', () => {
  it('package.json exists and has correct name', () => {
    ...
    expect(pkg.name).toBe('@ruflo/cli');
  });
