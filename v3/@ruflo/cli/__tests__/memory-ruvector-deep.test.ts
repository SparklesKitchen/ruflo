 ...
 * - Service layer: agentic bridge, registry-api validation
...
describe('Agentic Flow Bridge', () => {
  it('should export getReasoningBank function', async () => {
    const bridge = await import('../src/services/agentic-bridge.js');
  ...
  it('should handle missing agentic gracefully in isAvailable', async () => {
    const bridge = await import('../src/services/agentic-bridge.js');
  ...
  it('should return embedding or null for computeEmbedding', async () => {
    ...
    // If agentic is installed, returns an array-like of numbers (may be Float32Array);
    // otherwise null
  ...
  it('should return array or handle error for retrieveMemories', async () => {
    ...
    try {
      ...
      // If agentic is installed but DB not initialized, it may throw
...
describe('Registry API', () => {
  ...
  it('should reject invalid ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@ruflo/test', 0)).rejects.toThrow('Rating must be integer 1-5');
    await expect(api.rateItem('@ruflo/test', 6)).rejects.toThrow('Rating must be integer 1-5');
  });

  it('should reject non-integer ratings', async () => {
    const api = await import('../src/services/registry-api.js');
    await expect(api.rateItem('@ruflo/test', 3.5)).rejects.toThrow('Rating must be integer 1-5');
  });
  ...
  it('should accept valid item IDs with scopes', async () => {
    ...
    try {
      await api.getRating('@ruflo/test-plugin', 'plugin');
    } catch (e: any) {
