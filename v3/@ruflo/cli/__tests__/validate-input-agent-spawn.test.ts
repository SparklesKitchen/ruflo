 ...
 * `{ agentType, name }` to `@ruflo/security`'s SpawnAgentSchema, which
...
describe('validateAgentSpawn (#1567)', () => {
  ...
  it('accepts a custom agent type not in the SpawnAgentSchema enum', async () => {
    ...
    // @ruflo/security's AgentTypeSchema enumerates, but it's a valid
    // Ruflo agent type and should NOT be rejected.
