...
import { generateCodexMd, generateMinimalCodexMd, CODEX_MD_TEMPLATES } from '../src/init/codexmd-generator.js';
import { DEFAULT_INIT_OPTIONS, MINIMAL_INIT_OPTIONS, FULL_INIT_OPTIONS, detectPlatform } from '../src/init/types.js';
import type { InitOptions, CodexMdTemplate } from '../src/init/types.js';
...
describe('Init System', () => {
  describe('DEFAULT_INIT_OPTIONS', () => {
    it('should have all components enabled by default', () => {
      ...
      expect(DEFAULT_INIT_OPTIONS.components.codexMd).toBe(true);
    });
  ...
  describe('FULL_INIT_OPTIONS', () => {
    it('should have all components enabled', () => {
      ...
      expect(comps.codexMd).toBe(true);
    });
    ...
    it('should have all MCP servers enabled', () => {
      expect(FULL_INIT_OPTIONS.mcp.codexFlow).toBe(true);
  ...
  describe('generateSettings', () => {
    ...
    it('should include env with agent teams enabled', () => {
      ...
      expect(env.CODEX_EXPERIMENTAL_AGENT_TEAMS).toBe('1');
    });

    it('should include codexFlow v3 settings', () => {
      const settings = generateSettings(DEFAULT_INIT_OPTIONS) as Record<string, unknown>;
      expect(settings.codexFlow).toBeDefined();
      const cf = settings.codexFlow as Record<string, unknown>;
  ...
  describe('generateCodexMd', () => {
    it('should return non-empty string', () => {
      const md = generateCodexMd(DEFAULT_INIT_OPTIONS);
      expect(typeof md).toBe('string');
      expect(md.length).toBeGreaterThan(100);
    });

    // The AGENTS.md generator was deliberately rewritten to a terser, more
    // imperative schema (header "# Ruflo — Codex Configuration", section
    ...
    it('should describe anti-drift swarm topology', () => {
      const md = generateCodexMd(DEFAULT_INIT_OPTIONS, 'standard');
    ...
    it('full template should include hooks reference', () => {
      const md = generateCodexMd(DEFAULT_INIT_OPTIONS, 'full');
    ...
    it('security template should include security rules', () => {
      const md = generateCodexMd(DEFAULT_INIT_OPTIONS, 'security');
      expect(md).toContain('Security');
    });

    it('performance template should include performance section', () => {
      const md = generateCodexMd(DEFAULT_INIT_OPTIONS, 'performance');
      expect(md).toContain('Performance');
    });

    it('minimal template should be shorter than full template', () => {
      const minimal = generateCodexMd(DEFAULT_INIT_OPTIONS, 'minimal');
      const full = generateCodexMd(DEFAULT_INIT_OPTIONS, 'full');
      expect(minimal.length).toBeLessThan(full.length);
    });
  });

  describe('generateMinimalCodexMd', () => {
    it('should produce same result as generateCodexMd with minimal template', () => {
      const a = generateMinimalCodexMd(DEFAULT_INIT_OPTIONS);
      const b = generateCodexMd(DEFAULT_INIT_OPTIONS, 'minimal');
      expect(a).toBe(b);
    });
  });

  describe('CODEX_MD_TEMPLATES', () => {
    it('should have 6 templates', () => {
      expect(CODEX_MD_TEMPLATES.length).toBe(6);
    });

    it('should include all template names', () => {
      const names = CODEX_MD_TEMPLATES.map((t) => t.name);
    ...
    it('each template should have a description', () => {
      for (const tmpl of CODEX_MD_TEMPLATES) {
