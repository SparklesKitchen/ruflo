 ...
 *
 * Loads @ruflo/security validators when available
...
// Try to load the full @ruflo/security module for enhanced validation
...
async function getSecurityModule(): Promise<Record<string, any> | null> {
  ...
  try {
    // Dynamic import — @ruflo/security is an optional dependency
    _securityModule = await (Function('return import("@ruflo/security")')() as Promise<Record<string, any>>);
  } catch {
    // @ruflo/security is optional — fallback to inline validation above
  }
  return _securityModule;
}

/**
 * Enhanced validation using @ruflo/security Zod schemas when available.
...
export async function validateAgentSpawn(input: Record<string, unknown>): Promise<{ valid: boolean; errors: string[] }> {
  ...
  // Fix for #1567: @ruflo/security's SpawnAgentSchema expects `type` and
