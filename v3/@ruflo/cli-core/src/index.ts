#!/usr/bin/env node
/**
 * @ruflo/cli-core entry point — alpha.1 surface.
 *
 ...
 *                       stay in @ruflo/cli, dynamic-imported)
...
// Bin entry — runs when invoked as `ruflo-core <command>`.
...
const isMain = (() => {
  ...
  try {
    ...
    // npm bin-shim case: argv[1] is the .bin/ruflo-core symlink.
...
if (isMain) {
  ...
  if (args[0] === '--help' || args[0] === '-h' || args.length === 0) {
    console.log(`@ruflo/cli-core — alpha.1 (ADR-100)
...
Storage:
  ...
  RUFLO_MEMORY_PATH or --path. Search is substring-only — for semantic
  vector search, install the heavy @ruflo/cli@alpha.

Programmatic use:
  import { JsonMemoryBackend, runMemoryCommand } from '@ruflo/cli-core';

Track progress: https://github.com/ruvnet/ruflo/issues/1760`);
    process.exit(0);
  }

  console.error(`@ruflo/cli-core: command "${args[0]}" not yet wired into cli-core.
For now, use the full CLI:  npx @ruflo/cli@alpha ${args.join(' ')}`);
  process.exit(1);
}
