...
Lightweight core CLI surface for [Ruflo](https://github.com/ruvnet/ruflo) — `memory` + `hooks` commands only. Designed to load fast on a cold npx cache so plugin skills don't race Codex's 30 second MCP-startup timeout.

## Why a separate package?

Issue [#1748 #3](https://github.com/ruvnet/ruflo/issues/1748) documented a silent failure mode for new users: `npx ruflo@latest mcp start` from a cold npx cache regularly exceeds 30 seconds (1.8 MB / 999 files), Codex's MCP startup timeout fires, zero tools register, and the user observes "Ruflo is broken — no MCP tools available."

`@ruflo/cli-core` is a ≤250 KB packed subset containing only what plugin skills actually call: `memory store/list/retrieve/search/delete/init` and the `hooks` family (route, model-outcome, post-edit, pre-task, etc.). On a cold cache, `npx @ruflo/cli-core@alpha memory store ...` should complete in under 5 seconds — well under the timeout.

## Install

```bash
# Pre-release alpha
npm install @ruflo/cli-core@alpha

# Or just npx-invoke directly from a plugin Bash block
npx @ruflo/cli-core@alpha memory store --key x --value 1 --namespace patterns
```
...
## What's NOT included (use `@ruflo/cli` for these)
...
For any of these, install `@ruflo/cli@alpha` (the metapackage that re-exports cli-core and lazy-loads everything else).

## Compatibility

`@ruflo/cli-core@3.7.0-alpha.x` ships in lockstep with `@ruflo/cli@3.7.0-alpha.x`. Once promoted from alpha → latest, the two packages will continue to share the major.minor line.
...
rm -rf ~/.npm/_npx
time npx @ruflo/cli-core@alpha memory store --key smoke --value test --namespace test
...
- [Main `@ruflo/cli` README](../cli/README.md) — full feature list
