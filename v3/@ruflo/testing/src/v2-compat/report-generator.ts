...
function generateEnhancedMarkdown(report: FullValidationReport, options: ReportOptions): string {
  ...
  lines.push('|-----------|-----------|');
  lines.push('| codex/hive-mind | @ruflo/swarm |');
  lines.push('| codex/swarm | @ruflo/swarm |');
  lines.push('| codex/memory | @ruflo/memory |');
  lines.push('| codex/agents | @ruflo/agent-lifecycle |');
  lines.push('| codex/tasks | @ruflo/task-execution |');
  lines.push('');
...
function generateExamplesSection(): string[] {
  ...
  lines.push('npx ruflo hive-mind init');
  lines.push('npx ruflo hive-mind status');
  lines.push('');
  lines.push('# V3 (recommended)');
  lines.push('npx @ruflo/cli swarm init');
  lines.push('npx @ruflo/cli swarm status');
  lines.push('```');
  lines.push('');
  ...
  lines.push("import { HiveMind } from 'codex/hive-mind';");
  lines.push("import { MemoryManager } from 'codex/memory';");
  lines.push('');
  lines.push('// V3 imports with aliases');
  lines.push("import { UnifiedSwarmCoordinator as HiveMind } from '@ruflo/swarm';");
  lines.push("import { UnifiedMemoryService as MemoryManager } from '@ruflo/memory';");
  lines.push('');
...
function generateMigrationScriptsSection(): string[] {
  ...
  lines.push('npx @ruflo/cli migrate --from v2 --to v3');
  lines.push('');
  lines.push('# Migrate configuration');
  lines.push('npx @ruflo/cli migrate config --input .codex/config.yaml');
  lines.push('');
  lines.push('# Migrate memory database');
  lines.push('npx @ruflo/cli migrate memory --input .codex/memory.db');
  lines.push('```');
  lines.push('');
