...
export class IntegrationRegressionSuite {
  ...
  private registerDefaultTests(): void {
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { UnifiedMemoryService, HybridBackend } = await import('@ruflo/memory');
    ...
    this.tests.push({
      ...
      timeout: 10000,
      run: async () => {
        const { HybridBackend, UnifiedMemoryService } = await import('@ruflo/memory');
    ...
    this.tests.push({
      ...
      timeout: 5000,
      run: async () => {
        const { EventBus, createAgentSpawnedEvent } = await import('@ruflo/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const { UnifiedSwarmCoordinator } = await import('@ruflo/swarm');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const { HookRegistry, HookPriority } = await import('@ruflo/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          ...
          const shared = await import('@ruflo/shared');
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const memory = await import('@ruflo/memory');
          return (
    ...
    this.tests.push({
      ...
      run: async () => {
        try {
          const swarm = await import('@ruflo/swarm');
