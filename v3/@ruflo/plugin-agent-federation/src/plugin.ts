import type {
  ...
  AgentTypeDefinition,
} from '@ruflo/shared/src/plugin-interface.js';
...
export class AgentFederationPlugin implements CodexFlowPlugin {
  readonly name = '@ruflo/plugin-agent-federation';
  ...
  readonly dependencies = ['@ruflo/security', '@ruflo/aidefence'];
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    // signatures. Persist to .codex/federation/key-<nodeId>.json so
    ...
    const keyDir = join(process.cwd(), '.codex', 'federation');
    ...
    return [
      {
        ...
        defaultConfig: {
          ...
          timeout: 300_000,
          metadata: {
            pluginSource: '@ruflo/plugin-agent-federation',
          },
        },
