---
name: browser
description: Web browser automation with AI-optimized snapshots for codex agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @ruflo/cli memory store --namespace browser-patterns --key "login" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @ruflo/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @ruflo/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @ruflo/cli hooks post-task --task-id "browse-1" --success true
```
