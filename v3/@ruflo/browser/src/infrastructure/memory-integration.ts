/**
 * @ruflo/browser - Memory Integration
...
/**
 * Adapter for codex memory system with HNSW indexing
 */
export class CodexFlowMemoryAdapter implements IMemoryAdapter {
  ...
  async store(entry: BrowserMemoryEntry): Promise<void> {
    ...
    try {
      // This would call codex memory_store MCP tool
...
export class BrowserMemoryManager {
  ...
  constructor(sessionId: string, adapter?: IMemoryAdapter) {
    ...
    this.adapter = adapter || new CodexFlowMemoryAdapter();
  }
...
export function getMemoryAdapter(): IMemoryAdapter {
  if (!defaultAdapter) {
    defaultAdapter = new CodexFlowMemoryAdapter();
  }
