...
[![npm version](https://img.shields.io/npm/v/@ruflo/embeddings.svg)](https://www.npmjs.com/package/@ruflo/embeddings)
[![npm downloads](https://img.shields.io/npm/dm/@ruflo/embeddings.svg)](https://www.npmjs.com/package/@ruflo/embeddings)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0+-blue.svg)](https://www.typescriptlang.org/)
[![Performance](https://img.shields.io/badge/Performance-<5ms-brightgreen.svg)](https://github.com/SparklesKitchen/ruflo)
...
- **Auto-Install** - Automatically installs agentic when using `provider: 'auto'`
- **Smart Fallback** - Graceful fallback chain: agentic → transformers → mock
...
- **75x Faster** - Agentic ONNX is 75x faster than Transformers.js
...
codex embeddings embed "Your text here"

# Batch embed from file
codex embeddings batch documents.txt -o embeddings.json

# Similarity search
codex embeddings search "query" --index ./vectors

# Initialize agentic model
codex embeddings init --provider agentic
```
...
const autoService = await createEmbeddingServiceAsync({
  ...
  autoInstall: true,      // Install agentic if missing
...
```typescript
import { OpenAIEmbeddingService } from '@ruflo/embeddings';
...
```typescript
import { AgenticFlowEmbeddingService } from '@ruflo/embeddings';
...
```typescript
import { TransformersEmbeddingService } from '@ruflo/embeddings';
...
```typescript
import { MockEmbeddingService } from '@ruflo/embeddings';
...
```typescript
import { createEmbeddingService } from '@ruflo/embeddings';
...
```typescript
import { chunkText, estimateTokens, reconstructFromChunks } from '@ruflo/embeddings';
...
Access agentic's neural features for advanced embedding operations:
...
```typescript
import { PersistentEmbeddingCache, isPersistentCacheAvailable } from '@ruflo/embeddings';
...
```bash
# Document chunking
codex embeddings chunk document.txt --strategy sentence --max-size 512

# Normalize embedding file
codex embeddings normalize embeddings.json --type l2 -o normalized.json

# Convert to hyperbolic
codex embeddings hyperbolic embeddings.json -o poincare.json

# Neural operations
codex embeddings neural drift --baseline "context" --input "check this"
codex embeddings neural store --id mem-1 --content "data"
codex embeddings neural recall "query" --top-k 5

# List/download models
codex embeddings models list
codex embeddings models download all-MiniLM-L6-v2

# Cache management
codex embeddings cache stats
codex embeddings cache clear --older-than 7d
```

## Related Packages

- [@ruflo/memory](../memory) - HNSW indexing and vector storage
- [@ruflo/providers](../providers) - Multi-LLM provider system
- [@ruflo/neural](../neural) - SONA learning integration
