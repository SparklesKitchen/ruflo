 ...
 *
 * Production embedding service aligned with agentic@alpha:
...
export class AgenticFlowEmbeddingService extends BaseEmbeddingService {
  readonly provider: EmbeddingProvider = 'agentic';
  ...
  private async initialize(): Promise<void> {
    ...
    possiblePaths.push('agentic/embeddings');
    ...
    try {
      ...
      const absolutePaths = [
        path.join(cwd, 'node_modules/agentic/dist/embeddings/optimized-embedder.js'),
        path.join(cwd, '../node_modules/agentic/dist/embeddings/optimized-embedder.js'),
        '/workspaces/codex/node_modules/agentic/dist/embeddings/optimized-embedder.js',
      ];
    ...
    throw new Error(
      `Failed to initialize agentic embeddings.${errorDetail} ` +
      `Ensure agentic is installed and ONNX model is downloaded: ` +
      `npx agentic@alpha embeddings init`
    );
  }
  ...
  async embed(text: string): Promise<EmbeddingResult> {
    ...
    try {
      // Use agentic's optimized embedder (has its own internal cache)
      ...
      const message = error instanceof Error ? error.message : 'Unknown error';
      this.emitEvent({ type: 'embed_error', text, error: message });
      throw new Error(`Agentic embedding failed: ${message}`);
    }
  }
...
async function isAgenticFlowAvailable(): Promise<boolean> {
  try {
    await import('agentic/embeddings');
    return true;
  } catch {
    return false;
  }
...
/**
 * Auto-install agentic and initialize model
 */
async function autoInstallAgenticFlow(): Promise<boolean> {
  ...
  try {
    ...
    console.log('[embeddings] Installing agentic@alpha...');
    await execAsync('npm install agentic@alpha --save', { timeout: 120000 });

    // Initialize the model
    console.log('[embeddings] Downloading embedding model...');
    await execAsync('npx agentic@alpha embeddings init', { timeout: 300000 });
  ...
  switch (config.provider) {
    ...
    default:
      throw new Error(
        ...
        `Use 'agentic' (recommended), 'transformers', 'openai', 'rvf', or 'mock' (tests only).`
      );
  }
}
...
export interface AutoEmbeddingConfig {
  /** Provider: 'auto' will pick best available (agentic > transformers > mock) */
  provider: EmbeddingProvider | 'auto';
  /** Fallback provider if primary fails */
  fallback?: EmbeddingProvider;
  /** Auto-install agentic if not available (default: true for 'auto' provider) */
  autoInstall?: boolean;
  /** Model ID for agentic */
  modelId?: string;
 ...
 *
 * Features:
 * - 'auto' provider picks best available: agentic > transformers > mock
 ...
 *
 * // Try agentic, fallback to transformers
...
export async function createEmbeddingServiceAsync(
  ...
  if (provider === 'auto') {
    ...
    // Try agentic (fastest neural, ONNX-based)
    ...
    throw new Error(
      ...
      'Install agentic OR @xenova/transformers, OR pass provider:"openai" with apiKey, ' +
