...
export class PluginLoader {
   ...
   */
  async loadPlugin(plugin: CodexFlowPlugin, context: PluginContext): Promise<void> {
    // Validate plugin
   ...
   * Reload a plugin
   */
  async reloadPlugin(pluginName: string, newPlugin: CodexFlowPlugin, context: PluginContext): Promise<void> {
    await this.unloadPlugin(pluginName);
   ...
   */
  private validatePlugin(plugin: CodexFlowPlugin): void {
    if (!plugin.name) {
      throw new PluginError(
   ...
   */
  private validateDependencies(plugin: CodexFlowPlugin): void {
   ...
   */
  private async initializePlugin(plugin: CodexFlowPlugin, context: PluginContext): Promise<void> {
   ...
   */
  private async shutdownPlugin(plugin: CodexFlowPlugin): Promise<void> {
   ...
   * Build dependency graph
   */
  private buildDependencyGraph(plugins: CodexFlowPlugin[]): Map<string, DependencyNode> {
   ...
   */
  private topologicalSort(graph: Map<string, DependencyNode>): CodexFlowPlugin[] {
    const sorted: CodexFlowPlugin[] = [];
   ...
   */
  private groupByDepth(graph: Map<string, DependencyNode>): CodexFlowPlugin[][] {
    const levels: CodexFlowPlugin[][] = [];
    const maxDepth = Math.max(...Array.from(graph.values()).map((n) => n.depth));

    for (let depth = 0; depth <= maxDepth; depth++) {
      const level: CodexFlowPlugin[] = [];
