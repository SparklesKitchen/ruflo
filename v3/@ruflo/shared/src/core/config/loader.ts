...
function loadEnvConfig(): Partial<SystemConfig> {
  ...
  if (process.env.RUFLO_MAX_AGENTS) {
    config.orchestrator = {
      ...defaultSystemConfig.orchestrator,
      lifecycle: {
        ...defaultSystemConfig.orchestrator.lifecycle,
        maxConcurrentAgents: parseInt(process.env.RUFLO_MAX_AGENTS, 10),
      },
    };
  }

  // Data directory
  if (process.env.RUFLO_DATA_DIR) {
    config.orchestrator = {
      ...config.orchestrator,
      ...defaultSystemConfig.orchestrator,
      session: {
        ...defaultSystemConfig.orchestrator.session,
        dataDir: process.env.RUFLO_DATA_DIR,
      },
    };
  }

  // Memory type
  if (process.env.RUFLO_MEMORY_TYPE) {
    const memoryType = process.env.RUFLO_MEMORY_TYPE as NonNullable<SystemConfig['memory']>['type'];
  ...
  // MCP transport
  const defaultMcp = defaultSystemConfig.mcp ?? { name: 'codex', version: '3.0.0', transport: { type: 'stdio' as const } };
  if (process.env.RUFLO_MCP_TRANSPORT) {
    const transport = process.env.RUFLO_MCP_TRANSPORT as 'stdio' | 'http' | 'websocket';
  ...
  if (process.env.RUFLO_MCP_PORT) {
    config.mcp = {
      ...
      transport: {
        ...
        ...defaultMcp.transport,
        port: parseInt(process.env.RUFLO_MCP_PORT, 10),
      },
    };
  }
  ...
  if (process.env.RUFLO_SWARM_TOPOLOGY) {
    const topology = process.env.RUFLO_SWARM_TOPOLOGY as NonNullable<SystemConfig['swarm']>['topology'];
