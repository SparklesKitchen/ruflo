...
export class GitCommitHook {
  ...
  async processCommitMessage(
    ...
    // Add Codex reference and co-author
    let coAuthorAdded = false;
    if (this.config.addCodexReference || this.config.addCoAuthor) {
      const additions: string[] = [];

      if (this.config.addCodexReference) {
