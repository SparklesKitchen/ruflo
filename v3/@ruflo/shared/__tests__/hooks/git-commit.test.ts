...
describe('GitCommitHook', () => {
  ...
  describe('co-author addition', () => {
    it('should add co-author by default', async () => {
      ...
      expect(result.modifiedMessage).toContain('Codex');
    });

    it('should add Codex reference', async () => {
      const result = await gitCommit.process('Add feature');
    ...
    it('should not duplicate co-author if already present', async () => {
      ...
      // Should still add Codex co-author
      expect(result.modifiedMessage).toContain('Codex');
    });
  });
  ...
  describe('configuration', () => {
    ...
    it('should allow disabling Codex reference', async () => {
      const hook = createGitCommitHook(registry, { addCodexReference: false });
      const result = await hook.process('Add feature');

      expect(result.modifiedMessage).not.toContain('Codex');
    });
