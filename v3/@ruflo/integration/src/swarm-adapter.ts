/**
 * SwarmAdapter - Bridge between V3 Swarm and agentic@alpha Patterns
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Ruflo v3 UnifiedSwarmCoordinator
 * - agentic's AttentionCoordinator, SwarmTopology, and Expert routing
 *
 * This implements ADR-001: Adopt agentic as Core Foundation
 * by aligning V3 swarm patterns with agentic's coordination mechanisms.
 *
 * Key Alignments:
...
// ============================================================================
// agentic Pattern Types (Target Interface)
// ============================================================================

/**
 * agentic SwarmTopology types
...
/**
 * agentic Attention mechanism types
 */
...
/**
 * agentic AgentOutput interface
 * This is the expected output format from agents in agentic swarms
 */
...
/**
 * agentic SpecializedAgent interface
...
/**
 * agentic Attention coordination result
 */
...
// ============================================================================
// V3 Swarm Types (Source Interface from @ruflo/swarm)
// ============================================================================

/**
 * V3 Topology types (from @ruflo/swarm)
 */
export type V3TopologyType = 'mesh' | 'hierarchical' | 'centralized' | 'hybrid';

/**
 * V3 Agent Domain types (from @ruflo/swarm)
 */
export type V3AgentDomain = 'queen' | 'security' | 'core' | 'integration' | 'support';

/**
 * V3 Agent State interface (simplified from @ruflo/swarm)
 */
...
/**
 * V3 Task Definition interface (simplified from @ruflo/swarm)
 */
...
export interface SwarmAdapterConfig {
  ...
  /** GraphRoPE dimension */
  ropeDimension: number;
  /** Enable delegation to agentic when available */
...
/**
 * SwarmAdapter - Bridges V3 Swarm with agentic patterns
 *
 * Key Features:
 * - Topology conversion (V3 <-> agentic)
 ...
 * Usage:
 * ```typescript
 * import { SwarmAdapter, createSwarmAdapter } from '@ruflo/integration';
 *
 ...
 * });
 *
 * // Convert V3 agents to agentic format
...
export class SwarmAdapter extends EventEmitter {
  ...
  /**
   * Reference to agentic core for delegation
   */
  private agenticFlowCore: any = null;

  /**
   * Reference to agentic AttentionCoordinator
   */
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Attempt to connect to agentic for delegation
  ...
  /**
   * Convert V3 topology type to agentic topology
   *
   * Mapping:
   * - mesh -> mesh
   * - hierarchical -> hierarchical
   * - centralized -> star (agentic uses 'star' for central coordinator pattern)
  ...
  /**
   * Convert agentic topology to V3 topology type
   */
  ...
  /**
   * Convert V3 Agent to agentic AgentOutput format
   *
   * Creates the embedding from agent capabilities and produces
   * the standardized AgentOutput interface expected by agentic.
   */
  toAgentOutput(
    agent: V3AgentState,
    value: unknown,
  ...
  /**
   * Convert V3 Agent to agentic SpecializedAgent format
   *
  ...
  /**
   * Convert agentic SpecializedAgent back to partial V3 format
   * (for updates/sync)
   */
   ...
   *
   * Implements agentic's expert routing pattern for task assignment.
  ...
  async routeToExperts(
    ...
    // If delegation is available and enabled, use agentic's MoE
   ...
   *
   * Implements agentic's attention-based consensus pattern
  ...
  async coordinateWithAttention(
    ...
    // If delegation is available, use agentic's AttentionCoordinator
    if (
  ...
  /**
   * Map V3 domain to agentic specialization
   */
  ...
  /**
   * Map agentic specialization to V3 domain
   */
  ...
  /**
   * Check if delegation to agentic is available
   */
  ...
  private async connectToAgenticFlow(): Promise<void> {
    try {
      ...
      const agenticFlowModule: any = await import('agentic').catch(() => null);
