/**
 * AgentAdapter - Bridge between Ruflo and agentic Agents
 *
 * Provides bidirectional conversion and delegation patterns between:
 * - Ruflo v3 DDD agent architecture
 * - agentic's optimized agent implementations
 *
...
import {
  ...
  AgentStatus,
  AgentType,
} from './agentic-agent.js';

/**
 * Interface for agentic Agent (external package)
 * This represents the expected API from agentic's Agent class
 */
...
export interface AgentAdapterConfig {
  /** Enable bidirectional sync with agentic */
  enableSync: boolean;
...
/**
 * AgentAdapter - Bridges Ruflo and agentic agents
 *
 ...
 * 4. Fallback handling when agentic is unavailable
 *
 * Usage:
 * ```typescript
 ...
 *
 * // Convert agentic agent to Ruflo agent
 * const { agent, success } = adapter.fromAgenticFlow(agenticFlowAgent);
 *
 * // Create Ruflo agent with agentic delegation
...
export class AgentAdapter extends EventEmitter {
  ...
  /**
   * Reference to agentic core for accessing Agent factory
   */
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Attempt to load agentic for delegation
  ...
  /**
   * Convert agentic agent to Ruflo AgenticFlowAgent
   *
   * This method creates a Ruflo agent wrapper around an existing
   * agentic agent instance, enabling delegation and integration.
   *
   * @param agenticFlowAgent - agentic agent instance
  ...
  fromAgenticFlow(
    ...
    try {
      // Map agentic config to Ruflo config
  ...
  /**
   * Convert Ruflo AgenticFlowAgent to agentic format
   *
   * This exports the agent configuration in a format compatible with
   * agentic's Agent constructor/factory.
   *
   * @param agent - Ruflo agent instance
   * @returns agentic compatible configuration
   */
  ...
  /**
   * Create a Ruflo agent with agentic delegation
   *
   * This is the primary method for creating agents in v3 with
   * automatic delegation to agentic when available.
   *
   * ADR-001: Eliminates duplicate code by delegating to agentic
  ...
  async createWithDelegation(config: AgentConfig): Promise<AgenticFlowAgent> {
    ...
    // If agentic is available, create delegation reference
  ...
  /**
   * Check if an agent is delegated to agentic
   */
  ...
  /**
   * Connect to agentic package dynamically
   */
  private async connectToAgenticFlow(): Promise<void> {
    try {
      ...
      if (agenticFlowModule && typeof agenticFlowModule.createAgenticFlow === 'function') {
        ...
        this.logDebug('agentic not available, using local implementations');
      }
    } catch (error) {
      this.agenticFlowCore = null;
      ...
      this.logDebug('agentic connection failed', error);
    }
  }

  /**
   * Map agentic agent type to Ruflo AgentType
   */
  ...
  /**
   * Extract capabilities from agentic config
   */
  ...
  /**
   * Sync status between Ruflo and agentic agents
   */
  private syncStatus(
    agent: AgenticFlowAgent,
    agenticFlowAgent: AgenticFlowAgent_External
  ): void {
    // Map agentic status to Ruflo status
