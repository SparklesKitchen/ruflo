 ...
 * Multi-Model Router
 *
 * Cost-optimized routing across multiple LLM providers from agentic@alpha:
 * - openai: Codex models
 * - openai: GPT models
...
const DEFAULT_MODELS: ModelConfig[] = [
  // OpenAI
  {
    id: 'codex-3-5-sonnet-20241022',
    name: 'Codex 3.5 Sonnet',
    provider: 'openai',
...
export class MultiModelRouter extends EventEmitter {
  ...
  getEstimatedSavings(request: RoutingRequest): {
    ...
    // Default cost (using Codex 3 Sonnet as baseline)
    const defaultModel = this.models.get('codex-3-5-sonnet-20241022')!;
    const defaultCost =
  ...
  private initializeProviderHealth(): void {
    const providers: ProviderType[] = [
      'openai', 'openai', 'openrouter', 'ollama', 'litellm', 'onnx', 'gemini', 'custom'
    ];
