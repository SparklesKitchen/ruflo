/**
 * SDK Bridge for agentic API Compatibility
 *
 * Provides API compatibility layer between codex v3 and
 * agentic@alpha, handling version negotiation, feature
 ...
 * and API translation between codex and agentic.
 */
export class SDKBridge extends EventEmitter {
  ...
  private async detectVersion(): Promise<SDKVersion> {
    // Detect agentic version dynamically
    try {
      const af = await import('agentic');
      const version = (af as Record<string, unknown>)['VERSION'] as string | undefined;
      if (version) {
        return this.parseVersion(version);
      }
    } catch {
      // agentic not available, use fallback version
    }
