/**
 * AgenticFlowAgent - Ruflo Agent with agentic Delegation
 *
 * Per ADR-001: "Use agentic's Agent base class for all agents"
 * This class wraps agentic functionality while adding Ruflo specifics.
 *
 * This implements the adapter pattern to bridge between:
 * - Ruflo's agent lifecycle (v3 DDD architecture)
 * - agentic's optimized agent implementations
 *
 * When agentic is available, this class delegates core operations to
 * agentic's Agent implementations, eliminating 10,000+ lines of duplicate code.
 *
 ...
 *
 * @module v3/integration/agentic-agent
...
/**
 * Interface for agentic Agent reference (for delegation)
 * This represents the agentic Agent class API
 */
...
/**
 * AgenticFlowAgent - Base class for all Ruflo v3 agents
 *
 * This class serves as the foundation for all agent types in Ruflo v3,
 * implementing ADR-001 by delegating to agentic when available while
...
export class AgenticFlowAgent extends EventEmitter implements IAgent {
  ...
  /**
   * Reference to agentic Agent for delegation (ADR-001)
   * When set, core operations delegate to agentic's optimized implementations
   */
  private agenticFlowRef: AgenticFlowAgentReference | null = null;

  /**
   * Indicates if delegation to agentic is active
   */
   ...
   *
   * @param ref - The agentic Agent reference
   */
  ...
  /**
   * Check if delegation to agentic is enabled
   */
  ...
  /**
   * Get the agentic reference (if available)
   */
   ...
   *
   * ADR-001: When agentic is available, delegates initialization
   * to agentic's Agent.initialize() for optimized setup.
   */
  async initialize(): Promise<void> {
    ...
    try {
      // ADR-001: Delegate to agentic when available
   ...
   *
   * ADR-001: When agentic is available, delegates shutdown
   * to agentic's Agent.shutdown() for clean termination.
   */
   ...
   * Execute a task
   *
   * ADR-001: When agentic is available, delegates task execution
   * to agentic's Agent.execute() which leverages:
  ...
  async executeTask(task: Task): Promise<TaskResult> {
    ...
    try {
      ...
      // ADR-001: Delegate to agentic when available for optimized execution
      if (this.isDelegationEnabled() && this.agenticFlowRef?.execute) {
        ...
        // Local execution (fallback or when agentic not available)
   ...
   *
   * ADR-001: When agentic is available, delegates to agentic's
