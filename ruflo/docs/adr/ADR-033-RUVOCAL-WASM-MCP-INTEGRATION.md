...
- All ruflo-specific docs/AGENTS.md guidance
...
| Cloud Run deploy (after granting `secretmanager.secretAccessor` to default SA on `OPENAI_API_KEY`, `GOOGLE_AI_API_KEY`, `OPENROUTER_API_KEY`) | Service `ruvocal` revision `00007-4hd` serving 100% traffic |
...
| Provider API keys via Secret Manager | Mounted at runtime as `OPENAI_API_KEY`, `GOOGLE_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_API_KEY` |
...
### Provider switch: OpenRouter / Codex Sonnet 4.6 default
...
Switched the deployed default to **Codex Sonnet 4.6 via OpenRouter** with `OPENAI_BASE_URL=https://openrouter.ai/api/v1` and `OPENAI_API_KEY` mapped from the `OPENROUTER_API_KEY` Secret Manager secret. Models exposed: Codex Sonnet 4.6 (default), Codex Opus 4.7, Codex Haiku 4.5, Gemini 2.5 Pro, Gemini 2.5 Flash, GPT-4o.
...
| **P2** | **Parallel tool-call visualization parity with Codex task panel** | Server-side parallel execution works; UI renders one tool-call card per call but doesn't visually group them as a single "step" with collapsed thumbnails, lane layout, or per-tool durations. UX pass needed on `ChatMessage.svelte` and `ToolUpdate.svelte`. |
...
| **P3** | **Help-modal copy fix** | Quick Start section says "default: Gemini 2.5 Flash". Update to "default: Codex Sonnet 4.6" in `src/lib/components/RufloHelpModal.svelte`. |
