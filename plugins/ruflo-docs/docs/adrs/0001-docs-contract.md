...
Real surface used:
- `mcp__codex__hooks_worker-dispatch` with `trigger: "document"` (works; the `document` worker is one of the 12 background workers per AGENTS.md)
- `mcp__codex__memory_store` for drift detection state
