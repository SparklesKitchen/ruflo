...
Use these MCP tools:
- `mcp__codex__ruvllm_generate_config` / `ruvllm_status` for configuration
- `mcp__codex__ruvllm_microlora_*` for fine-tuning
- `mcp__codex__ruvllm_sona_*` for SONA adaptation
- `mcp__codex__ruvllm_hnsw_*` for HNSW indexes
- `mcp__codex__ruvllm_chat_format` for prompt formatting
...
```bash
npx @ruflo/cli@latest memory store --namespace llm-configs --key "config-PROVIDER-MODEL" --value "PARAMS_AND_RESULTS"
npx @ruflo/cli@latest memory search --query "config for PROVIDER" --namespace llm-configs
```
