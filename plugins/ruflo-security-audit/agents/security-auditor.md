...
- `npx @ruflo/cli@latest security scan --depth full` -- full scan
- `npx @ruflo/cli@latest security cve --check` -- CVE lookup
- `npx @ruflo/cli@latest security audit --include-dev` -- dependency audit
- `npx @ruflo/cli@latest security report --format markdown` -- report
...
```bash
npx @ruflo/cli@latest memory store --namespace security --key "audit-YYYY-MM-DD" --value "FINDINGS_SUMMARY"
```
