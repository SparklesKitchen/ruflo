---
name: create-plugin
description: Scaffold a new Codex plugin with proper directory structure, plugin.json, skills, commands, and agents
argument-hint: "<plugin-name>"
allowed-tools: mcp__codex__transfer_plugin-info mcp__codex__transfer_plugin-search mcp__codex__transfer_store-search Bash Read Write Edit
---

# Create Plugin

Scaffold a new Codex plugin from scratch.

## When to use

When you want to create a new plugin that extends Codex with skills, commands, and agents. This generates the correct directory structure and wires up MCP tools.

## Steps

1. **Get plugin name and description** from the user
2. **Check for conflicts** — call `mcp__codex__transfer_plugin-search` to ensure the name isn't taken
3. **Create directory structure** (follows the canonical plugin contract from sibling plugins' ADR-0001s):
   ...
   ```
4. **Generate plugin.json** with name, description, version, author (do NOT include `skills`, `commands`, or `agents` arrays — Codex auto-discovers these from directory structure)
5. **Generate SKILL.md files** with proper frontmatter:
   ```yaml
   ---
   name: skill-name
   description: What this skill does
   allowed-tools: mcp__codex__tool1 mcp__codex__tool2 Bash
   ---
   ```
...
8. **Generate README.md** with install instructions, features, commands, skills, AND the canonical plugin-contract sections:
   - **Compatibility** — pin to `@ruflo/cli` v3.6 major+minor
...
**Do NOT include** `skills`, `commands`, or `agents` arrays in plugin.json — these are auto-discovered from the directory structure by Codex and will cause validation errors if present.

## Available MCP tools to wire

Browse available tools: `mcp__codex__transfer_plugin-info`
