...
description: Validate a Codex plugin structure, frontmatter, and MCP tool references
argument-hint: "[plugin-path]"
allowed-tools: mcp__codex__transfer_plugin-info Bash Read Glob Grep
---

# Validate Plugin

Validate that a plugin follows the correct Codex plugin format.
...
1. **Directory structure** — `.codex-plugin/plugin.json` exists at plugin root
...
8. **No files in wrong locations** — skills/commands/agents not inside `.codex-plugin/`
9. **MCP tool references** — tools in `allowed-tools` are valid `mcp__codex__*` identifiers
