...
Use these MCP tools:
- `mcp__codex__workflow_create` / `workflow_delete` for definition
- `mcp__codex__workflow_execute` / `workflow_run` for execution
- `mcp__codex__workflow_pause` / `workflow_resume` / `workflow_cancel` for control
- `mcp__codex__workflow_status` / `workflow_list` for monitoring
- `mcp__codex__workflow_template` for templates
...
```bash
npx @ruflo/cli@latest memory store --namespace workflow-patterns --key "workflow-NAME" --value "TEMPLATE_AND_METRICS"
npx @ruflo/cli@latest memory search --query "workflow for TASK_TYPE" --namespace workflow-patterns
```
