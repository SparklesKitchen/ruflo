...
Use these MCP tools:
- `mcp__codex__wasm_agent_create` / `wasm_agent_terminate` for lifecycle
- `mcp__codex__wasm_agent_prompt` / `wasm_agent_tool` for interaction
- `mcp__codex__wasm_agent_files` / `wasm_agent_export` for data management
- `mcp__codex__wasm_gallery_*` for gallery operations
...
```bash
npx @ruflo/cli@latest memory store --namespace wasm-patterns --key "agent-TYPE" --value "CONFIG_AND_PERFORMANCE"
npx @ruflo/cli@latest memory search --query "wasm agent for TASK" --namespace wasm-patterns
```
