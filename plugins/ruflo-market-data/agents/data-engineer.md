...
- `mcp__codex__agentdb_hierarchical-store` -- store normalized OHLCV data and pattern metadata
- `mcp__codex__agentdb_hierarchical-recall` -- recall historical market data by symbol/period
- `mcp__codex__agentdb_pattern-store` -- store detected candlestick patterns with vectors
- `mcp__codex__agentdb_pattern-search` -- search for similar patterns via HNSW
- `mcp__codex__agentdb_semantic-route` -- route queries to relevant market data sources
- `mcp__codex__embeddings_generate` -- generate embeddings for pattern descriptions
- `mcp__codex__ruvllm_hnsw_create` -- create HNSW index for pattern vectors
- `mcp__codex__ruvllm_hnsw_add` -- add pattern vectors to HNSW index
- `mcp__codex__ruvllm_hnsw_route` -- nearest-neighbor search in pattern index
...
npx @ruflo/cli@latest neural train --pattern-type market-data --epochs 15
```
...
```bash
npx @ruflo/cli@latest memory store --namespace market-data --key "symbol-SYMBOL" --value "OHLCV_SUMMARY_JSON"
npx @ruflo/cli@latest memory store --namespace market-patterns --key "pattern-PATTERN_ID" --value "PATTERN_METADATA_JSON"
npx @ruflo/cli@latest memory search --query "bearish reversal patterns for AAPL" --namespace market-patterns
```
