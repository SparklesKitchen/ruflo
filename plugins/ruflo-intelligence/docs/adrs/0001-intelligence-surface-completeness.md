...
- `.codex-plugin/plugin.json:4` — `version: "0.1.0"`, keywords `intelligence, sona, neural, learning, routing`
...
|--------|-----------------|------------|--------|
| `neural_*` | 5 of 6 | **6** (`neural_train`, `neural_predict`, `neural_patterns`, `neural_compress`, `neural_status`, `neural_optimize`) | `v3/@ruflo/cli/src/mcp-tools/neural-tools.ts:195, 312, 413, 539, 651, 706` |
| `hooks_intelligence_*` (and dispatcher / reset) | 6 of 10 | **10** (`hooks_intelligence`, `hooks_intelligence-reset`, `hooks_intelligence_trajectory-start/step/end`, `hooks_intelligence_pattern-store`, `hooks_intelligence_pattern-search`, `hooks_intelligence_stats`, `hooks_intelligence_learn`, `hooks_intelligence_attention`) | `v3/@ruflo/cli/src/mcp-tools/hooks-tools.ts:2093, 2226, 2296, 2355, 2404, 2556, 2634, 2741, 2952, 3027` |
...
| `ruvllm_sona_*` + `ruvllm_microlora_*` | 2 of 4 (only sona_create / sona_adapt) | **4** | `v3/@ruflo/cli/src/mcp-tools/ruvllm-tools.ts:142, 169, 192, 222` |
| **Total** | ~14 of **29** | **29** | — |
...
AGENTS.md describes the **4-step intelligence pipeline (RETRIEVE → JUDGE → DISTILL → CONSOLIDATE)**. The plugin never surfaces this pipeline in user-facing form, even though it's the conceptual frame the whole substrate revolves around.

### Namespace coordination

`ruflo-agentdb` ADR-0001 (just landed) introduced a namespace convention with three reserved namespaces: `pattern` (ReasoningBank fallback), `codex-memories` (Codex bridge), `default`. **The intelligence pipeline writes to `pattern`** (via `agentdb_pattern-store` and the `hooks post-task --train-neural` path). The plugin should explicitly cite the convention so consumers don't reinvent it.
...
- **The 4-step intelligence pipeline** — operationalized as a section that maps each step (RETRIEVE / JUDGE / DISTILL / CONSOLIDATE) to specific tool calls. Aligns with AGENTS.md but is now verifiable.
...
- **Compatibility** — pinned to `@ruflo/cli` v3.6 (matches ruflo-agentdb).
...
13. Compatibility section pins to `@ruflo/cli` v3.6.
...
- The 4-step intelligence pipeline (AGENTS.md's V3 framing) is now operationalized in a user-facing form.
...
- `plugins/ruflo-agentdb/docs/adrs/0001-agentdb-optimization.md` — namespace convention + reserved namespaces (`pattern`/`codex-memories`/`default`); pluralization gotcha; hook integration table
...
- `v3/@ruflo/cli/src/mcp-tools/neural-tools.ts` — 6 neural tool definitions
- `v3/@ruflo/cli/src/mcp-tools/hooks-tools.ts` — 19 hooks-family tools (intelligence, route, model, transfer, metrics, explain, pretrain, build-agents)
- `v3/@ruflo/cli/src/mcp-tools/ruvllm-tools.ts` — 4 SONA + MicroLoRA tools
- AGENTS.md (V3 Performance Targets + 4-step pipeline framing)
