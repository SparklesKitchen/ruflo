...
`ruflo-loop-workers` (v0.1.0) — cache-aware `/loop` workers + `CronCreate` background automation. Wraps **5 `hooks_worker-*` MCP tools** at `v3/@ruflo/cli/src/mcp-tools/hooks-tools.ts:3489, 3538, 3656, 3714, 3923` (`worker-list`, `worker-dispatch`, `worker-status`, `worker-detect`, `worker-cancel`) and exposes 12 background worker triggers per AGENTS.md.
...
- `v3/@ruflo/cli/src/mcp-tools/hooks-tools.ts` — 5 `hooks_worker-*` tools
