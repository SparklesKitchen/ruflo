...
- **Monitor Streams**: Real-time swarm status via `Monitor("npx @ruflo/cli@latest swarm watch --stream")`
...
Sources: `v3/@ruflo/cli/src/mcp-tools/swarm-tools.ts:71, 145, 208, 270` and `agent-tools.ts:182, 287, 319, 356, 395, 451, 573, 651`.

## Built-in Codex coordination tools

This plugin pairs with Codex's native multi-agent tools (no MCP needed):
...
## Anti-drift defaults (per AGENTS.md)
...
This plugin owns the `swarm-state` AgentDB namespace (kebab-case, follows the convention from [ruflo-agentdb ADR-0001 §"Namespace convention"](../ruflo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `codex-memories`, `default`) MUST NOT be shadowed.
