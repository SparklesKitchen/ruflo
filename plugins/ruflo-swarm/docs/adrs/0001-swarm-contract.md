...
| Family | Count | Source |
|--------|-------|--------|
| `swarm_*` | 4 (`init`, `status`, `shutdown`, `health`) | `v3/@ruflo/cli/src/mcp-tools/swarm-tools.ts:71, 145, 208, 270` |
| `agent_*` | 8 (`spawn`, `execute`, `terminate`, `status`, `list`, `pool`, `health`, `update`) | `v3/@ruflo/cli/src/mcp-tools/agent-tools.ts:182, 287, 319, 356, 395, 451, 573, 651` |

Plus the Monitor + Task tools from Codex (built-in: `Task`, `TaskList`, `TaskGet`, `TaskUpdate`, `Monitor`, etc.) which pair with this plugin for live streaming.

## Decision

1. Add this ADR (Proposed).
2. README augment: Compatibility (pin v3.6); 12-tool MCP surface table; Monitor/Task built-in cross-reference; Anti-drift guidance (hierarchical topology, max 8 agents per AGENTS.md); Namespace coordination (claims `swarm-state`); Verification + Architecture Decisions sections.
...
**Positive:** plugin joins the cadence. The 12-tool surface is now contractually documented. Anti-drift defaults from AGENTS.md are smoke-checked.
...
- `v3/@ruflo/cli/src/mcp-tools/swarm-tools.ts` — 4 `swarm_*` tools
- `v3/@ruflo/cli/src/mcp-tools/agent-tools.ts` — 8 `agent_*` tools
