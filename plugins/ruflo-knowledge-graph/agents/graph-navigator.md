...
- `mcp__codex__agentdb_causal-edge` -- create/query causal edges between entities
- `mcp__codex__agentdb_hierarchical-store` -- store entity metadata in hierarchical structure
- `mcp__codex__agentdb_hierarchical-recall` -- recall entities by path or query
- `mcp__codex__agentdb_semantic-route` -- semantic similarity routing for graph search
- `mcp__codex__agentdb_pattern-store` -- store discovered graph patterns
- `mcp__codex__agentdb_pattern-search` -- search for similar graph structures
- `mcp__codex__agentdb_context-synthesize` -- synthesize context from multiple graph nodes
- `mcp__codex__embeddings_generate` -- generate embeddings for entity descriptions
...
npx @ruflo/cli@latest neural train --pattern-type knowledge-graph --epochs 10
```
...
```bash
npx @ruflo/cli@latest memory store --namespace knowledge-graph --key "entity-ENTITY_NAME" --value "ENTITY_METADATA_JSON"
npx @ruflo/cli@latest memory store --namespace knowledge-graph --key "pattern-PATTERN_NAME" --value "GRAPH_PATTERN_JSON"
npx @ruflo/cli@latest memory search --query "entities related to authentication" --namespace knowledge-graph
```
