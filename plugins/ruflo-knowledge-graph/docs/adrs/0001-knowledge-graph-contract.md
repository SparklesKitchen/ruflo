...
Two files reference `mcp__codex__embeddings_embed`, but the real tool is `embeddings_generate` (`v3/@ruflo/cli/src/mcp-tools/embeddings-tools.ts:260`). There is no `embeddings_embed` MCP tool.
...
- `v3/@ruflo/cli/src/mcp-tools/embeddings-tools.ts:260` — `embeddings_generate` (the real tool name)
