...
- `mcp__codex__agentdb_hierarchical-store` -- store trace spans and log entries
- `mcp__codex__agentdb_hierarchical-recall` -- recall traces by traceId or correlationId
- `mcp__codex__agentdb_pattern-store` -- store anomaly patterns for future detection
- `mcp__codex__agentdb_pattern-search` -- search for similar anomaly patterns
- `mcp__codex__agentdb_semantic-route` -- route observability queries to relevant data
- `mcp__codex__agentdb_context-synthesize` -- synthesize context from multiple trace spans
...
npx @ruflo/cli@latest neural train --pattern-type observability --epochs 10
```
...
```bash
npx @ruflo/cli@latest memory store --namespace observability --key "trace-TRACE_ID" --value "TRACE_SUMMARY_JSON"
npx @ruflo/cli@latest memory store --namespace observability-patterns --key "anomaly-ANOMALY_TYPE" --value "ANOMALY_SIGNATURE_JSON"
npx @ruflo/cli@latest memory search --query "latency spikes in authentication flow" --namespace observability
```
