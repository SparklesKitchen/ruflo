...
- `mcp__codex__agentdb_hierarchical-store` / `agentdb_hierarchical-query` — ADR tree storage.
- `mcp__codex__agentdb_causal-edge` / `agentdb_causal-query` — relationship tracking.
- `mcp__codex__memory_store` / `memory_search` — semantic search.
...
```bash
npx @ruflo/cli@latest memory store --namespace adr-patterns --key "decision-CATEGORY" --value "CONTEXT_AND_OUTCOME"
npx @ruflo/cli@latest memory search --query "architectural decision" --namespace adr-patterns
```
