...
4. **No Compatibility section.** Plugin doesn't pin to `@ruflo/cli` v3.6.
...
- **Compatibility** — pin to `@ruflo/cli` v3.6 (matches the cadence).
- **Namespace coordination** — explicit deferral to `ruflo-agentdb` ADR-0001 §"Namespace convention". `adr-patterns` is the canonical namespace this plugin owns; the reserved namespaces (`pattern`, `codex-memories`, `default`) MUST NOT be shadowed.
...
6. README has Compatibility section pinning to `@ruflo/cli` v3.6.
