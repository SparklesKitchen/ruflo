...
Direct wrapper around `agent-booster.apply()` (npm `agent-booster` v0.2.x, exposed via `agentic/agent-booster`). Use when a transform is **already classified** as Tier 1 eligible — `cost-booster-route` recommends *whether*; this skill *executes*.
...
| **`350×` speedup vs. LLM**         | **Verified — exceeded against every tier:** 1000.9× vs Gemini 2.0 Flash, **1838.7× vs Codex Sonnet 4.6**, **2634.1× vs Codex Opus 4.7**. Run `BENCH_LLM_BASELINE=1 BENCH_OpenAI=1 node scripts/bench.mjs` to refresh. |
...
**Override the LLM model:** `BENCH_LLM_MODEL='codex-sonnet-4'` (when wired against `api.openai.com`) or `BENCH_LLM_MODEL='models/gemini-2.5-flash'` for a reasoning-model comparison. Pricing flags: `BENCH_LLM_PRICE_IN`, `BENCH_LLM_PRICE_OUT`.
