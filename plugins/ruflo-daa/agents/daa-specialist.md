...
Use these MCP tools:
- `mcp__codex__daa_agent_create` / `daa_agent_adapt` for agent management
- `mcp__codex__daa_cognitive_pattern` for reasoning patterns
- `mcp__codex__daa_workflow_create` / `daa_workflow_execute` for workflows
- `mcp__codex__daa_knowledge_share` for cross-agent learning
- `mcp__codex__daa_learning_status` / `daa_performance_metrics` for monitoring
...
```bash
npx @ruflo/cli@latest memory store --namespace daa-patterns --key "cognitive-PATTERN" --value "ADAPTATION_DATA"
npx @ruflo/cli@latest memory search --query "cognitive pattern for TASK" --namespace daa-patterns
```
