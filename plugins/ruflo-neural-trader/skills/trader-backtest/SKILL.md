...
allowed-tools: Bash Read mcp__codex__memory_store mcp__codex__memory_retrieve mcp__codex__memory_search mcp__codex__neural_train mcp__codex__agentdb_pattern-store
...
2. Check for saved strategy config:
   `mcp__codex__memory_retrieve({ key: "strategy-STRATEGY_NAME", namespace: "trading-strategies" })`
   If not found, list available: `mcp__codex__memory_search({ query: "strategy", namespace: "trading-strategies", limit: 10 })`
...
5. Store backtest results:
   `mcp__codex__memory_store({ key: "backtest-STRATEGY-TIMESTAMP", value: "RESULTS_JSON", namespace: "trading-backtests" })`
6. If Sharpe > 1.5, store as successful pattern:
   `mcp__codex__agentdb_pattern-store({ pattern: "profitable-STRATEGY_TYPE", data: "PARAMS_AND_RESULTS" })`
7. Train SONA on the outcome:
   `mcp__codex__neural_train({ patternType: "trading-strategy", epochs: 10 })`
