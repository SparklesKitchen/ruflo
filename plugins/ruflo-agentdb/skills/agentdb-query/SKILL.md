...
argument-hint: "<query>"
allowed-tools: mcp__codex__agentdb_semantic-route mcp__codex__agentdb_hierarchical-recall mcp__codex__agentdb_hierarchical-store mcp__codex__agentdb_context-synthesize mcp__codex__agentdb_causal-edge mcp__codex__agentdb_pattern-search mcp__codex__agentdb_pattern-store mcp__codex__agentdb_controllers mcp__codex__agentdb_health mcp__codex__agentdb_batch mcp__codex__agentdb_feedback mcp__codex__agentdb_consolidate mcp__codex__agentdb_session-start mcp__codex__agentdb_session-end Bash
---
...
1. **Check health** — `mcp__codex__agentdb_health`. Sanity-check `available: true`.
2. **Start session** — `mcp__codex__agentdb_session-start` if not already active.
3. **Store knowledge** — `mcp__codex__agentdb_hierarchical-store` for structured tier-keyed data (tiers: `working|episodic|semantic`).
4. **Recall knowledge** — `mcp__codex__agentdb_hierarchical-recall` with a query.
5. **Search patterns** — `mcp__codex__agentdb_pattern-search` for learned patterns (ReasoningBank-routed).
6. **Synthesize context** — `mcp__codex__agentdb_context-synthesize` to combine multiple memories.
7. **Build causal graph** — `mcp__codex__agentdb_causal-edge` to link related knowledge.

## Available controller groups

Call `mcp__codex__agentdb_controllers` to list the runtime registry. Functional categories surfaced via the 15 MCP tools:
...
```bash
npx @ruflo/cli@latest memory search --query "your query" --namespace patterns
npx @ruflo/cli@latest memory store --key "key" --value "value" --namespace patterns
