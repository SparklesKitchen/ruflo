...
allowed-tools: mcp__codex__embeddings_generate mcp__codex__embeddings_search mcp__codex__embeddings_compare mcp__codex__embeddings_init mcp__codex__embeddings_status mcp__codex__embeddings_hyperbolic mcp__codex__embeddings_neural mcp__codex__embeddings_rabitq_build mcp__codex__embeddings_rabitq_search mcp__codex__embeddings_rabitq_status mcp__codex__ruvllm_hnsw_create mcp__codex__ruvllm_hnsw_add mcp__codex__ruvllm_hnsw_route mcp__codex__memory_search_unified Bash
---
...
|------|-------------|---------|----------|---------|
| **Large-scale corpus** | `embeddings_*` | `@ruflo/memory` HNSW (Rust/Native) | up to millions of vectors | 150×–12,500× faster than brute-force, depending on N and parameters |
...
1. **Check status** — `mcp__codex__embeddings_status` to verify the embedding engine.
2. **Initialize** — `mcp__codex__embeddings_init` if not active.
3. **Generate** — `mcp__codex__embeddings_generate` for text input.
4. **Search** — `mcp__codex__embeddings_search` with the query.
5. **Compare** — `mcp__codex__embeddings_compare` to measure similarity.
6. **Unified search** — `mcp__codex__memory_search_unified` for cross-namespace.
...
- `mcp__codex__ruvllm_hnsw_create` — create the WASM index (cap ~11)
- `mcp__codex__ruvllm_hnsw_add` — add a pattern
- `mcp__codex__ruvllm_hnsw_route` — route an incoming query

This is **not** a corpus index. Treat it as a fast classifier over a curated set of patterns.

## Hyperbolic embeddings

For hierarchical data (code trees, org charts), use `mcp__codex__embeddings_hyperbolic` which maps to Poincare ball space. Distance is geodesic, not cosine.

## CLI alternative

```bash
npx @ruflo/cli@latest embeddings search --query "authentication patterns"
npx @ruflo/cli@latest embeddings init
npx @ruflo/cli@latest memory search --query "your query"
```
