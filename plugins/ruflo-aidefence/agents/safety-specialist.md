...
Use these MCP tools:
- `mcp__codex__aidefence_scan` / `aidefence_analyze` / `aidefence_is_safe` for scanning
- `mcp__codex__aidefence_has_pii` / `mcp__codex__transfer_detect-pii` for PII
- `mcp__codex__aidefence_learn` to train on confirmed threats
- `mcp__codex__aidefence_stats` for metrics
...
```bash
npx @ruflo/cli@latest memory store --namespace security-patterns --key "threat-TYPE" --value "PATTERN_DATA"
npx @ruflo/cli@latest memory search --query "similar threats" --namespace security-patterns
```
