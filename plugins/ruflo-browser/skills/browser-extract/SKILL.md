...
allowed-tools: mcp__codex__browser_open mcp__codex__browser_close mcp__codex__browser_get-text mcp__codex__browser_get-value mcp__codex__browser_eval mcp__codex__browser_snapshot mcp__codex__browser_screenshot mcp__codex__browser_scroll mcp__codex__browser_wait mcp__codex__browser_click mcp__codex__aidefence_has_pii mcp__codex__aidefence_is_safe mcp__codex__aidefence_scan Bash Read Write
---
...
3. **Choose a path**:
   - **Template path** (`--template <name>`): retrieve from AgentDB and apply.
     ```bash
     npx -y @ruflo/cli@latest memory retrieve --namespace browser-templates --key "<name>"
     ```
...
4. **AIDefence pre-storage**: every extracted string passes the PII gate.
   ```bash
   # Pseudocode — mcp__codex__aidefence_has_pii returns true/false per string.
...
6. **Persist the template** if `--save-template <name>` was passed:
   ```bash
   npx -y @ruflo/cli@latest memory store --namespace browser-templates \
