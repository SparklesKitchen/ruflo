...
Session-as-skill browser automation. Playwright-backed via 23 `mcp__codex__browser_*` tools, with each session captured as a first-class **RVF cognitive container** holding manifest + trajectory + screenshots + sanitized cookies + findings, indexed in AgentDB and gated by AIDefence.
...
| `browser-auth` | Probe an auth flow for redirect leaks, missing CSRF, weak session cookies. |
...
23 existing `mcp__codex__browser_*` tools (interaction primitive) **+ 5 new `browser_session_*` lifecycle tools (implemented in v0.2.0)**:
...
Implementation: [`v3/@ruflo/cli/src/mcp-tools/browser-session-tools.ts`](../../v3/@ruflo/cli/src/mcp-tools/browser-session-tools.ts), registered in `mcp-client.ts`. Each handler shells out to the pinned `ruvector@0.2.25` CLI for trajectory + RVF, the existing `agent-browser` CLI for browser actions, and the bridged `codex memory` for AgentDB. Missing dependencies degrade with structured `success: false` errors instead of crashing.
