...
1. **ls [--query "<text>"] [--host <host>] [--verdict pass|fail|partial]**
   ...
   ```bash
   npx -y @ruflo/cli@latest memory list --namespace browser-sessions
   # or with semantic filter:
   npx -y @ruflo/cli@latest memory search --namespace browser-sessions --query "QUERY"
   ```
   For active (live) sessions, also call `mcp__codex__browser_session-list`.
...
7. **doctor**
   ...
   - Playwright runner reachable (`mcp__codex__browser_open` against `about:blank`)
   - AgentDB controllers initialized (`mcp__codex__agentdb_health`)
   - AIDefence loaded (`mcp__codex__aidefence_stats`)
