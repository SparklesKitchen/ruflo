...
- `.codex-plugin/plugin.json:4` — `version: "0.1.0"`, keywords `mcp, orchestration, codex-code`
- `.mcp.json` — registers `ruflo` MCP server via `npx -y @ruflo/cli@latest`
...
2. **No smoke test.**
3. **No Compatibility section** pinning to `@ruflo/cli` v3.6.
...
- **Compatibility** — pin to `@ruflo/cli` v3.6. Note the `npx -y @ruflo/cli@latest` invocation in `.mcp.json` is the dynamic resolver; smoke verifies the resolved version.
...
- `v3/@ruflo/cli/` — the MCP server source backing this plugin
