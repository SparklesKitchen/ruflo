...
allowed-tools: Bash(mkdir * find * ls *) Read Write Edit Grep Glob mcp__codex__memory_store mcp__codex__memory_search mcp__codex__agentdb_hierarchical-store mcp__codex__hooks_pre-task mcp__codex__hooks_post-task
---
...
2. **Pre-task hook**: `npx @ruflo/cli@latest hooks pre-task --description "DDD aggregate: <aggregate-name> in <context>"`
...
9. **Store in domain model graph**:
   ```
   mcp__codex__agentdb_hierarchical-store --parent "context:<context>" --child "aggregate:<aggregate-name>" --relation "contains"
   mcp__codex__memory_store --key "ddd-aggregate-<context>-<aggregate-name>" --value "AGGREGATE_SUMMARY" --namespace tasks
   ```

10. **Post-task hook**: `npx @ruflo/cli@latest hooks post-task --task-id "ddd-aggregate-<aggregate-name>" --success true --train-neural true`
