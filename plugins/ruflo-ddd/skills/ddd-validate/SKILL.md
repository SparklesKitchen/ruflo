...
argument-hint: ""
allowed-tools: Bash(find * grep * npx *) Read Grep Glob mcp__codex__memory_store mcp__codex__memory_search mcp__codex__hooks_pre-task mcp__codex__hooks_post-task
---
...
7. **Store results**:
   ```bash
   npx @ruflo/cli@latest memory store --key "ddd-validation-TIMESTAMP" --value "RESULTS_SUMMARY" --namespace tasks
   npx @ruflo/cli@latest hooks post-task --task-id "ddd-validate" --success true --store-results true
   ```
