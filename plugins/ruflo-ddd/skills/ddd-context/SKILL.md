...
allowed-tools: Bash(mkdir * find * ls *) Read Write Edit Grep Glob mcp__codex__memory_store mcp__codex__memory_search mcp__codex__agentdb_hierarchical-store
---
...
4. **Store in domain model graph**:
   ```
   mcp__codex__agentdb_hierarchical-store --parent "domain" --child "context:<context-name>" --relation "contains"
   mcp__codex__memory_store --key "ddd-context-<context-name>" --value "Created bounded context" --namespace tasks
   ```
