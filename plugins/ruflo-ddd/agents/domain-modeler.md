...
- `npx @ruflo/cli@latest memory search --query "domain MODEL" --namespace patterns` — retrieve prior domain models.
- `npx @ruflo/cli@latest memory store --key "domain-CONTEXT" --value "MODEL" --namespace tasks` — persist domain decisions.
...
```bash
npx @ruflo/cli@latest memory search --query "bounded context DOMAIN" --namespace patterns
npx @ruflo/cli@latest memory search --query "aggregate DOMAIN" --namespace tasks
```
...
npx @ruflo/cli@latest memory store --key "ddd-pattern-CONTEXT" --value "APPROACH" --namespace patterns
```
