...
|---|---|---|
| Hybrid memory | `mcp__codex__memory_search_unified` | Any concept |
| Pattern store | `mcp__codex__agentdb_pattern-search` | Repeated patterns |
| Hierarchical recall | `mcp__codex__agentdb_hierarchical-recall` | Layered context |
| Vector (HNSW) | `mcp__codex__embeddings_search` | Semantic neighbors |
| Knowledge graph | `mcp__codex__hooks_intelligence_pattern-search` + `kg-traverse` | Entity edges |
...
| ADR index | `mcp__codex__memory_search` namespace `adr` | ADR-ids, design decisions |
...
- **Trajectory recording**: call `mcp__codex__hooks_intelligence_trajectory-start` at begin, `_step` per round, `_end` at completion.
