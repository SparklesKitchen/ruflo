...
```bash
npx @ruflo/cli@latest memory search --query "$ARGUMENTS" --limit 5
```
...
This searches across patterns, tasks, solutions, feedback, security, and codex-memories namespaces. Results are ranked by composite score: cosine similarity * MMR diversity * recency decay.

If no arguments provided, show recent memory entries:
```bash
npx @ruflo/cli@latest memory list --limit 10
```
