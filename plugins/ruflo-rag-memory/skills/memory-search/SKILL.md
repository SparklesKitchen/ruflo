...
allowed-tools: Bash Read mcp__codex__memory_search mcp__codex__memory_store mcp__codex__memory_list mcp__codex__memory_retrieve mcp__codex__memory_search_unified mcp__codex__agentdb_pattern-search mcp__codex__agentdb_context-synthesize
...
2. **Select retrieval strategy**:
   ...
   **Dense search (default)**:
   ...
   ```
   Or via MCP: `mcp__codex__memory_search({ query: "QUERY", namespace: "NAMESPACE", limit: 10 })`
   ...
   **Smart retrieval** (when --smart or complex recall needed):
   ```bash
   npx @ruflo/cli@latest memory search --query "QUERY" --smart --limit 10
   ```
   Or via MCP: `mcp__codex__memory_search({ query: "QUERY", smart: true, limit: 10 })`
   ...
   **Unified cross-namespace**:
   `mcp__codex__memory_search_unified({ query: "QUERY", limit: 10 })`
...
5. **Synthesize context** (for complex queries):
   `mcp__codex__agentdb_context-synthesize({ query: "QUERY", sources: ["patterns", "tasks", "solutions"] })`
