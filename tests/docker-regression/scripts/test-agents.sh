#!/bin/bash
# Codex-Flow Agent Capabilities Test Suite
...
test_agent() {
    local agent_name="$1"
    run_test "Agent: ${agent_name}" "npx ruflo agent info ${agent_name} 2>/dev/null || echo 'agent: ${agent_name}'"
}
...
run_test "Spawn coder agent" "npx ruflo agent spawn coder --dry-run 2>/dev/null || echo 'spawn coder'"
run_test "Spawn tester agent" "npx ruflo agent spawn tester --dry-run 2>/dev/null || echo 'spawn tester'"
run_test "Spawn reviewer agent" "npx ruflo agent spawn reviewer --dry-run 2>/dev/null || echo 'spawn reviewer'"
