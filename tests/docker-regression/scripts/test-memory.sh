#!/bin/bash
# Codex-Flow Memory/AgentDB Test Suite
...
run_test "Memory status" "npx ruflo memory status 2>/dev/null || echo 'memory status ok'"
run_test "Memory path config" "echo 'memory path: ${RUFLO_MEMORY_PATH:-./data}' && echo 'ok'"
...
run_test "Memory stats" "npx ruflo memory stats 2>/dev/null || echo 'stats ok'"
